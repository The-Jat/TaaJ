 ...
 */
#ifndef	_K_LAYOUT_UTILS_H
#define	_K_LAYOUT_UTILS_H
...
//	static	float				SubtractSizesFloat(float a, float b);
	...
	static	void				AlignInFrame(KView* view, BRect frame);
	static	BRect				AlignOnRect(BRect rect, BSize size, BAlignment alignment);
	static	BRect				MoveIntoFrame(BRect rect, BSize frameSize);

	// debugging
	static	BString				GetLayoutTreeDump(KView* view);
	static	BString				GetLayoutTreeDump(KLayoutItem* item);

private:
	static	void				_GetLayoutTreeDump(KView* view, int level,
									BString& _output);
	static	void				_GetLayoutTreeDump(KLayoutItem* item,
