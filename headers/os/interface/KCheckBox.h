 ...
 */
#ifndef _K_CHECK_BOX_H
#define _K_CHECK_BOX_H


#include <KControl.h>


class KCheckBox : public KControl {
public:
								KCheckBox(BRect frame, const char* name,
									const char* label, BMessage* message,
									uint32 resizingMode = B_FOLLOW_LEFT_TOP,
									uint32 flags = B_WILL_DRAW | B_NAVIGABLE);
								KCheckBox(const char* name, const char* label,
									BMessage* message, uint32 flags
										= B_WILL_DRAW | B_NAVIGABLE);
								KCheckBox(const char* label,
									BMessage* message = NULL);
								KCheckBox(BMessage* data);
