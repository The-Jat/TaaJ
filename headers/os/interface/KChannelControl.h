 ...
 */
#ifndef _K_CHANNEL_CONTROL_H
#define _K_CHANNEL_CONTROL_H


//! KChannelControl is the base class for controls that have several
...
class KChannelControl : public KControl {
public:
								KChannelControl(BRect frame, const char* name,
									const char* label, BMessage* model,
									int32 channelCount = 1,
									uint32 resizingMode = B_FOLLOW_LEFT_TOP,
									uint32 flags = B_WILL_DRAW);
								KChannelControl(const char* name,
									const char* label, BMessage* model,
									int32 channelCount = 1,
									uint32 flags = B_WILL_DRAW);
								KChannelControl(BMessage* archive);
	virtual						~KChannelControl();
...
private:
	// Forbidden (and unimplemented)
								KChannelControl(const KChannelControl& other);
			KChannelControl&	operator=(const KChannelControl& other);
...
inline int32* const&
KChannelControl::MinLimitList() const
{
	return fChannelMin;
}
...
inline int32* const&
KChannelControl::MaxLimitList() const
{
	return fChannelMax;
}
...
inline int32* const&
KChannelControl::ValueList() const
{
