 ...
 */
#ifndef _K_LIST_VIEW_H
#define _K_LIST_VIEW_H
...
class KListView : public KView, public BInvoker {
public:
								KListView(BRect frame, const char* name,
									list_view_type type
										= B_SINGLE_SELECTION_LIST,
									uint32 resizeMask = B_FOLLOW_LEFT_TOP,
									uint32 flags = B_WILL_DRAW
										| B_FRAME_EVENTS | B_NAVIGABLE);
								KListView(const char* name,
									list_view_type type
										= B_SINGLE_SELECTION_LIST,
									uint32 flags = B_WILL_DRAW
										| B_FRAME_EVENTS | B_NAVIGABLE);
								KListView(list_view_type type
									= B_SINGLE_SELECTION_LIST);
								KListView(BMessage* data);
	...
	virtual void				TargetedByScrollView(KScrollView* view);
	...
	virtual void				SetListType(list_view_type type);
			...
			KListItem*			ItemAt(int32 index) const;
			int32				IndexOf(BPoint point) const;
			int32				IndexOf(KListItem* item) const;
			KListItem*			FirstItem() const;
			KListItem*			LastItem() const;
			bool				HasItem(KListItem* item) const;
			int32				CountItems() const;
	virtual void				MakeEmpty();
			bool				IsEmpty() const;
			void				DoForEach(bool (*func)(KListItem* item));
			void				DoForEach(bool (*func)(KListItem* item,
									void* arg), void* arg);
			const KListItem**	Items() const;
	...
	// These functions bottleneck through DoMiscellaneous()
			...
			bool				ReplaceItem(int32 index, KListItem* item);
...
protected:
	...
	union MiscData {
		...
		struct Replace { int32 index; KListItem *item; } replace;
...
private:
	...
	virtual void				DrawItem(KListItem* item, BRect itemRect,
			...
			bool				_ReplaceItem(int32 index, KListItem* item);
...
inline void
KListView::ScrollTo(float x, float y)
{
	ScrollTo(BPoint(x, y));
}
