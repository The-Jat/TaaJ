 ...
 */
#ifndef K__TEXTVIEW_H
#define K__TEXTVIEW_H
...
class KTextView : public KView {
public:
								KTextView(BRect frame, const char* name,
	...
	virtual	status_t			Perform(perform_code code, void* data);
			...
			void				SetText(const char* text,
									const k_text_run_array* runs = NULL);
	...
	virtual	void				Select(int32 startOffset, int32 endOffset);
			...
			k_text_run_array*		RunArray(int32 startOffset, int32 endOffset,
	...
	static	k_text_run_array*		AllocRunArray(int32 entryCount,
									int32* outSize = NULL);
	static	k_text_run_array*		CopyRunArray(const k_text_run_array* orig,
									int32 countDelta = 0);
	static	void				FreeRunArray(k_text_run_array* array);
	static	void*				FlattenRunArray(const k_text_run_array* runArray,
									int32* _size = NULL);
	static	k_text_run_array*		UnflattenRunArray(const void* data,
									int32* _size = NULL);

protected:
	virtual	void				InsertText(const char* text, int32 length,
									int32 offset, const k_text_run_array* runs);
	virtual	void				DeleteText(int32 fromOffset, int32 toOffset);

public:
	virtual	void				Undo(BClipboard* clipboard);
			k_undo_state			UndoState(bool* isRedo) const;

protected:
	virtual	void				GetDragParameters(BMessage* drag,
									KBitmap** _bitmap, BPoint* point,
									BHandler** _handler);
...
private:
			...
			void				_DrawLine(KView* view, const int32 &startLine,
			...
			void				_FilterDisallowedChars(char* text,
									ssize_t& length, k_text_run_array* runArray);

			void				_UpdateInsets(const BRect& rect);

private:
			BPrivate::KTextGapBuffer*	fText;
			LineBuffer*			fLines;
			StyleBuffer*		fStyles;
			BRect				fTextRect;
			int32				fSelStart;
			...
			BPoint				fWhere;
			KTextTrackState*		fTrackingMouse;
