 ...
 */
#ifndef	K__SPACE_LAYOUT_ITEM_H
#define	K__SPACE_LAYOUT_ITEM_H

#include <KLayoutItem.h>


class KSpaceLayoutItem : public KLayoutItem {
public:
								KSpaceLayoutItem(BSize minSize, BSize maxSize,
									BSize preferredSize, BAlignment alignment);
								KSpaceLayoutItem(BMessage* archive);
	virtual						~KSpaceLayoutItem();

	static	KSpaceLayoutItem*	CreateGlue();
	static	KSpaceLayoutItem*	CreateHorizontalStrut(float width);
	static	KSpaceLayoutItem*	CreateVerticalStrut(float height);
...
private:
	...
	// forbidden methods
								KSpaceLayoutItem(const KSpaceLayoutItem&);
			void				operator =(const KSpaceLayoutItem&);
