 ...
 */
#ifndef	_K_SCROLL_BAR_H
#define	_K_SCROLL_BAR_H
...
class KScrollBar : public KView {
public:
								KScrollBar(BRect frame, const char* name,
									KView* target, float min, float max,
									orientation direction);
								KScrollBar(const char* name, KView* target,
									float min, float max,
									orientation orientation);
								KScrollBar(BMessage* archive);
	virtual						~KScrollBar();
	...
	virtual	void				ValueChanged(float newValue);
			...
			// to be used in case the KScrollBar should draw part of
...
private:
	...
	// disabled
			KScrollBar&			operator=(const KScrollBar& other);
