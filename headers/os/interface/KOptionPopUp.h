 ...
 */
#ifndef _K_OPTION_POP_UP_H
#define _K_OPTION_POP_UP_H
...
class KOptionPopUp : public KOptionControl {
public:
								KOptionPopUp(BRect frame, const char* name,
								...
								KOptionPopUp(BRect frame, const char* name,
									...
									bool fixed, uint32 resizeMask
										= B_FOLLOW_LEFT_TOP,
									uint32 flags = B_WILL_DRAW);
								KOptionPopUp(const char* name,
									const char* label, BMessage* message,
									uint32 flags = B_WILL_DRAW);

	virtual						~KOptionPopUp();
...
private:
	// Forbidden and FBC padding
								KOptionPopUp();
								KOptionPopUp(const KOptionPopUp& other);
			KOptionPopUp&		operator=(const KOptionPopUp& other);
