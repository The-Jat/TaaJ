...
class KBox : public KView {
	public:
							KBox(BRect frame, const char* name = NULL,
								uint32 resizingMode = B_FOLLOW_LEFT_TOP,
								uint32 flags = B_WILL_DRAW | B_FRAME_EVENTS
									| B_NAVIGABLE_JUMP,
								border_style border = B_FANCY_BORDER);
							KBox(const char* name,
								uint32 flags = B_WILL_DRAW | B_FRAME_EVENTS
									| B_NAVIGABLE_JUMP,
								border_style border = B_FANCY_BORDER,
								KView* child = NULL);
							KBox(border_style border, KView* child);
		virtual				~KBox();

		/* Archiving */
							KBox(BMessage* archive);
		...
		status_t			SetLabel(KView* viewLabel);

		const char*			Label() const;
		KView*				LabelView() const;
