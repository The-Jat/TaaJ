 ...
 */
#ifndef _K_TAB_VIEW_H
#define _K_TAB_VIEW_H
...
class KTab : public BArchivable {
public:
								KTab(KView* contentsView = NULL);
	virtual						~KTab();

								KTab(BMessage* archive);
	...
	virtual	void				Select(KView* owner);
	...
	virtual	void				SetView(KView* view);
			KView*				View() const;

	virtual	void				DrawFocusMark(KView* owner, BRect frame);
	virtual	void				DrawLabel(KView* owner, BRect frame);
	virtual	void				DrawTab(KView* owner, BRect frame,
...
private:
	...
	virtual	void				_ReservedTab12();
			...
			KTab&				operator=(const KTab&);

private:
			uint32				_Borders(KView* owner, BRect frame);
...
public:
			enum tab_side {
				...
				kBottomSide	= 1 << 3
								...
								KTabView(const char* name,
									button_width width = B_WIDTH_FROM_WIDEST,
									uint32 flags = B_FULL_UPDATE_ON_RESIZE
										| B_WILL_DRAW | B_NAVIGABLE_JUMP
										| B_FRAME_EVENTS | B_NAVIGABLE);
								KTabView(BRect frame, const char* name,
	...
	// KTabView
	virtual	void				AddTab(KView* target, KTab* tab = NULL);
	virtual	KTab*				RemoveTab(int32 tabIndex);

	virtual	KTab*				TabAt(int32 index) const;
	...
	virtual	void				SetTabSide(tab_side tabSide);
			...
			KView*				ContainerView() const;

			int32				CountTabs() const;
			KView*				ViewForTab(int32 tabIndex) const;

			int32				IndexOf(KTab* tab) const;
