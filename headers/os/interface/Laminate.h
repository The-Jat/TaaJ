...
enum {
	...
	B_FULL_POINTER_HISTORY				= 0x00000008
		...
		// specified, both KWindow and KView::GetMouse()
...
public:
								KView(const char* name, uint32 flags,
									KLayout* layout = NULL);
								KView(BRect frame, const char* name,
									uint32 resizingMode, uint32 flags);
	virtual						~KView();

								KView(BMessage* archive);
	...
	virtual	void				TargetedByScrollView(KScrollView* scrollView);
			...
			void				DragMessage(BMessage* message, KBitmap* bitmap,
	...
	virtual	void				ConstrainClippingRegion(BRegion* region);
			void				ClipToPicture(KPicture* picture,
									BPoint where = B_ORIGIN, bool sync = true);
			void				ClipToInversePicture(KPicture* picture,
									BPoint where = B_ORIGIN, bool sync = true);
	...
	virtual	void				SetPenSize(float size);
			...
			void				AdoptViewColors(KView* view);
	...
	virtual	void				SetViewColor(rgb_color color);
			...
			void				SetViewBitmap(const KBitmap* bitmap,
			...
			status_t			SetViewOverlay(const KBitmap* overlay,
									BRect srcRect, BRect dstRect,
									rgb_color* colorKey,
									uint32 followFlags = B_FOLLOW_LEFT_TOP,
									uint32 options = 0);
	...
	virtual	void				SetLowColor(rgb_color color);
			...
			void				DrawBitmapAsync(const KBitmap* aBitmap,
			...
			void				DrawBitmap(const KBitmap* aBitmap,
									BRect bitmapRect, BRect viewRect,
									uint32 options);
			void				DrawBitmap(const KBitmap* aBitmap,
									BRect bitmapRect, BRect viewRect);
			...
			void				DrawTiledBitmapAsync(const KBitmap* aBitmap,
									BRect viewRect, BPoint phase = B_ORIGIN);

			void				DrawTiledBitmap(const KBitmap* aBitmap,
									BRect viewRect, BPoint phase = B_ORIGIN);
	...
	virtual	void				SetFont(const BFont* font,
			...
			void				BeginPicture(KPicture* a_picture);
			void				AppendToPicture(KPicture* a_picture);
			KPicture*			EndPicture();

			void				DrawPicture(const KPicture* a_picture);
			void				DrawPicture(const KPicture* a_picture,
									BPoint where);
			void				DrawPicture(const char* filename, long offset,
									BPoint where);
			void				DrawPictureAsync(const KPicture* a_picture);
			void				DrawPictureAsync(const KPicture* a_picture,
									BPoint where);
...
public:
	// tool tip support
			...
			void				SetToolTip(KToolTip* tip);
			KToolTip*			ToolTip() const;

			void				ShowToolTip(KToolTip* tip = NULL);
			void				HideToolTip();

protected:
	virtual	bool				GetToolTipAt(BPoint point, KToolTip** _tip);

	virtual	void				LayoutChanged();

			status_t			ScrollWithMouseWheelDelta(KScrollBar*, float);

private:
			void				_Layout(bool force, BLayoutContext* context);
			void				_LayoutLeft(KLayout* deleted);
...
private:
	...
	virtual	void				_ReservedView16();
								...
								KView(const KView&);
			KView&				operator=(const KView&);

private:
	struct LayoutData;

	friend class KPrivate;
	friend class KBitmap;
	friend class KLayout;
	friend class BPrintJob;
	friend class KScrollBar;
	friend class KShelf;
	friend class KTabView;
	friend class KWindow;

			void				_InitData(BRect frame, const char* name,
									uint32 resizingMode, uint32 flags);
			status_t			_SetViewBitmap(const KBitmap* bitmap,
									BRect srcRect, BRect dstRect,
									uint32 followFlags, uint32 options);
			void				_ClipToPicture(KPicture* picture, BPoint where,
			...
			bool				_CreateSelf();
			bool				_AddChildToList(KView* child,
									KView* before = NULL);
			bool				_RemoveChildFromList(KView* child);

			bool				_AddChild(KView *child, KView *before);
			bool				_RemoveSelf();
	...
	// Debugging methods
			...
			KPicture*			fCurrentPicture;
			_array_data_*		fCommArray;

			KScrollBar*			fVerScroller;
			KScrollBar*			fHorScroller;
			bool				fIsPrinting;
			bool				fAttached;
			bool				_unused_bool1;
			bool				_unused_bool2;
			::BPrivate::KViewState* fState;
			BRect				fBounds;
			KShelf*				fShelf;
			uint32				fEventMask;
...
inline void
KView::ScrollTo(float x, float y)
{
	ScrollTo(BPoint(x, y));
}
...
inline void
KView::SetViewColor(uchar red, uchar green, uchar blue, uchar alpha)
{
	rgb_color color;
	color.red = red;
	color.green = green;
...
inline void
KView::SetHighColor(uchar red, uchar green, uchar blue, uchar alpha)
{
	rgb_color color;
	color.red = red;
	color.green = green;
...
inline void
KView::SetLowColor(uchar red, uchar green, uchar blue, uchar alpha)
{
	rgb_color color;
	color.red = red;
	color.green = green;
