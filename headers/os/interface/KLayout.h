/*
 * Copyright 2006-2010, Haiku, Inc. All rights reserved.
 * Distributed under the terms of the MIT License.
 */
#ifndef	_K_LAYOUT_H
#define	_K_LAYOUT_H


#include <Alignment.h>
#include <Archivable.h>
#include <KLayoutItem.h>
#include <List.h>
#include <Size.h>


class BLayoutContext;
class KLayoutItem;
class KView;


class KLayout : public KLayoutItem {
public:
								KLayout();
								KLayout(BMessage* archive);
	virtual						~KLayout();

			KView*				Owner() const;
			KView*				TargetView() const;
	virtual	KView*				View(); // from KLayoutItem

	// methods dealing with items
	virtual	KLayoutItem*		AddView(KView* child);
	virtual	KLayoutItem*		AddView(int32 index, KView* child);

	virtual	bool				AddItem(KLayoutItem* item);
	virtual	bool				AddItem(int32 index, KLayoutItem* item);

	virtual	bool				RemoveView(KView* child);
	virtual	bool				RemoveItem(KLayoutItem* item);
	virtual	KLayoutItem*		RemoveItem(int32 index);

			KLayoutItem*		ItemAt(int32 index) const;
			int32				CountItems() const;
			int32				IndexOfItem(const KLayoutItem* item) const;
			int32				IndexOfView(KView* child) const;

			bool				AncestorsVisible() const;

	// Layouting related methods

	virtual	void				InvalidateLayout(bool children = false);
	virtual	void				Relayout(bool immediate = false);
									// from KLayoutItem
			void				RequireLayout();
			bool				IsValid();
			void				EnableLayoutInvalidation();
			void				DisableLayoutInvalidation();

			void				LayoutItems(bool force = false);
			BRect				LayoutArea();
			BLayoutContext*		LayoutContext() const;

	virtual status_t			Archive(BMessage* into, bool deep = true) const;

	virtual status_t			Perform(perform_code d, void* arg);

protected:
	// Archiving hook methods
	virtual	status_t			AllArchived(BMessage* archive) const;
	virtual	status_t			AllUnarchived(const BMessage* from);

	virtual status_t			ItemArchived(BMessage* into, KLayoutItem* item,
									int32 index) const;
	virtual	status_t			ItemUnarchived(const BMessage* from,
									KLayoutItem* item, int32 index);
	// KLayout hook methods
	virtual	bool				ItemAdded(KLayoutItem* item, int32 atIndex);
	virtual	void				ItemRemoved(KLayoutItem* item, int32 fromIndex);
	virtual	void				LayoutInvalidated(bool children);
	virtual	void				DoLayout() = 0;
	virtual	void				OwnerChanged(KView* was);

	// KLayoutItem hook methods
	virtual	void				AttachedToLayout();
	virtual void				DetachedFromLayout(KLayout* layout);
	virtual	void				AncestorVisibilityChanged(bool shown);

	// To be called by sub-classes in SetVisible().
			void				VisibilityChanged(bool show);
	// To be called when layout data is known to be good
			void				ResetLayoutInvalidation();

private:

	// FBC padding
	virtual	void				_ReservedLayout1();
	virtual	void				_ReservedLayout2();
	virtual	void				_ReservedLayout3();
	virtual	void				_ReservedLayout4();
	virtual	void				_ReservedLayout5();
	virtual	void				_ReservedLayout6();
	virtual	void				_ReservedLayout7();
	virtual	void				_ReservedLayout8();
	virtual	void				_ReservedLayout9();
	virtual	void				_ReservedLayout10();

	// forbidden methods
								KLayout(const KLayout&);
			void				operator =(const KLayout&);

			friend class KView;
			//friend class KView;

			void				SetOwner(KView* owner);
			void				SetTarget(KView* target);

			void				_LayoutWithinContext(bool force,
									BLayoutContext* context);

			uint32				fState;
			bool				fAncestorsVisible;
			int32				fInvalidationDisabled;
			BLayoutContext*		fContext;
			KView*				fOwner;
			KView*				fTarget;
			BList				fItems;
			BList				fNestedLayouts;

			uint32				_reserved[10];
};


#endif	//	_LAYOUT_H
