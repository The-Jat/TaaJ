 ...
 */
#ifndef _K_DECIMAL_SPINNER_H
#define _K_DECIMAL_SPINNER_H


#include <KSpinner.h>


class KDecimalSpinner : public KAbstractSpinner {
public:
								KDecimalSpinner(BRect frame, const char* name,
									const char* label, BMessage* message,
									uint32 resizingMode = B_FOLLOW_LEFT_TOP,
									uint32 flags = B_WILL_DRAW | B_NAVIGABLE);
								KDecimalSpinner(const char* name, const char* label,
									BMessage* message,
									uint32 flags = B_WILL_DRAW | B_NAVIGABLE);
								KDecimalSpinner(BMessage* data);
	virtual						~KDecimalSpinner();
