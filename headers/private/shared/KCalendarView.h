 ...
 */
#ifndef K__CALENDAR_VIEW_H_
#define K__CALENDAR_VIEW_H_
...
class KCalendarView : public KView, public BInvoker {
public:
								KCalendarView(BRect frame, const char* name,
									uint32 resizeMask = B_FOLLOW_LEFT_TOP,
									uint32 flags = B_WILL_DRAW | B_FRAME_EVENTS
										| B_NAVIGABLE | B_PULSE_NEEDED);

								KCalendarView(const char* name,
									uint32 flags = B_WILL_DRAW | B_FRAME_EVENTS
										| B_NAVIGABLE | B_PULSE_NEEDED);

	virtual						~KCalendarView();

								KCalendarView(BMessage* archive);
	...
	virtual void				DrawDay(KView* owner, BRect frame,
									const char* text, bool isSelected = false,
									bool isEnabled = true, bool focus = false,
									bool highlight = false);
	virtual void				DrawDayName(KView* owner, BRect frame,
									const char* text);
	virtual void				DrawWeekNumber(KView* owner, BRect frame,
									const char* text);
...
private:
			...
			void				_DrawItem(KView* owner, BRect frame,
									const char* text, bool isSelected = false,
									bool isEnabled = true, bool focus = false,
									bool highlight = false);
...
private:
			...
			// hide copy constructor & assignment
								KCalendarView(const KCalendarView& view);
			KCalendarView&		operator=(const KCalendarView& view);
};
