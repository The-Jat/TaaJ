...
class K_View: public Canvas {
public:
							K_View(IntRect frame, IntPoint scrollingOffset,
	...
	virtual void			DetachedFromWindow();
			::K_Window*		K_Window() const { return fWindow; }

			// Shorthands for opaque K_Window access
			...
			// tree stuff
			void			AddChild(K_View* view);
			bool			RemoveChild(K_View* view);

	inline	bool			HasParent(K_View* candidate) const
							{
	...
	inline	K_View*			Parent() const
								{ return fParent; }

	inline	K_View*			FirstChild() const
								{ return fFirstChild; }
	inline	K_View*			LastChild() const
								{ return fLastChild; }
	inline	K_View*			PreviousSibling() const
								{ return fPreviousSibling; }
	inline	K_View*			NextSibling() const
								{ return fNextSibling; }
			...
			void			FindViews(uint32 flags, BObjectList<K_View>& list,
								int32& left);

			bool			HasView(K_View* view);
