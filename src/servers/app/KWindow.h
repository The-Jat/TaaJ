 ...
 */
#ifndef ___K_WINDOW_H
#define ___K_WINDOW_H
...
typedef	BObjectList<K_Window>	K_StackWindows;
...
public:
	...
	const	K_StackWindows&		LayerOrder() const { return fWindowLayerOrder; }
			...
			K_Window*				TopLayerWindow() const;

			int32				CountWindows();
			K_Window*				WindowAt(int32 index);
			bool				AddWindow(K_Window* window,
									int32 position = -1);
			bool				RemoveWindow(K_Window* window);
...
private:
			...
			K_StackWindows		fWindowLayerOrder;
};
...
class K_WorkspacesView;

// TODO: move this into a proper place
#define AS_REDRAW 'rdrw'

enum {
	K_UPDATE_REQUEST		= 0x01,
	K_UPDATE_EXPOSE		= 0x02,
};


class K_Window {
public:
								K_Window(const BRect& frame, const char *name,
	...
	virtual						~K_Window();
			...
			k_window_anchor&		Anchor(int32 index);
	...
	inline	BRegion&			VisibleRegion() { return fVisibleRegion; }
			...
			// make private? when used inside K_Window, it has the ReadLock()
			...
			void				ScrollViewBy(K_View* view, int32 dx, int32 dy);

			void				SetTopView(K_View* topView);
			K_View*				TopView() const { return fTopView.Get(); }
			K_View*				ViewAt(const BPoint& where);

	virtual	bool				IsOffscreenWindow() const { return false; }

			void				GetEffectiveDrawingRegion(K_View* view,
									BRegion& region);
			bool				DrawingRegionChanged(K_View* view) const;
			...
			void				InvalidateView(K_View* view, BRegion& viewRegion);
	...
	inline	void				RecycleRegion(BRegion* region)
			...
			// TODO: move this to KServerWindow maybe?
	...
	inline	bool				IsMinimized() const { return fMinimized; }
			...
			K_Window*				Frontmost(K_Window* first = NULL,
									int32 workspace = -1);
			K_Window*				Backmost(K_Window* first = NULL,
									int32 workspace = -1);

			bool				AddToSubset(K_Window* window);
			void				RemoveFromSubset(K_Window* window);
			bool				HasInSubset(const K_Window* window) const;
			bool				SameSubset(K_Window* window);
			...
			void				FindWorkspacesViews(
									BObjectList<K_WorkspacesView>& list) const;
	...
	static	uint32				ValidWindowFlags(window_feel feel);
			...
			// K_Window stack methods.
			K_WindowStack*		GetWindowStack();

			bool				DetachFromWindowStack(
									bool ownStackNeeded = true);
			bool				AddWindowToStack(K_Window* window);
...
protected:
			...
			k_window_anchor		fAnchor[k_kListCount];
			...
			BObjectList<K_Window> fSubsets;

			ObjectDeleter<K_WindowBehaviour>
								fWindowBehaviour;
			ObjectDeleter<K_View>	fTopView;
			::KServerWindow*		fWindow;
	...
	public:
		...
		inline	bool				IsExpose() const
										{ return fCause & K_UPDATE_EXPOSE; }
		inline	bool				IsRequest() const
										{ return fCause & K_UPDATE_REQUEST; }
...
private:
			K_WindowStack*		_InitWindowStack();

			BReference<K_WindowStack>		fCurrentStack;
};
