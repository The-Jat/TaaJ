...
#include "KDefaultWindowBehaviour.h"
...
struct K_DefaultWindowBehaviour::State {
	State(K_DefaultWindowBehaviour& behavior)
		:
		fBehavior(behavior),
...
protected:
	K_DefaultWindowBehaviour&	fBehavior;
	K_Window*					fWindow;
	Desktop*				fDesktop;
};
...
struct K_DefaultWindowBehaviour::MouseTrackingState : State {
	MouseTrackingState(K_DefaultWindowBehaviour& behavior, BPoint where,
	...
	virtual void MouseUpWindowAction()
	{
		...
		fDesktop->K_ActivateWindow(fWindow);
	}
...
struct K_DefaultWindowBehaviour::DragState : MouseTrackingState {
	DragState(K_DefaultWindowBehaviour& behavior, BPoint where,
	...
	virtual bool MouseDown(BMessage* message, BPoint where, bool& _unhandled)
	{
	debug_printf("[K_DefaultWindowBehaviour]{MouseDown}\n");
		// right-click while dragging shall bring the window to front
		int32 buttons = message->FindInt32("buttons");
		if ((buttons & B_SECONDARY_MOUSE_BUTTON) != 0) {
			debug_printf("[K_DefaultWindowBehaviour]{MouseDown} this is right click\n");
			if (fWindow == fDesktop->K_BackWindow())
			{
			debug_printf("[K_DefaultWindowBehaviour]{MouseDowna] if fWindow is backwindow then send it to front\n");
				fDesktop->K_ActivateWindow(fWindow);
			}
			else{
				debug_printf("[K_DefaultWindowBehaviour]{MouseDown}if window is already front window\n");
				fDesktop->K_SendWindowBehind(fWindow);
			}
			return true;
		}
	debug_printf("[K_DefaultWindowBehaviour]{MouseDown} about to jump to MouseTrackingState::MouseDown\n");
	...
	virtual void MouseMovedAction(BPoint& delta, bigtime_t now)
	{
		if ((fWindow->Flags() & B_NOT_MOVABLE) == 0) {
			...
			fDesktop->K_MoveWindowBy(fWindow, delta.x, delta.y);
...
struct K_DefaultWindowBehaviour::ResizeState : MouseTrackingState {
	BPoint fDelta;

	ResizeState(K_DefaultWindowBehaviour& behavior, BPoint where,
	...
	virtual void ExitState(State* nextState)
	{
		if ((fWindow->Flags() & B_OUTLINE_RESIZE) != 0) {
			fDesktop->K_SetWindowOutlinesDelta(fWindow, BPoint(0, 0));
			fDesktop->K_ResizeWindowBy(fWindow, fDelta.x, fDelta.y);
		}
	}
	...
	virtual void MouseMovedAction(BPoint& delta, bigtime_t now)
	{
		if ((fWindow->Flags() & B_NOT_RESIZABLE) == 0) {
			...
			if ((fWindow->Flags() & B_OUTLINE_RESIZE) != 0) {
				fDelta = delta;
				fDesktop->K_SetWindowOutlinesDelta(fWindow, delta);
			} else
				fDesktop->K_ResizeWindowBy(fWindow, delta.x, delta.y);
...
struct K_DefaultWindowBehaviour::SlideTabState : MouseTrackingState {
	SlideTabState(K_DefaultWindowBehaviour& behavior, BPoint where)
		:
	...
	~SlideTabState()
	{
		fDesktop->K_SetWindowTabLocation(fWindow, fWindow->TabLocation(), false);
	}
	...
	virtual void MouseMovedAction(BPoint& delta, bigtime_t now)
	{
		...
		if (fDesktop->K_SetWindowTabLocation(fWindow, location, true))
			delta.y = 0;
		else
			delta = BPoint(0, 0);
	}
...
struct K_DefaultWindowBehaviour::ResizeBorderState : MouseTrackingState {
	BPoint fDelta;
	...
	virtual void ExitState(State* nextState)
	{
		...
		if (fWindow->Flags() & B_OUTLINE_RESIZE) {
			fDesktop->K_SetWindowOutlinesDelta(fWindow, B_ORIGIN);
			fDesktop->K_ResizeWindowBy(fWindow, fDelta.x, fDelta.y);
		}
	}
	...
	virtual void MouseMovedAction(BPoint& delta, bigtime_t now)
	{
		...
		if (fWindow->Flags() & B_OUTLINE_RESIZE) {
			fDelta = delta;
			fDesktop->K_SetWindowOutlinesDelta(fWindow, BPoint(
				delta.x * fHorizontal, delta.y * fVertical));
		} else {
			fDesktop->K_ResizeWindowBy(fWindow, delta.x * fHorizontal,
				delta.y * fVertical);
		}
		...
		if (moveX != 0 || moveY != 0)
			fDesktop->K_MoveWindowBy(fWindow, moveX, moveY);
	}
...
struct K_DefaultWindowBehaviour::ManageWindowState : State {
	ManageWindowState(K_DefaultWindowBehaviour& behavior, BPoint where)
		:
		State(behavior),
		fLastMousePosition(where),
		fHorizontal(NONE),
	...
	virtual void MouseMoved(BMessage* message, BPoint where, bool isFake)
	{
		...
		// leave the state.
		if (fDesktop->K_WindowAt(where) == fWindow) {
			fLastMousePosition = where;
			_UpdateBorders(fLastMousePosition);
		} else
			fBehavior._NextState(NULL);
...
// #pragma mark - K_DefaultWindowBehaviour


K_DefaultWindowBehaviour::K_DefaultWindowBehaviour(K_Window* window)
	:
	fWindow(window),
...
K_DefaultWindowBehaviour::~K_DefaultWindowBehaviour()
{
}


bool
K_DefaultWindowBehaviour::MouseDown(BMessage* message, BPoint where,
	int32 lastHitRegion, int32& clickCount, int32& _hitRegion)
{
	...
	if (action != ACTION_RESIZE_BORDER) {
		...
		if (desktopSettings.MouseMode() == B_NORMAL_MOUSE) {
			...
			fDesktop->K_SetFocusWindow(fWindow);
...
void
K_DefaultWindowBehaviour::MouseUp(BMessage* message, BPoint where)
{
	if (fState.IsSet())
...
void
K_DefaultWindowBehaviour::MouseMoved(BMessage* message, BPoint where, bool isFake)
{
	if (fState.IsSet()) {
	...
	if (desktopSettings.FocusFollowsMouse()
		...
		fDesktop->K_SetFocusWindow(isFake ? NULL : fWindow);
	}
}


void
K_DefaultWindowBehaviour::ModifiersChanged(int32 modifiers)
{
	BPoint where;
	int32 buttons;
...
bool
K_DefaultWindowBehaviour::_IsWindowModifier(int32 modifiers) const
{
...
ServerCursor*
K_DefaultWindowBehaviour::_ResizeCursorFor(int8 horizontal, int8 vertical)
{
...
void
K_DefaultWindowBehaviour::_SetResizeCursor(int8 horizontal, int8 vertical)
{
...
void
K_DefaultWindowBehaviour::_ResetResizeCursor()
{
...
/*static*/ void
K_DefaultWindowBehaviour::_ComputeResizeDirection(float x, float y,
...
void
K_DefaultWindowBehaviour::_NextState(State* state)
{
	// exit the old state
	if (fState.IsSet())
