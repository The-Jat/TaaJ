...
K_TabDecorator::~K_TabDecorator()
{
	STRACE(("K_TabDecorator: ~K_TabDecorator()\n"));
}
...
*/
void
K_TabDecorator::Draw(BRect updateRect)
{
	STRACE(("K_TabDecorator::Draw(BRect "
...
K_Decorator::Region
K_TabDecorator::RegionAt(BPoint where, int32& tab) const
{
	// Let the base class version identify hits of the buttons and the tab.
	Region region = K_Decorator::RegionAt(where, tab);
...
bool
K_TabDecorator::SetRegionHighlight(Region region, uint8 highlight,
	BRegion* dirty, int32 tabIndex)
{
	K_Decorator::Tab* tab
		= static_cast<K_Decorator::Tab*>(_TabAt(tabIndex));
	if (tab != NULL) {
	...
	return K_Decorator::SetRegionHighlight(region, highlight, dirty, tabIndex);
}
...
void
K_TabDecorator::_DoOutlineLayout()
{
...
void
K_TabDecorator::_DistributeTabSize(float delta)
{
	...
	K_Decorator::Tab* previousTab = NULL;
	for (int32 i = 0; i < tabCount; i++) {
		K_Decorator::Tab* tab = fTabList.ItemAt(i);
		if (tab == NULL)
			continue;
...
void
K_TabDecorator::_DrawOutlineFrame(BRect rect)
{
...
void
K_TabDecorator::_SetTitle(K_Decorator::Tab* tab, const char* string,
...
void
K_TabDecorator::_MoveBy(BPoint offset)
{
	STRACE(("K_TabDecorator: Move By (%.1f, %.1f)\n", offset.x, offset.y));
...
void
K_TabDecorator::_ResizeBy(BPoint offset, BRegion* dirty)
{
	STRACE(("K_TabDecorator: Resize By (%.1f, %.1f)\n", offset.x, offset.y));
...
bool
K_TabDecorator::_SetTabLocation(K_Decorator::Tab* _tab, float location,
	bool isShifting, BRegion* updateRegion)
{
	STRACE(("K_TabDecorator: Set Tab Location(%.1f)\n", location));
...
bool
K_TabDecorator::_SetSettings(const BMessage& settings, BRegion* updateRegion)
{
	float tabLocation;
	bool modified = false;
	for (int32 i = 0; i < fTabList.CountItems(); i++) {
...
bool
K_TabDecorator::_RemoveTab(int32 index, BRegion* updateRegion)
{
...
bool
K_TabDecorator::_MoveTab(int32 from, int32 to, bool isMoving,
	BRegion* updateRegion)
{
	K_Decorator::Tab* toTab = _TabAt(to);
	if (toTab == NULL)
		return false;
...
void
K_TabDecorator::_GetFootprint(BRegion *region)
{
	STRACE(("K_TabDecorator: GetFootprint\n"));
...
void
K_TabDecorator::_DrawButtons(K_Decorator::Tab* tab, const BRect& invalid)
{
	STRACE(("K_TabDecorator: _DrawButtons\n"));
...
void
K_TabDecorator::_GetButtonSizeAndOffset(const BRect& tabRect, float* _offset,
...
float
K_TabDecorator::_DefaultTextOffset() const
{
	if (fTopTab->look == B_FLOATING_WINDOW_LOOK
			|| fTopTab->look == kLeftTitledWindowLook)
...
float
K_TabDecorator::_SingleTabOffsetAndSize(float& tabSize)
{
	float maxLocation;
	if (fTopTab->look != kLeftTitledWindowLook) {
...
void
K_TabDecorator::_CalculateTabsRegion()
{
	fTabsRegion.MakeEmpty();
	for (int32 i = 0; i < fTabList.CountItems(); i++)
