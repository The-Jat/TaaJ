	...
	window_look look, uint32 flags, int32 index, BRegion* updateRegion)
{
	...
	K_Decorator::Tab* oldTop = fTopTab;
	fTopTab = tab;
...
bool
K_Decorator::RemoveTab(int32 index, BRegion* updateRegion)
{
	AutoWriteLocker _(fLocker);

	K_Decorator::Tab* tab = fTabList.RemoveItemAt(index);
	if (tab == NULL)
		return false;
...
bool
K_Decorator::MoveTab(int32 from, int32 to, bool isMoving, BRegion* updateRegion)
{
	AutoWriteLocker _(fLocker);
...
int32
K_Decorator::TabAt(const BPoint& where) const
{
...
void
K_Decorator::SetTopTab(int32 tab)
{
	AutoWriteLocker _(fLocker);
...
*/
void
K_Decorator::SetDrawingEngine(DrawingEngine* engine)
{
	AutoWriteLocker _(fLocker);
...
*/
void
K_Decorator::SetFlags(int32 tab, uint32 flags, BRegion* updateRegion)
{
debug_printf("[K_Decorator]{ SetFlags}...\n");
debug_printf("[K_Decorator]{ SetFlags} tab = %d\n", tab);
	AutoWriteLocker _(fLocker);
	...
	if ((flags & (B_NOT_H_RESIZABLE | B_NOT_V_RESIZABLE))
		...
		debug_printf("[K_Decorator] {SetFlags} B_NOT_H_RESIZABLE | B_NOT_V_RESIZABLE.\n");
		flags |= B_NOT_RESIZABLE;
	}
	if (flags & B_NOT_RESIZABLE){
		debug_printf("[K_Decorator]{ SetFlags} B_NOT_RESIZABLE\n");
		flags |= B_NOT_H_RESIZABLE | B_NOT_V_RESIZABLE;
	}

	K_Decorator::Tab* decoratorTab = fTabList.ItemAt(tab);
...
*/
window_look
K_Decorator::Look(int32 tab) const
{
	AutoReadLocker _(fLocker);
...
*/
uint32
K_Decorator::Flags(int32 tab) const
{
	AutoReadLocker _(fLocker);
...
BRect
K_Decorator::TitleBarRect() const
{
	AutoReadLocker _(fLocker);
...
*/
BRect
K_Decorator::TabRect(int32 tab) const
{
	AutoReadLocker _(fLocker);
...
BRect
K_Decorator::TabRect(K_Decorator::Tab* tab) const
{
	return tab->tabRect;
}
...
*/
void
K_Decorator::SetClose(int32 tab, bool pressed)
{
	AutoWriteLocker _(fLocker);
...
*/
void
K_Decorator::SetMinimize(int32 tab, bool pressed)
{
	AutoWriteLocker _(fLocker);
...
*/
void
K_Decorator::SetZoom(int32 tab, bool pressed)
{
	AutoWriteLocker _(fLocker);
...
*/
void
K_Decorator::SetTitle(int32 tab, const char* string, BRegion* updateRegion)
{
	AutoWriteLocker _(fLocker);
...
*/
const char*
K_Decorator::Title(int32 tab) const
{
	AutoReadLocker _(fLocker);
...
const char*
K_Decorator::Title(K_Decorator::Tab* tab) const
{
	AutoReadLocker _(fLocker);
	return tab->title;
}
...
float
K_Decorator::TabLocation(int32 tab) const
{
	AutoReadLocker _(fLocker);

	K_Decorator::Tab* decoratorTab = _TabAt(tab);
	if (decoratorTab == NULL)
		return 0.0f;
...
bool
K_Decorator::SetTabLocation(int32 tab, float location, bool isShifting,
...
*/
void
K_Decorator::SetFocus(int32 tab, bool active)
{
	AutoWriteLocker _(fLocker);
...
bool
K_Decorator::IsFocus(int32 tab) const
{
	AutoReadLocker _(fLocker);
...
bool
K_Decorator::IsFocus(K_Decorator::Tab* tab) const
{
	AutoReadLocker _(fLocker);
...
*/
const BRegion&
K_Decorator::GetFootprint()
{
debug_printf("[K_Decorator] {GetFootprint}...\n");
	AutoReadLocker _(fLocker);

	if (!fFootprintValid) {
	debug_printf("[K_Decorator]{GetFootprint} fFootprintValid not set.\n");
	...
	debug_printf("[K_Decorator]{GetFootprint} return fFootprint\n");
...
*/
K_Decorator::Region
K_Decorator::RegionAt(BPoint where, int32& tabIndex) const
{
	AutoReadLocker _(fLocker);
...
/*!	\brief Moves the decorator frame and all default rectangles
	...
	If a subclass implements this method, be sure to call K_Decorator::MoveBy
	to ensure that internal members are also updated. All members of the
	K_Decorator class are automatically moved in this method

	\param x X Offset
	\param y y Offset
*/
void
K_Decorator::MoveBy(float x, float y)
{
	MoveBy(BPoint(x, y));
}
...
*/
void
K_Decorator::MoveBy(BPoint offset)
{
	AutoWriteLocker _(fLocker);
...
*/
void
K_Decorator::ResizeBy(float x, float y, BRegion* dirty)
{
...
void
K_Decorator::ResizeBy(BPoint offset, BRegion* dirty)
{
	AutoWriteLocker _(fLocker);
...
void
K_Decorator::SetOutlinesDelta(BPoint delta, BRegion* dirty)
{
...
void
K_Decorator::ExtendDirtyRegion(Region region, BRegion& dirty)
{
	AutoReadLocker _(fLocker);
...
*/
bool
K_Decorator::SetRegionHighlight(Region region, uint8 highlight, BRegion* dirty,
	int32 tab)
{
	AutoWriteLocker _(fLocker);
...
bool
K_Decorator::SetSettings(const BMessage& settings, BRegion* updateRegion)
{
	AutoWriteLocker _(fLocker);
...
bool
K_Decorator::GetSettings(BMessage* settings) const
{
	AutoReadLocker _(fLocker);
...
void
K_Decorator::GetSizeLimits(int32* minWidth, int32* minHeight,
...
//! draws the title
void
K_Decorator::DrawTitle(int32 tab)
{
	AutoReadLocker _(fLocker);
...
void
K_Decorator::DrawClose(int32 tab)
{
	AutoReadLocker _(fLocker);
...
void
K_Decorator::DrawMinimize(int32 tab)
{
	AutoReadLocker _(fLocker);
...
void
K_Decorator::DrawZoom(int32 tab)
{
	AutoReadLocker _(fLocker);
...
void
K_Decorator::_DrawTabs(BRect rect)
{
	K_Decorator::Tab* focusTab = NULL;
	for (int32 i = 0; i < fTabList.CountItems(); i++) {
		K_Decorator::Tab* tab = fTabList.ItemAt(i);
		if (tab->isFocused) {
			focusTab = tab;
...
bool
K_Decorator::_SetTabLocation(K_Decorator::Tab* tab, float location, bool isShifting,
	BRegion* /*updateRegion*/)
{
	return false;
}
...
K_Decorator::Tab*
K_Decorator::_TabAt(int32 index) const
{
	return static_cast<K_Decorator::Tab*>(fTabList.ItemAt(index));
}
...
void
K_Decorator::_SetFlags(K_Decorator::Tab* tab, uint32 flags, BRegion* updateRegion)
{
debug_printf("[K_Decorator]{ _SetFlags}...\n");
	// TODO: we could be much smarter about the update region
...
void
K_Decorator::_MoveBy(BPoint offset)
{
	for (int32 i = 0; i < fTabList.CountItems(); i++) {
		K_Decorator::Tab* tab = fTabList.ItemAt(i);
...
void
K_Decorator::_MoveOutlineBy(BPoint offset)
{
...
void
K_Decorator::_ResizeOutlineBy(BPoint offset, BRegion* dirty)
{
...
void
K_Decorator::_SetOutlinesDelta(BPoint delta, BRegion* dirty)
{
...
bool
K_Decorator::_SetSettings(const BMessage& settings, BRegion* updateRegion)
{
	return false;
}
...
*/
void
K_Decorator::_GetFootprint(BRegion *region)
{
}


void
K_Decorator::_GetOutlineFootprint(BRegion* region)
{
	if (region == NULL)
		return;
...
void
K_Decorator::_InvalidateFootprint()
{
	fFootprintValid = false;
}
...
void
K_Decorator::_InvalidateBitmaps()
{
	for (int32 i = 0; i < fTabList.CountItems(); i++) {
		K_Decorator::Tab* tab = static_cast<K_Decorator::Tab*>(_TabAt(i));
