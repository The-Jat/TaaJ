...
/*!	\class KServerWindow
	...
	The KServerWindow class handles all BWindow messaging; it forwards all
	...
	is one KServerWindow per BWindow.
*/
...
*/
KServerWindow::KServerWindow(const char* title, ServerApp* app,
		port_id clientPort, port_id looperPort, int32 clientToken)
	:
	MessageLooper(title && *title ? title : "Unnamed K_Window"),
	fTitle(NULL),
	fDesktop(app->GetDesktop()),
	fServerApp(app),
	fWindowAddedToDesktop(false),//window is not added to desktop yet
	...
	IsKWindow(true)// called from KWindow
{
debug_printf("[KServerWindow]{KServerWindow Constructor}...\n");
	STRACE(("KServerWindow(%s)::KServerWindow()\n", title));

	SetTitle(title);
	fServerToken = BPrivate::gDefaultTokens.NewToken(B_SERVER_TOKEN, this);

debug_printf("[KServerWindow]{KServerWindow Constructor} fServerToken=%d\n",fServerToken);
...
/*! Tears down all connections the main app_server objects, and deletes some
	internals.
*/
KServerWindow::~KServerWindow()
{
	STRACE(("KServerWindow(%s@%p):~KServerWindow()\n", fTitle, this));

	if (!fWindow->IsOffscreenWindow()) {
		fWindowAddedToDesktop = false;
		fDesktop->K_RemoveWindow(fWindow.Get());
	}

	if (App() != NULL) {
		App()->K_RemoveWindow(this);
		fServerApp = NULL;
	}
	...
	STRACE(("KServerWindow(%p) will exit NOW\n", this));
...
status_t
KServerWindow::Init(BRect frame, window_look look, window_feel feel,
	uint32 flags, uint32 workspace)
{
debug_printf("[KServerWindow]{Init} ...\n");
	if (!App()->K_AddWindow(this)) {
		debug_printf("[KServerWindow]{} ...\n");
		fServerApp = NULL;
		return B_NO_MEMORY;
	}

	if (fTitle == NULL){
	debug_printf("[KServerWindow]{Init} fTitle == NULL...\n");
		return B_NO_MEMORY;
	}
	...
	debug_printf("[KServerWindow]{Init} !fWindow->IsOffScreenWindow...\n");

		fDesktop->K_AddWindow(fWindow.Get());
...
void
KServerWindow::_GetLooperName(char* name, size_t length)
{
	const char *title = Title();
	if (title == NULL || !title[0])
		title = "Unnamed K_Window";

	snprintf(name, length, "w:%" B_PRId32 ":%s", ClientTeam(), title);
}


/*! Shows the window's K_Window.
*/
void
KServerWindow::_Show()
{
debug_printf("[KServerWindow] {_Show}\n");
	// NOTE: if you do something else, other than sending a port message, PLEASE lock
	STRACE(("KServerWindow %s: _Show\n", Title()));
	...
	fDesktop->UnlockSingleWindow();
debug_printf("[KServerWindow] {_Show} before K_ShowWindow\n");
	fDesktop->K_ShowWindow(fWindow.Get());
debug_printf("[KServerWindow] {_Show} after K_ShowWindow\n");
	if (fDirectWindowInfo.IsSet() && fDirectWindowInfo->IsFullScreen())
	{debug_printf("[KServerWindow] {_Show} IsFullScreen()\n");
		_ResizeToFullScreen();
	}
...
/*! Hides the window's K_Window. You need to have all windows locked when
	calling this function.
*/
void
KServerWindow::_Hide()
{
debug_printf("[KServerWindow]{_Hide}\n");

	STRACE(("KServerWindow %s: _Hide\n", Title()));

	if (fWindow->IsHidden() || fWindow->IsOffscreenWindow())
		return;

	fDesktop->UnlockSingleWindow();
	fDesktop->K_HideWindow(fWindow.Get());
	fDesktop->LockSingleWindow();
}
...
void
KServerWindow::SetTitle(const char* newTitle)
{
	...
	if (fWindow.IsSet())
		fDesktop->K_SetWindowTitle(fWindow.Get(), newTitle);
}


//! Requests that the KServerWindow's BWindow quit
void
KServerWindow::NotifyQuitRequested()
{
	// NOTE: if you do something else, other than sending a port message,
	// PLEASE lock
	STRACE(("KServerWindow %s: Quit\n", fTitle));
...
void
KServerWindow::NotifyMinimize(bool minimize)
{
...
void
KServerWindow::GetInfo(window_info& info)
{
...
K_View*
KServerWindow::_CreateView(BPrivate::LinkReceiver& link, K_View** _parent)
{
debug_printf("[KServerWindow]{_CreateView} Into _CreateView...\n");
	...
	STRACE(("KServerWindow(%s)::_CreateView()-> view %s, token %" B_PRId32 "\n",
		fTitle, name, token));
debug_printf("[KServerWindow]{_CreateView} (%s) _CreateView()-> view %s...\n",fTitle, name);

	K_View* newView;

	if ((flags & kWorkspacesViewFlag) != 0) {
	debug_printf("[KServerWindow]{_CreateView} (flags & kWorkspacesViewFlag) != 0...\n");
		newView = new (nothrow) K_WorkspacesView(frame, scrollingOffset, name,
			token, resizeMask, flags);
	} else {
	debug_printf("[KServerWindow]{_CreateView} Creating simple view from else statement...\n");
		newView = new (nothrow) K_View(frame, scrollingOffset, name, token,
			resizeMask, flags);
	}
...
//		fDesktop->LockSingleWindow();
	...
	if (_parent) {
		...
		if (App()->ViewTokens().GetToken(parentToken, B_HANDLER_TOKEN,
				(void**)&parent) != B_OK
			|| parent->K_Window()->KServerWindow() != this) {
			debug_printf("K_View token not found!\n");
			parent = NULL;
		}
...
debug_printf("[KServerWindow]{_CreateView} _CreateView ends good\n");
	return newView;
}
...
*/
void
KServerWindow::_DispatchMessage(int32 code, BPrivate::LinkReceiver& link)
{
debug_printf("[KServerWindow] {_DispatchMessage}code =%d\n",code);


	switch (code) {
		case AS_SHOW_OR_HIDE_WINDOW:
		{
		debug_printf("[KServerWindow] {_DispatchMessage} Message AS_SHOW_OR_HIDE_WINDOW\n");
			int32 showLevel;
			if (link.Read<int32>(&showLevel) == B_OK) {
				debug_printf("KServerWindow %s: Message AS_SHOW_OR_HIDE_WINDOW, "
					"show level: %d \n", Title(), showLevel);
		...
		{
		debug_printf("[KServerWindow] {_DispatchMessage} Message AS_INTERNAL_HIDE_WINDOW\n");
			_Hide();
			break;
		}
		case AS_MINIMIZE_WINDOW:
		{
		debug_printf("[KServerWindow] {_DispatchMessage} AS_MINIMIZE_WINDOW\n");
			bool minimize;
			if (link.Read<bool>(&minimize) == B_OK) {
				DTRACE(("KServerWindow %s: Message AS_MINIMIZE_WINDOW, "
					"minimize: %d\n", Title(), minimize));

				fDesktop->UnlockSingleWindow();
				fDesktop->K_MinimizeWindow(fWindow.Get(), minimize);
				fDesktop->LockSingleWindow();
			}
		// tododo
			break;
		...
		{
		debug_printf("[KServerWindow] {_DispatchMessage} : Message AS_ACTIVATE_WINDOW\n");
			bool activate = true;
			if (link.Read<bool>(&activate) != B_OK)
				break;

			DTRACE(("KServerWindow %s: Message AS_ACTIVATE_WINDOW: activate: "
				"%d\n", Title(), activate));

			fDesktop->UnlockSingleWindow();

			if (activate)
				fDesktop->K_SelectWindow(fWindow.Get());
			else
				fDesktop->K_SendWindowBehind(fWindow.Get(), NULL);
		...
		case AS_SEND_BEHIND:
		{
		debug_printf("[KServerWindow] {_DispatchMessage} AS_SEND_BEHIND\n");
			...
			if (link.Read<team_id>(&teamID) == B_OK) {
				::K_Window* behindOf = fDesktop->K_FindWindowByClientToken(token,
					teamID);

				DTRACE(("KServerWindow %s: Message AS_SEND_BEHIND %s\n",
					Title(), behindOf != NULL ? behindOf->Title() : "NULL"));

				if (behindOf != NULL || token == -1) {
					fDesktop->K_SendWindowBehind(fWindow.Get(), behindOf);
					status = B_OK;
				} else
		...
		case B_QUIT_REQUESTED:
			DTRACE(("KServerWindow %s received quit request\n", Title()));
			NotifyQuitRequested();
			break;

		case AS_ENABLE_UPDATES:
		debug_printf("[KServerWindow]{_DispatchMessage} AS_ENABLE_UPDATES\n");
			DTRACE(("KServerWindow %s: Message AS_ENABLE_UPDATES\n", Title()));
			fWindow->EnableUpdateRequests();
			break;

		case AS_DISABLE_UPDATES:
		debug_printf("[KServerWindow]{_DispatchMessage} AS_DISBLE_UPDATES\n");
			DTRACE(("KServerWindow %s: Message AS_DISABLE_UPDATES\n", Title()));
			fWindow->DisableUpdateRequests();
			break;

		case AS_NEEDS_UPDATE:
		debug_printf("[KServerWindow] {_DispatchMessage} : Message AS_NEEDS_UPDATE\n");
			DTRACE(("KServerWindow %s: Message AS_NEEDS_UPDATE: %d\n",
		...
		case AS_SET_WINDOW_TITLE:
		{
			...
			if (link.ReadString(&newTitle) == B_OK) {
				DTRACE(("KServerWindow %s: Message AS_SET_WINDOW_TITLE: %s\n",
					Title(), newTitle));
		...
		{
		debug_printf("[KServerWindow] {_DispatchMessage} : Message AS_ADD_TO_SUBSET....\n");
			
			// Has the all-window lock
			DTRACE(("KServerWindow %s: Message AS_ADD_TO_SUBSET\n", Title()));
			status_t status = B_ERROR;

			int32 token;
			if (link.Read<int32>(&token) == B_OK) {
				::K_Window* window = fDesktop->K_FindWindowByClientToken(token,
					App()->ClientTeam());
				if (window == NULL || window->Feel() != B_NORMAL_WINDOW_FEEL) {
					status = B_BAD_VALUE;
				} else {
					status = fDesktop->K_AddWindowToSubset(fWindow.Get(), window)
						? B_OK : B_NO_MEMORY;
				}
			}
		...
		{
		debug_printf("[KServerWindow]{_DispatchMessage} AS_REMOVE_FROM_SUBSET\n");
			// Has the all-window lock
			DTRACE(("KServerWindow %s: Message AS_REM_FROM_SUBSET\n", Title()));
			status_t status = B_ERROR;
			...
			if (link.Read<int32>(&token) == B_OK) {
				::K_Window* window = fDesktop->K_FindWindowByClientToken(token,
					App()->ClientTeam());
				if (window != NULL) {
					fDesktop->K_RemoveWindowFromSubset(fWindow.Get(), window);
					status = B_OK;
				} else
					status = B_BAD_VALUE;
			}
		...
		case AS_SET_LOOK:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_SET_LOOK\n", Title()));

			status_t status = B_ERROR;
			int32 look;
			if (link.Read<int32>(&look) == B_OK) {
				// test if look is valid
				status = K_Window::IsValidLook((window_look)look)
					? B_OK : B_BAD_VALUE;
			}

			if (status == B_OK && !fWindow->IsOffscreenWindow())
				fDesktop->K_SetWindowLook(fWindow.Get(), (window_look)look);
		...
		{
			// Has the all-window look
			DTRACE(("KServerWindow %s: Message AS_SET_FEEL\n", Title()));

			status_t status = B_ERROR;
			int32 feel;
			if (link.Read<int32>(&feel) == B_OK) {
				// test if feel is valid
				status = K_Window::IsValidFeel((window_feel)feel)
					? B_OK : B_BAD_VALUE;
			}

			if (status == B_OK && !fWindow->IsOffscreenWindow())
				fDesktop->K_SetWindowFeel(fWindow.Get(), (window_feel)feel);
		...
		case AS_SET_FLAGS:
		{
			...
			// Has the all-window look
			DTRACE(("KServerWindow %s: Message AS_SET_FLAGS\n", Title()));

			status_t status = B_ERROR;
			uint32 flags;
			if (link.Read<uint32>(&flags) == B_OK) {
				// test if flags are valid
				status = (flags & ~K_Window::ValidWindowFlags()) == 0
					? B_OK : B_BAD_VALUE;
			}

			if (status == B_OK && !fWindow->IsOffscreenWindow())
				fDesktop->K_SetWindowFlags(fWindow.Get(), flags);
		...
		case AS_SET_ALIGNMENT:
		{
			...
			DTRACE(("KServerWindow %s: Message Set_Alignment unimplemented\n",
				Title()));
			break;
		}
		case AS_GET_ALIGNMENT:
		{
			...
			DTRACE(("KServerWindow %s: Message Get_Alignment unimplemented\n",
				Title()));
			break;
		}
#endif
		...
		{
		debug_printf("[KServerWindow]{_DispatchMessage} AS_IS_FRONT_WINDOW\n");
			bool isFront = fDesktop->K_FrontWindow() == fWindow.Get();
			DTRACE(("KServerWindow %s: Message AS_IS_FRONT_WINDOW: %d\n",
				Title(), isFront));
		...
		{
		debug_printf("[KServerWindow]{_DispatchMessage} AS_GET_WORKSPACES\n");
			DTRACE(("KServerWindow %s: Message AS_GET_WORKSPACES\n", Title()));
			fLink.StartMessage(B_OK);
		...
		{
		debug_printf("[KServerWindow]{_DispatchMessage} AS_SET_WORKSPACES\n");
			...
			DTRACE(("KServerWindow %s: Message AS_SET_WORKSPACES %" B_PRIx32 "\n",
				Title(), newWorkspaces));

			fDesktop->K_SetWindowWorkspaces(fWindow.Get(), newWorkspaces);
			break;
		}
		case AS_WINDOW_RESIZE:
		{
		debug_printf("[KServerWindow]{_DispatchMessage} AS_WINDOW_RESIZE\n");
			// Has the all-window look
			float xResizeTo;
			float yResizeTo;
			...
			DTRACE(("KServerWindow %s: Message AS_WINDOW_RESIZE %.1f, %.1f\n",
...
//			} else {
				fDesktop->K_ResizeWindowBy(fWindow.Get(),
...
//			}
		...
		case AS_WINDOW_MOVE:
		{
		debug_printf("[KServerWindow]{_DispatchMessage} AS_WINDOW_MOVE\n");
			// Has the all-window look
			float xMoveTo;
			float yMoveTo;
			...
			DTRACE(("KServerWindow %s: Message AS_WINDOW_MOVE: %.1f, %.1f\n",
			...
			if (fWindow->IsDragging()) {
				...
				fDesktop->K_MoveWindowBy(fWindow.Get(),
		...
		{
		debug_printf("[KServerWindow]{_DispatchMessage} AS_SET_SIZE_LIMITS\n");
			// Has the all-window look
...
*/
			DTRACE(("KServerWindow %s: Message AS_SET_SIZE_LIMITS: "
			...
			fDesktop->K_NotifySizeLimitsChanged(fWindow.Get(), minWidth, maxWidth,
				minHeight, maxHeight);
			break;
		}
		...
		{
			// Has the all-window look
			DTRACE(("KServerWindow %s: Message AS_SET_DECORATOR_SETTINGS\n",
				Title()));
			...
			if (fWindow.IsSet() && link.Read<int32>(&size) == B_OK) {
				...
				if (link.Read(buffer, size) == B_OK) {
					...
					if (settings.Unflatten(buffer) == B_OK)
						fDesktop->K_SetWindowDecoratorSettings(
		...
		{
			DTRACE(("KServerWindow %s: Message AS_GET_DECORATOR_SETTINGS\n",
				Title()));
		...
		case AS_SYSTEM_FONT_CHANGED:
		{
			...
			// Has the all-window look
			fDesktop->K_FontsChanged(fWindow.Get());
			//tododo
			break;
		}
		...
		case AS_SYNC:
			DTRACE(("KServerWindow %s: Message AS_SYNC\n", Title()));
		...
		case AS_BEGIN_UPDATE:
			debug_printf("[KServerWindow] {_DispatchMessage} %s: Message AS_BEGIN_UPDATE\n", Title());
			DTRACE(("KServerWindow %s: Message AS_BEGIN_UPDATE\n", Title()));
			fWindow->BeginUpdate(fLink);
			break;

		case AS_END_UPDATE:
		debug_printf("[KServerWindow] {_DispatchMessage} %s: Message AS_END_UPDATE\n", Title());
			DTRACE(("KServerWindow %s: Message AS_END_UPDATE\n", Title()));
			fWindow->EndUpdate();
			break;

		case AS_GET_MOUSE:
		{
		debug_printf("[KServerWindow] {_DispatchMessage}AS_GET_MOUSE\n");
			// Has the all-window look
			DTRACE(("KServerWindow %s: Message AS_GET_MOUSE\n", fTitle));
		...
		case AS_SET_CURRENT_VIEW_2:
		{
		debug_printf("[KServerWindow] {_DispatchMessage} %s: Message AS_SET_CURRENT_VIEW_2\n", Title());
			int32 token;
			if (link.Read<int32>(&token) != B_OK)
				break;
			...
			if (App()->ViewTokens().GetToken(token, B_HANDLER_TOKEN,
					(void**)&current) != B_OK
				|| current->K_Window()->KServerWindow() != this) {
				// TODO: if this happens, we probably want to kill the app and
				// clean up
				debug_printf("KServerWindow %s: Message "
					"\n\n\nAS_SET_CURRENT_VIEW_2: view not found, token %"
					B_PRId32 "\n", fTitle, token);
				current = NULL;
			} else {
			debug_printf("[KServerWindow]{_DispatchMessage} AS_SET_CURRENT_VIEW_2 else part\n");
				DTRACE(("\n\n\nServerWindow %s: Message AS_SET_CURRENT_VIEW_2: %s, "
		...
		case AS_CREATE_LAMINATE_JADD:
		{debug_printf("[KServerWindow] {_DispatchMessage} the code that received to server is AS_CREATE_LAMINATE_JADD...\n");
			DTRACE(("KServerWindow %s: Message AS_VIEW_CREATE_ROOT\n", fTitle));
			...
			if (fCurrentView != NULL) {
				debug_printf("KServerWindow %s: Message "
					"AS_CREATE_LAMINATE_JADD: fCurrentView already set!!\n",
					fTitle);
				break;
			}
		...
		case AS_VIEW_CREATE_2:
		{
		debug_printf("[KServerWindow]{_DispatchMessage} AS_VIEW_CREATE_2\n");
			DTRACE(("KServerWindow %s: Message AS_VIEW_CREATE: K_View name: "
				"%s\n", fTitle, fCurrentView->Name()));

			K_View* parent = NULL;
			K_View* newView = _CreateView(link, &parent);
			if (parent != NULL && newView != NULL)
				parent->AddChild(newView);
			else {
				delete newView;
				debug_printf("KServerWindow %s: Message AS_VIEW_CREATE: "
		...
		{
		debug_printf("[KServerWindow]{_DispatchMessage} AS_TALK_TO_DESKTOP_LISTENER\n");
			if (fDesktop->K_MessageForListener(fWindow.Get(), fLink.Receiver(),
				fLink.Sender()))
				break;
			// unhandled message at least send an error if needed
			if (link.NeedsReply()) {
				debug_printf("[KServerWindow]{_DispatchMessage} AS_TALK_TO_DESKTOP_LISTENER link.NeedsReply()\n");
				fLink.StartMessage(B_ERROR);
				fLink.Flush();
			}
			break;
		...
		default:
		debug_printf("[KServerWindow]{_DispatchMessage} default\n");
			if (fCurrentView == NULL) {
				debug_printf("KServerWindow %s received unexpected code - "
...
*/
void
KServerWindow::_DispatchViewMessage(int32 code,
	BPrivate::LinkReceiver &link)
{
debug_printf("[KServerWindow]{_DispatchViewMessage}code=%d\n",code);
	...
	switch (code) {
		case AS_VIEW_SCROLL:
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_SCROLL and K_View name=%s\n",fCurrentView->Name());
			float dh;
			float dv;
			link.Read<float>(&dh);
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_SCROLL: K_View name: "
		...
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_COPY_BITS and K_View name=%s\n",fCurrentView->Name());
			BRect src;
			BRect dst;
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_COPY_BITS: K_View name: "
				"%s, BRect(%.1f, %.1f, %.1f, %.1f) -> "
				"BRect(%.1f, %.1f, %.1f, %.1f)\n", fTitle,
		...
		case AS_VIEW_DELETE:
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_DELETE and K_View name=%s\n",fCurrentView->Name());
			...
			if (App()->ViewTokens().GetToken(token, B_HANDLER_TOKEN,
					(void**)&view) == B_OK
				&& view->K_Window()->KServerWindow() == this) {
				K_View* parent = view->Parent();

				DTRACE(("KServerWindow %s: AS_VIEW_DELETE view: %p, "
					"parent: %p\n", fTitle, view, parent));
...
fDesktop->LockSingleWindow();
		...
		case AS_VIEW_SET_STATE_2:
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_SET_STATE_2 and K_View name=%s\n",fCurrentView->Name());
		
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_STATE_2: "
				"K_View name: %s\n", fTitle, fCurrentView->Name()));
		...
		{
		debug_printf("[KServerWindow]{_DispatchPictureMessage} AS_VIEW_SET_FONT_STATE\n");
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_FONT_STATE: "
				"K_View name: %s\n", fTitle, fCurrentView->Name()));
		...
		case AS_VIEW_GET_STATE_2:
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_GET_STATE_2 and K_View name=%s\n",fCurrentView->Name());
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_STATE_2: "
				"K_View name: %s\n", fTitle, fCurrentView->Name()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_EVENT_MASK: "
				"K_View name: %s\n", fTitle, fCurrentView->Name()));
...
fDesktop->LockSingleWindow();
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_MOUSE_EVENT_MASK: "
				"K_View name: %s\n", fTitle, fCurrentView->Name()));
			uint32 eventMask, options;
...
fDesktop->UnlockSingleWindow();
				...
				if (eventMask != 0 || options != 0) {
					if (options & B_LOCK_WINDOW_FOCUS)
						fDesktop->K_SetFocusLocked(fWindow.Get());
...
fDesktop->LockSingleWindow();
		...
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_MOVE_TO and K_View name=%s\n",fCurrentView->Name());
			float x, y;
			link.Read<float>(&x);
			if (link.Read<float>(&y) != B_OK)
				break;

			DTRACE(("KServerWindow %s: Message AS_VIEW_MOVE_TO: K_View name: "
			...
			// follow modes
			if (K_View* parent = fCurrentView->Parent())
		...
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_RESIZE_TO and K_View name=%s\n",fCurrentView->Name());
			float newWidth, newHeight;
			link.Read<float>(&newWidth);
			if (link.Read<float>(&newHeight) != B_OK)
				break;

			DTRACE(("KServerWindow %s: Message AS_VIEW_RESIZE_TO: K_View name: "
		...
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_GET_COORD and K_View name=%s\n",fCurrentView->Name());
			// our offset in the parent -> will be originX and originY
			// in BView
			BPoint parentOffset = fCurrentView->Frame().LeftTop();

			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_COORD: "
				"K_View: %s -> x: %.1f, y: %.1f\n", Title(),
		...
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_SET_ORIGIN and K_View name=%s\n",fCurrentView->Name());
			float x, y;
			link.Read<float>(&x);
			if (link.Read<float>(&y) != B_OK)
				break;

			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_ORIGIN: "
				"K_View: %s -> x: %.1f, y: %.1f\n", Title(),
		...
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_GET_ORIGIN and K_View name=%s\n",fCurrentView->Name());
			BPoint drawingOrigin = fCurrentView->DrawingOrigin();

			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_ORIGIN: "
				"K_View: %s -> x: %.1f, y: %.1f\n", Title(),
		...
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_RESIZE_MODE and K_View name=%s\n",fCurrentView->Name());
			uint32 resizeMode;
			if (link.Read<uint32>(&resizeMode) != B_OK)
				break;

			DTRACE(("KServerWindow %s: Message AS_VIEW_RESIZE_MODE: "
		...
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_SET_FLAGS and K_View name=%s\n",fCurrentView->Name());
			uint32 flags;
			link.Read<uint32>(&flags);
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_FLAGS: "
				"K_View: %s -> flags: %" B_PRIu32 "\n", Title(),
				fCurrentView->Name(), flags));
			break;
		}
		case AS_VIEW_HIDE_2:
			debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_HIDE_2 and K_View name=%s\n",fCurrentView->Name());
			DTRACE(("KServerWindow %s: Message AS_VIEW_HIDE: K_View: %s\n",
				Title(), fCurrentView->Name()));
			fCurrentView->SetHidden(true);
			break;

		case AS_VIEW_SHOW_2:
			debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_SHOW_2 and K_View name=%s\n",fCurrentView->Name());
			DTRACE(("KServerWindow %s: Message AS_VIEW_SHOW: K_View: %s\n",
				Title(), fCurrentView->Name()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_LINE_MODE: "
				"K_View: %s\n", Title(), fCurrentView->Name()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_LINE_MODE: "
				"K_View: %s\n", Title(), fCurrentView->Name()));
			ViewSetLineModeInfo info;
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_FILL_RULE: "
				"K_View: %s\n", Title(), fCurrentView->Name()));
			int32 fillRule;
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_FILL_RULE: "
				"K_View: %s\n", Title(), fCurrentView->Name()));
		...
		{
		debug_printf("[KServerWindow]{_DispatchViewMessage}AS_VIEW_PUSH_STATE and K_View name=%s\n",fCurrentView->Name());
			DTRACE(("KServerWindow %s: Message AS_VIEW_PUSH_STATE: K_View: "
				"%s\n", Title(), fCurrentView->Name()));
...
//			_UpdateDrawState(fCurrentView);
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_POP_STATE: K_View: %s\n",
				Title(), fCurrentView->Name()));
		...
		case AS_VIEW_SET_SCALE:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_SCALE: "
				"K_View: %s -> scale: %.2f\n", Title(), fCurrentView->Name(),
				scale));
		...
		case AS_VIEW_GET_SCALE:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_SCALE: "
				"K_View: %s -> scale: %.2f\n",
		...
		case AS_VIEW_SET_TRANSFORM:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_TRANSFORM: "
				"K_View: %s -> transform: %.2f, %.2f, %.2f, %.2f, %.2f, %.2f\n",
		...
		case AS_VIEW_GET_TRANSFORM:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_TRANSFORM: "
				"K_View: %s -> transform: %.2f, %.2f, %.2f, %.2f, %.2f, %.2f\n",
				Title(), fCurrentView->Name(), transform.sx, transform.shy,
				transform.shx, transform.sy, transform.tx, transform.ty));
		...
		case AS_VIEW_SET_PEN_LOC:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_PEN_LOC: "
				"K_View: %s -> BPoint(%.1f, %.1f)\n", Title(),
		...
		case AS_VIEW_GET_PEN_LOC:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_PEN_LOC: "
				"K_View: %s -> BPoint(%.1f, %.1f)\n", Title(),
				fCurrentView->Name(), location.x, location.y));
		...
		case AS_VIEW_SET_PEN_SIZE:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_PEN_SIZE: "
		...
		case AS_VIEW_GET_PEN_SIZE:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_PEN_SIZE: "
				"K_View: %s -> %.1f\n", Title(), fCurrentView->Name(), penSize));
		...
		case AS_VIEW_SET_VIEW_COLOR:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_VIEW_COLOR: "
				"K_View: %s -> rgb_color(%d, %d, %d, %d)\n", Title(),
		...
		case AS_VIEW_GET_VIEW_COLOR:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_VIEW_COLOR: "
				"K_View: %s -> rgb_color(%d, %d, %d, %d)\n",
		...
		case AS_VIEW_SET_HIGH_COLOR:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_HIGH_COLOR: "
				"K_View: %s -> rgb_color(%d, %d, %d, %d)\n",
				Title(), fCurrentView->Name(), color.red, color.green,
				color.blue, color.alpha));
		...
		case AS_VIEW_GET_HIGH_UI_COLOR:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_HIGH_UI_COLOR: "
				"K_View: %s -> color_which(%i) tint(%.3f) - rgb_color(%i, %i,"
		...
		case AS_VIEW_GET_LOW_UI_COLOR:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_LOW_UI_COLOR: "
				"K_View: %s -> color_which(%i) tint(%.3f) - rgb_color(%i, %i,"
				" %i, %i)\n", Title(), fCurrentView->Name(), which, tint,
				color.red, color.green, color.blue, color.alpha));
		...
		case AS_VIEW_GET_VIEW_UI_COLOR:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_VIEW_UI_COLOR: "
				"K_View: %s -> color_which(%i) tint(%.3f) - rgb_color(%i, %i,"
				" %i, %i)\n", Title(), fCurrentView->Name(), which, tint,
				color.red, color.green, color.blue, color.alpha));
		...
		case AS_VIEW_GET_HIGH_COLOR:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_HIGH_COLOR: "
				"K_View: %s -> rgb_color(%d, %d, %d, %d)\n",
				Title(), fCurrentView->Name(), color.red, color.green,
				color.blue, color.alpha));
		...
		case AS_VIEW_SET_LOW_COLOR:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_LOW_COLOR: "
				"K_View: %s -> rgb_color(%d, %d, %d, %d)\n",
				Title(), fCurrentView->Name(), color.red, color.green,
				color.blue, color.alpha));
		...
		case AS_VIEW_GET_LOW_COLOR:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_LOW_COLOR: "
				"K_View: %s -> rgb_color(%d, %d, %d, %d)\n",
				Title(), fCurrentView->Name(), color.red, color.green,
				color.blue, color.alpha));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_PATTERN: "
				"K_View: %s\n", fTitle, fCurrentView->Name()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_BLEND_MODE: "
				"K_View: %s\n", Title(), fCurrentView->Name()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_BLEND_MODE: "
				"K_View: %s\n", Title(), fCurrentView->Name()));
		...
		case AS_VIEW_SET_DRAWING_MODE:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_DRAW_MODE: "
				"K_View: %s -> %s\n", Title(), fCurrentView->Name(),
		...
		case AS_VIEW_GET_DRAWING_MODE:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_DRAW_MODE: "
				"K_View: %s -> %s\n", Title(), fCurrentView->Name(),
				kDrawingModeMap[drawingMode]));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_SET_VIEW_BITMAP: "
				"K_View: %s\n", Title(), fCurrentView->Name()));
			...
			if (status == B_OK) {
				...
				if (bitmapToken == -1 || bitmap != NULL) {
					...
					//	in K_View::Draw() to the BeOS version, we never
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_PRINT_ALIASING: "
				"K_View: %s\n", Title(), fCurrentView->Name()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_CLIP_TO_PICTURE: "
				"K_View: %s\n", Title(), fCurrentView->Name()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_GET_CLIP_REGION: "
				"K_View: %s\n", Title(), fCurrentView->Name()));
		...
		case AS_VIEW_SET_CLIP_REGION:
		{
			...
			if (rectCount >= 0) {
				...
				DTRACE(("KServerWindow %s: Message AS_VIEW_SET_CLIP_REGION: "
					"K_View: %s -> rect count: %" B_PRId32 ", frame = "
					"BRect(%.1f, %.1f, %.1f, %.1f)\n",
		...
		case AS_VIEW_INVALIDATE_RECT:
		{
			...
			if (link.Read<BRect>(&invalidRect) == B_OK) {
				DTRACE(("KServerWindow %s: Message AS_VIEW_INVALIDATE_RECT: "
					"K_View: %s -> BRect(%.1f, %.1f, %.1f, %.1f)\n", Title(),
					fCurrentView->Name(), invalidRect.left, invalidRect.top,
					invalidRect.right, invalidRect.bottom));

				K_View* view = NULL;
				if (link.Read<K_View*>(&view) != B_OK)
					view = fCurrentView;
		...
		case AS_VIEW_DELAYED_INVALIDATE_RECT:
		{
			...
			if (link.Read<bigtime_t>(&time) == B_OK
				...
				DTRACE(("KServerWindow %s: Message "
					"AS_VIEW_DELAYED_INVALIDATE_RECT: "
					"K_View: %s -> BRect(%.1f, %.1f, %.1f, %.1f) at time %llu\n",
				...
				if (delayed.Attach<BRect>(invalidRect) == B_OK
						&& delayed.Attach<K_View*>(fCurrentView) == B_OK)
					delayed.Flush();
			}
			break;
		}
		...
		{
		debug_printf("[KServerWindow]{_DispatchMessage} AS_VIEW_INVALIDATE_REGION\n");
			// NOTE: looks like this call is NOT affected by origin and scale
			// on R5 so this implementation is "correct"
			BRegion region;
			...
			DTRACE(("KServerWindow %s: Message AS_VIEW_INVALIDATE_REGION: "
					"K_View: %s -> rect count: %" B_PRId32 ", frame: BRect(%.1f, "
					"%.1f, %.1f, %.1f)\n", Title(),
		...
		case AS_VIEW_DRAG_IMAGE:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_DRAG_IMAGE\n", Title()));
...
fDesktop->LockSingleWindow();
		...
		case AS_VIEW_DRAG_RECT:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_DRAG_RECT\n", Title()));
...
fDesktop->LockSingleWindow();
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_BEGIN_RECT_TRACK\n",
				Title()));
			BRect dragRect;
			uint32 style;
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_END_RECT_TRACK\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_BEGIN_PICTURE\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_APPEND_TO_PICTURE\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_END_PICTURE\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_BEGIN_LAYER\n",
				Title()));
...
*/
void
KServerWindow::_DispatchViewDrawingMessage(int32 code,
	BPrivate::LinkReceiver &link)
{
debug_printf("[KServerWindow]{_DispatchViewDrawingMessage} \n");

	if (!fCurrentView->IsVisible() || !fWindow->IsVisible()) {
		if (link.NeedsReply()) {
			debug_printf("KServerWindow::DispatchViewDrawingMessage() got "
	...
	if (!drawingEngine) {
		// ?!?
		debug_printf("KServerWindow %s: no drawing engine!!\n", Title());
		if (link.NeedsReply()) {
			// the client is now blocking and waiting for a reply!
			fLink.StartMessage(B_ERROR);
			fLink.Flush();
	...
	if (fCurrentDrawingRegion.CountRects() <= 0 && code != AS_VIEW_END_LAYER) {
		...
		DTRACE(("KServerWindow %s: _DispatchViewDrawingMessage(): K_View: %s, "
			"INVALID CLIPPING!\n", Title(), fCurrentView->Name()));
		if (link.NeedsReply()) {
			// the client is now blocking and waiting for a reply!
			fLink.StartMessage(B_ERROR);
	...
	switch (code) {
		case AS_STROKE_LINE:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_STROKE_LINE: K_View: %s -> "
			...
			// location is in K_View coordinates (I think it should be) or in
			// screen coordinates.
		...
		case AS_VIEW_INVERT_RECT:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_INVERT_RECT: K_View: %s -> "
				"BRect(%.1f, %.1f, %.1f, %.1f)\n", Title(),
		...
		case AS_STROKE_RECT:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_STROKE_RECT: K_View: %s -> "
				"BRect(%.1f, %.1f, %.1f, %.1f)\n", Title(),
				fCurrentView->Name(), rect.left, rect.top, rect.right,
				rect.bottom));
		...
		case AS_FILL_RECT:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_FILL_RECT: K_View: %s -> "
				"BRect(%.1f, %.1f, %.1f, %.1f)\n", Title(),
				fCurrentView->Name(), rect.left, rect.top, rect.right,
				rect.bottom));
		...
		case AS_FILL_RECT_GRADIENT:
		{
			...
			GTRACE(("KServerWindow %s: Message AS_FILL_RECT_GRADIENT: K_View: %s "
				"-> BRect(%.1f, %.1f, %.1f, %.1f)\n", Title(),
				fCurrentView->Name(), rect.left, rect.top, rect.right,
				rect.bottom));
		...
		case AS_VIEW_DRAW_BITMAP:
		{
			...
			if (bitmap != NULL) {
				DTRACE(("KServerWindow %s: Message AS_VIEW_DRAW_BITMAP: "
					"K_View: %s, bitmap: %" B_PRId32 " (size %" B_PRId32 " x "
...
//					fDesktop->HWInterface()->WaitForRetrace(20000);
		...
		case AS_FILL_ARC:
		{
			DTRACE(("KServerWindow %s: Message AS_STROKE/FILL_ARC\n", Title()));
		...
		{
			GTRACE(("KServerWindow %s: Message AS_FILL_ARC_GRADIENT\n",
				Title()));
		...
		case AS_FILL_BEZIER:
		{
			DTRACE(("KServerWindow %s: Message AS_STROKE/FILL_BEZIER\n",
				Title()));
		...
		{
			GTRACE(("KServerWindow %s: Message AS_FILL_BEZIER_GRADIENT\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_STROKE/FILL_ELLIPSE\n",
				Title()));
		...
		{
			GTRACE(("KServerWindow %s: Message AS_FILL_ELLIPSE_GRADIENT\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_STROKE/FILL_ROUNDRECT\n",
				Title()));
		...
		{
			GTRACE(("KServerWindow %s: Message AS_FILL_ROUNDRECT_GRADIENT\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_STROKE/FILL_TRIANGLE\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_FILL_TRIANGLE_GRADIENT\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_STROKE/FILL_POLYGON\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_FILL_POLYGON_GRADIENT\n",
				Title()));
		...
		case AS_FILL_SHAPE:
		{
			DTRACE(("KServerWindow %s: Message AS_STROKE/FILL_SHAPE\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_FILL_SHAPE_GRADIENT\n",
				Title()));
		...
		case AS_FILL_REGION:
		{
			DTRACE(("KServerWindow %s: Message AS_FILL_REGION\n", Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_FILL_REGION_GRADIENT\n",
				Title()));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_STROKE_LINEARRAY\n",
				Title()));
		...
		case AS_DRAW_STRING_WITH_DELTA:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_DRAW_STRING, K_View: %s "
		...
		case AS_DRAW_STRING_WITH_OFFSETS:
		{
			...
			DTRACE(("KServerWindow %s: Message AS_DRAW_STRING_WITH_OFFSETS, K_View: %s "
				"-> %s\n", Title(), fCurrentView->Name(), string));
		...
		{
			DTRACE(("KServerWindow %s: Message AS_VIEW_END_LAYER\n",
				Title()));
		...
		default:
			debug_printf("KServerWindow %s received unexpected code: %s\n",
				Title(), string_for_message_code(code));
...
bool
KServerWindow::_DispatchPictureMessage(int32 code, BPrivate::LinkReceiver& link)
{
debug_printf("[KServerWindow]{_DispatchPictureMessage} \n");
	...
	switch (code) {
		...
		case AS_VIEW_PUSH_STATE:
		{
		debug_printf("[KServerWindow]{_DispatchPictureMessage} AS_VIEW_PUSH_STATE\n");
			fCurrentView->PushState();
		...
		case AS_VIEW_POP_STATE:
		{
		debug_printf("[KServerWindow]{_DispatchPictureMessage} AS_VIEW_POP_STATE\n");
			fCurrentView->PopState();
...
/*!	\brief Message-dispatching loop for the KServerWindow

	Watches the KServerWindow's message port and dispatches as necessary
*/
void
KServerWindow::_MessageLooper()
{
debug_printf("[KServerWindow]{_MessageLooper}\n");
	...
	while (!quitLoop) {
		//STRACE(("info: KServerWindow::MonitorWin listening on port %ld.\n",
		//	fMessagePort));
		...
		if (diff > 10000) {
			printf("KServerWindow %s: lock acquisition took %" B_PRId64 " usecs\n",
				Title(), diff);
		}
#endif
		...
		while (true) {
			if (code == AS_DELETE_WINDOW || code == kMsgQuitLooper) {
				...
				DTRACE(("KServerWindow %s received 'AS_DELETE_WINDOW' message "
					"code\n", Title()));
				...
				// KServerWindow's destructor takes care of pulling this object
				// off the desktop.
...
#endif
				if (diff > 10000) {
					printf("KServerWindow %s: message %" B_PRId32 " took %"
						B_PRId64 " usecs\n", Title(), code, diff);
				}
			}
#endif
...
void
KServerWindow::ScreenChanged(const BMessage* message)
{
	SendMessageToClient(message);
...
status_t
KServerWindow::SendMessageToClient(const BMessage* msg, int32 target) const
{
...
K_Window*
KServerWindow::MakeWindow(BRect frame, const char* name,
	window_look look, window_feel feel, uint32 flags, uint32 workspace)
{
	debug_printf("[KServerWindow]{MakeWindow} MakeWindow func entered...\n");

	// The non-offscreen KServerWindow uses the DrawingEngine instance from
	// the desktop.
	return new(std::nothrow) ::K_Window(frame, name, look, feel, flags,
		workspace, this, fDesktop->HWInterface()->CreateDrawingEngine());
}


void
KServerWindow::HandleDirectConnection(int32 bufferState, int32 driverState)
{
...
void
KServerWindow::_SetCurrentView(K_View* view)
{
debug_printf("[KServerWindow] {_SetCurrentView} ...\n");
...
debug_printf("[KServerWindow] {_SetCurrentView} ends\n");
}


void
KServerWindow::_UpdateDrawState(K_View* view)
{
debug_printf("[KServerWindow] {_UpdateDrawState} ...\n");
...
debug_printf("[KServerWindow] {_UpdateDrawState} ends...\n");
}


void
KServerWindow::_UpdateCurrentDrawingRegion()
{
...
bool
KServerWindow::_MessageNeedsAllWindowsLocked(uint32 code) const
{
	switch (code) {
		case AS_SET_WINDOW_TITLE:
		case AS_ADD_TO_SUBSET:
...
void
KServerWindow::_ResizeToFullScreen()
{
	debug_printf("[KServerWindow] {_ResizeToFullScreen}\n");
/*
	BRect screenFrame;
...
status_t
KServerWindow::_EnableDirectWindowMode()
{
	if (fDirectWindowInfo.IsSet()) {
...
void
KServerWindow::_DirectWindowSetFullScreen(bool enable)
{
