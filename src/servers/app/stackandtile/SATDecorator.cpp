...
{
debug_printf("[SATDecorator]{ UpdateColors}...\n");
...
//khidki start
K_SATWindowBehaviour::K_SATWindowBehaviour(K_Window* window, K_StackAndTile* sat)
	:
	K_DefaultWindowBehaviour(window),
...
bool
K_SATWindowBehaviour::AlterDeltaForSnap(K_Window* window, BPoint& delta,
	bigtime_t now)
{
	if (K_DefaultWindowBehaviour::AlterDeltaForSnap(window, delta, now) == true)
		return true;
