...
void
k_resize_frame(IntRect& frame, uint32 resizingMode, int32 x, int32 y)
{
...
K_View::K_View(IntRect frame, IntPoint scrollingOffset, const char* name,
...
{
debug_printf("[K_View]{K_View Constructor} Into the K_View constructor...\n");
	if (fDrawState.IsSet())
	debug_printf("[K_View]{K_View Constructor} fDrawState.IsSet()...\n");
...
void
K_View::ConvertToVisibleInTopView(IntRect* bounds) const
{
...
void
K_View::AttachedToWindow(::K_Window* window)
{
debug_printf("[K_View] {AttachedToWindow}...\n");
	fWindow = window;
	...
	if (fWindow != NULL) {
		fWindow->KServerWindow()->App()->ViewTokens().SetToken(fToken,
			B_HANDLER_TOKEN, this);
	}

	// attach child views as well
	for (K_View* child = FirstChild(); child; child = child->NextSibling())
		child->AttachedToWindow(window);
}


void
K_View::DetachedFromWindow()
{
	// remove view from local token space
	if (fWindow != NULL && fWindow->KServerWindow()->App() != NULL)
		fWindow->KServerWindow()->App()->ViewTokens().RemoveToken(fToken);
...
DrawingEngine*
K_View::GetDrawingEngine() const
{
	return K_Window()->GetDrawingEngine();
}


ServerPicture*
K_View::GetPicture(int32 token) const
{
	return K_Window()->KServerWindow()->App()->GetPicture(token);
}


void
K_View::ResyncDrawState()
{
	return K_Window()->KServerWindow()->ResyncDrawState();
}


void
K_View::UpdateCurrentDrawingRegion()
{
	return K_Window()->KServerWindow()->UpdateCurrentDrawingRegion();
}


void
K_View::AddChild(K_View* view)
{
	if (view->fParent) {
		printf("K_View::AddChild() - K_View already has a parent\n");
		return;
	}
...
bool
K_View::RemoveChild(K_View* view)
{
	if (view == NULL || view->fParent != this) {
		printf("K_View::RemoveChild(%p - %s) - K_View is not child of "
...
uint32
K_View::CountChildren(bool deep) const
{
	uint32 count = 0;
	for (K_View* child = FirstChild(); child; child = child->NextSibling()) {
		count++;
...
void
K_View::CollectTokensForChildren(BList* tokenMap) const
{
	for (K_View* child = FirstChild(); child; child = child->NextSibling()) {
...
#if 0
bool
K_View::MarkAt(DrawingEngine* engine, const BPoint& where, int32 level)
{
...
void
K_View::FindViews(uint32 flags, BObjectList<K_View>& list, int32& left)
{
...
K_View*
K_View::ViewAt(const BPoint& where)
{
	if (!fVisible)
		return NULL;
	...
	for (K_View* child = FirstChild(); child; child = child->NextSibling()) {
		K_View* view = child->ViewAt(where);
		if (view != NULL)
			return view;
	}
...
void
K_View::SetName(const char* string)
{
	fName.SetTo(string);
}
...
void
K_View::SetViewBitmap(ServerBitmap* bitmap, IntRect sourceRect,
...
void
K_View::_UpdateOverlayView() const
{
	Overlay* overlay = _Overlay();
	if (overlay == NULL)
		return;
...
void
K_View::_LocalToScreenTransform(SimpleTransform& transform) const
{
	const K_View* view = this;
	int32 offsetX = 0;
	int32 offsetY = 0;
	do {
...
void
K_View::_ScreenToLocalTransform(SimpleTransform& transform) const
{
	const K_View* view = this;
	int32 offsetX = 0;
	int32 offsetY = 0;
...
void
K_View::MoveBy(int32 x, int32 y, BRegion* dirtyRegion)
{
	if (x == 0 && y == 0)
		return;
...
void
K_View::ResizeBy(int32 x, int32 y, BRegion* dirtyRegion)
{
	if (x == 0 && y == 0)
		return;
	...
	if (fVisible && dirtyRegion) {
		...
		if (dirty->CountRects() > 0) {
			if ((fFlags & B_DRAW_ON_CHILDREN) == 0) {
				...
				for (K_View* child = FirstChild(); child;
	...
	// view bitmap
	if (fViewBitmap != NULL)
		k_resize_frame(fBitmapDestination, fBitmapResizingMode, x, y);
...
void
K_View::ParentResized(int32 x, int32 y, BRegion* dirtyRegion)
{
	IntRect newFrame = fFrame;
	k_resize_frame(newFrame, fResizeMode & 0x0000ffff, x, y);
...
void
K_View::ScrollBy(int32 x, int32 y, BRegion* dirtyRegion)
{
	if (!fVisible || !fWindow) {
...
void
K_View::CopyBits(IntRect src, IntRect dst, BRegion& windowContentClipping)
{
	if (!fVisible || !fWindow)
		return;
...
void
K_View::ColorUpdated(color_which which, rgb_color color)
{
	...
	for (K_View* child = FirstChild(); child != NULL;
			child = child->NextSibling()) {
...
void
K_View::SetViewUIColor(color_which which, float tint)
{
	if (which != B_NO_COLOR) {
...
void
K_View::SetEventMask(uint32 eventMask, uint32 options)
{
...
void
K_View::SetCursor(ServerCursor* cursor)
{
	if (cursor == fCursor)
		return;
...
void
K_View::SetPicture(ServerPicture* picture)
{
	if (picture == fPicture)
		return;
...
void
K_View::Draw(DrawingEngine* drawingEngine, BRegion* effectiveClipping,
...
// XXX: locking removed because the K_Window keeps the engine locked
...
void
K_View::MouseDown(BMessage* message, BPoint where)
{
	// empty hook method
}
...
void
K_View::MouseUp(BMessage* message, BPoint where)
{
	// empty hook method
}
...
void
K_View::MouseMoved(BMessage* message, BPoint where)
{
	// empty hook method
}
...
void
K_View::UpdateVisibleDeep(bool parentVisible)
{
...
void
K_View::AddTokensForViewsInRegion(BPrivate::PortLink& link, BRegion& region,
...
void
K_View::RebuildClipping(bool deep)
{
debug_printf("[K_View]{RebuildClipping}\n");

	// the clipping spans over the bounds area
	fLocalClipping.Set((clipping_rect)Bounds());

	if (K_View* child = FirstChild()) {
...
debug_printf("[K_View]{RebuildClipping}ends\n");
}


BRegion&
K_View::ScreenAndUserClipping(BRegion* windowContentClipping, bool force) const
{
...
void
K_View::InvalidateScreenClipping()
{
...
BRegion&
K_View::_ScreenClipping(BRegion* windowContentClipping, bool force) const
{
...
void
K_View::_MoveScreenClipping(int32 x, int32 y, bool deep)
{
	if (fScreenClippingValid) {
