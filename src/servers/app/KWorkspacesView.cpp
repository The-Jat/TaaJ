...
K_WorkspacesView::K_WorkspacesView(BRect frame, BPoint scrollingOffset,
		const char* name, int32 token, uint32 resizeMode, uint32 flags)
	:
	K_View(frame, scrollingOffset, name, token, resizeMode, flags),
	fSelectedWindow(NULL),
	fSelectedWorkspace(-1),
	fHasMoved(false)
{
debug_printf("[K_WorkspacesView] {K_WorkspacesView constructor} \n");
...
K_WorkspacesView::~K_WorkspacesView()
{
}


void
K_WorkspacesView::AttachedToWindow(::K_Window* window)
{
	K_View::AttachedToWindow(window);

	window->AddWorkspacesView();
	window->Desktop()->K_AddWorkspacesView(this);
}


void
K_WorkspacesView::DetachedFromWindow()
{
	fWindow->Desktop()->K_RemoveWorkspacesView(this);
...
*/
BRect
K_WorkspacesView::_ScreenFrame(int32 i)
{
	return K_Window()->Desktop()->WorkspaceFrame(i);
}
...
*/
BRect
K_WorkspacesView::_WorkspaceAt(int32 i)
{
	int32 columns, rows;
...
*/
BRect
K_WorkspacesView::_WorkspaceAt(BPoint where, int32& index)
{
	int32 columns, rows;
	_GetGrid(columns, rows);
...
BRect
K_WorkspacesView::_WindowFrame(const BRect& workspaceFrame,
...
void
K_WorkspacesView::_DrawWindow(DrawingEngine* drawingEngine,
	const BRect& workspaceFrame, const BRect& screenFrame, ::K_Window* window,
	BPoint windowPosition, BRegion& backgroundRegion, bool workspaceActive)
{
	...
	// K_Window class, so that it has only to be recalculated on demand. With
...
void
K_WorkspacesView::_DrawWorkspace(DrawingEngine* drawingEngine,
	BRegion& redraw, int32 index)
{
	BRect rect = _WorkspaceAt(index);

	Workspace workspace(*K_Window()->Desktop(), index, true);
	...
	font.SetSize(fWindow->KServerWindow()->App()->PlainFont().Size());
	...
	::K_Window* window;
	BPoint leftTop;
	while (workspace.K_GetPreviousWindow(window, leftTop) == B_OK) {
...
void
K_WorkspacesView::_DarkenColor(rgb_color& color) const
{
...
void
K_WorkspacesView::_Invalidate() const
{
debug_printf("[K_WorkspacesView] {_Invalidate} \n");
	BRect frame = Bounds();
debug_printf(" BRect(l:%f , t:%f, r:%f , b:%f \n",
			 frame.left, frame.top, frame.right - 1, frame.bottom - 1);
	LocalToScreenTransform().Apply(&frame);
debug_printf(" BRect(l:%f , t:%f, r:%f , b:%f \n",
			 frame.left, frame.top, frame.right - 1, frame.bottom - 1);
...
//khidki start
	debug_printf("[K_Window]{_Invalidate}debug before K_Window()->MarkContentDirty\n");
	...
	//end
	K_Window()->MarkContentDirty(region);


//khidki start
	debug_printf("[K_Window]{_Invalidate}debug after K_Window()->MarkContentDirty\n");
	debug_printf("region.fCount=%d\n",region.FCount());
	debug_printf("region.fDataSize=%d\n",region.FDataSize());
	for (int32 i = 0; i < region.FCount(); i++) {
	debug_printf("[K_Window]{GetFullRegion} inside loop i=%d\n",i);
...
void
K_WorkspacesView::Draw(DrawingEngine* drawingEngine, BRegion* effectiveClipping,
	BRegion* windowContentClipping, bool deep)
{
	...
	// to reduce flicker
	BRect activeRect = _WorkspaceAt(K_Window()->Desktop()->CurrentWorkspace());
	...
	fWindow->KServerWindow()->ResyncDrawState();
}


void
K_WorkspacesView::MouseDown(BMessage* message, BPoint where)
{
	...
	Workspace workspace(*K_Window()->Desktop(), index);
	...
	if (fSelectedWindow != NULL
		...
		if ((modifiers & B_CONTROL_KEY) != 0) {
			...
			if ((modifiers & B_SHIFT_KEY) != 0)
				fSelectedWindow->KServerWindow()->NotifyMinimize(true);
			else
				K_Window()->Desktop()->K_ActivateWindow(fSelectedWindow);
			fSelectedWindow = NULL;
		} else if ((modifiers & B_OPTION_KEY) != 0) {
			// Also, send window to back if clicked with the option
			// key pressed.
			K_Window()->Desktop()->K_SendWindowBehind(fSelectedWindow);
			fSelectedWindow = NULL;
		}
	}
...
void
K_WorkspacesView::MouseUp(BMessage* message, BPoint where)
{
	if (!fHasMoved && fSelectedWorkspace >= 0) {
		int32 index;
		_WorkspaceAt(where, index);
		if (index >= 0)
			K_Window()->Desktop()->SetWorkspaceAsync(index);
	}
...
void
K_WorkspacesView::MouseMoved(BMessage* message, BPoint where)
{
debug_printf("[K_WorkspacesView]{MouseMoved}\n");
	...
	if (!fHasMoved) {
		K_Window()->Desktop()->K_SetMouseEventWindow(K_Window());
	...
	if (index != fSelectedWorkspace) {
		if (fSelectedWindow->IsNormal() && !fSelectedWindow->InWorkspace(index)) {
			...
			K_Window()->Desktop()->K_SetWindowWorkspaces(fSelectedWindow,
				newWorkspaces);
		}
		fSelectedWorkspace = index;
	}
	...
	BPoint leftTop;
	if (fSelectedWorkspace == K_Window()->Desktop()->CurrentWorkspace())
	...
	if (fHasMoved) {
		K_Window()->Desktop()->K_MoveWindowBy(fSelectedWindow, left - leftTop.x,
			top - leftTop.y, fSelectedWorkspace);
	}

debug_printf("[K_WorkspacesView]{MouseMoved}end\n");
 // tododo
}


void
K_WorkspacesView::WindowChanged(::K_Window* window)
{
debug_printf("[K_WorkspacesView] {WindowChanged} \n");
...
void
K_WorkspacesView::WindowRemoved(::K_Window* window)
{
