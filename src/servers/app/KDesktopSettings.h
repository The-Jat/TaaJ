 ...
 */
#ifndef K_DESKTOP_SETTINGS_H
#define K_DESKTOP_SETTINGS_H
...
class Desktop;
class K_DesktopSettingsPrivate;
class ServerFont;


static const int32 k_kMaxWorkspaces = 32;

enum {
	k_kAllSettings		= 0xff,
	k_kWorkspacesSettings	= 0x01,
	k_kFontSettings		= 0x02,
	k_kAppearanceSettings	= 0x04,
	k_kMouseSettings		= 0x08,
	k_kDraggerSettings	= 0x10,
};


class K_DesktopSettings {
public:
								K_DesktopSettings(Desktop* desktop);

			status_t			Save(uint32 mask = k_kAllSettings);
			...
			rgb_color			UIColor(light_color_scheme_which which) const;
...
protected:
			K_DesktopSettingsPrivate*	fSettings;
};


class K_LockedDesktopSettings : public K_DesktopSettings {
public:
								K_LockedDesktopSettings(Desktop* desktop);
								~K_LockedDesktopSettings();
