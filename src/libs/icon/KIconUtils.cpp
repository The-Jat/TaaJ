	...
	int32 srcBPR, int32 dstBPR)
{
	...
	KBitmap* tmp = new KBitmap(BRect(0, 0, srcWidth * 2 - 1,
...
status_t
KIconUtils::GetIcon(BNode* node, const char* vectorIconAttrName,
	const char* smallIconAttrName, const char* largeIconAttrName,
	icon_size which, KBitmap* icon)
{
	...
	switch (icon->ColorSpace()) {
		...
		case B_CMAP8:
			...
			if (result != B_OK) {
				...
				KBitmap temp(icon->Bounds(), B_BITMAP_NO_SERVER_LINK,
					B_RGBA32);
#else
				KBitmap temp(icon->Bounds(), B_RGBA32);
#endif
		...
		default:
			printf("KIconUtils::GetIcon() - unsupported colorspace\n");
			result = B_ERROR;
			break;
	}
...
status_t
KIconUtils::GetVectorIcon(BNode* node, const char* attrName, KBitmap* icon)
{
...
status_t
KIconUtils::GetVectorIcon(const uint8* buffer, size_t size, KBitmap* icon)
{
//debug_printf("[KIconUtils]{GetVectorIcon}\n");
	...
	KBitmap* temp = icon;
	ObjectDeleter<KBitmap> deleter;

	if (icon->ColorSpace() != B_RGBA32 && icon->ColorSpace() != B_RGB32) {
		temp = new (nothrow) KBitmap(icon->Bounds(),
...
//debug_printf("[KIconUtils]{GetVectorIcon}ends\n");
	return result;
}
...
status_t
KIconUtils::GetCMAP8Icon(BNode* node, const char* smallIconAttrName,
	const char* largeIconAttrName, icon_size which, KBitmap* icon)
{
...
status_t
KIconUtils::ConvertFromCMAP8(KBitmap* source, KBitmap* destination)
{
...
status_t
KIconUtils::ConvertToCMAP8(KBitmap* source, KBitmap* destination)
{
...
status_t
KIconUtils::ConvertFromCMAP8(const uint8* src, uint32 width, uint32 height,
	uint32 srcBPR, KBitmap* icon)
{
	...
	if (dstWidth < width || dstHeight < height
		...
		KBitmap* converted = new KBitmap(BRect(0, 0, width - 1, height - 1),
			icon->ColorSpace());
	...
	if (dstWidth > width && dstHeight > height
		...
		KBitmap* temp = new KBitmap(BRect(0, 0, width * 2 - 1, height * 2 - 1),
			icon->ColorSpace());
		...
		KBitmap* temp = new KBitmap(BRect(0, 0, width * 3 - 1, height * 3 - 1),
			icon->ColorSpace());
		uint8* tempBits = (uint8*)temp->Bits();
		uint32 tempBPR = temp->BytesPerRow();
		...
		KBitmap* temp = new KBitmap(BRect(0, 0, width * 4 - 1, height * 4 - 1),
			icon->ColorSpace());
		uint8* tempBits = (uint8*)temp->Bits();
		uint32 tempBPR = temp->BytesPerRow();
...
status_t
KIconUtils::ConvertToCMAP8(const uint8* src, uint32 width, uint32 height,
	uint32 srcBPR, KBitmap* icon)
{
	if (src == NULL || icon == NULL || srcBPR == 0)
		return B_BAD_VALUE;
...
KIconUtils::KIconUtils() {}
KIconUtils::~KIconUtils() {}
KIconUtils::KIconUtils(const KIconUtils&) {}
KIconUtils& KIconUtils::operator=(const KIconUtils&) { return *this; }
