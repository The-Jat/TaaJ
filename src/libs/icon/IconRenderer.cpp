...
//khidki start
class KIconRenderer::StyleHandler {
	struct StyleItem {
		Style*			style;
		Transformation	transformation;
	};
...
// color
const agg::rgba8&
KIconRenderer::StyleHandler::color(unsigned styleIndex)
{
	StyleItem* styleItem = (StyleItem*)fStyles.ItemAt(styleIndex);
	if (!styleItem) {
		printf("no style at index: %u!\n", styleIndex);
...
// generate_span
void
KIconRenderer::StyleHandler::generate_span(agg::rgba8* span, int x, int y,
	unsigned len, unsigned styleIndex)
{
	StyleItem* styleItem = (StyleItem*)fStyles.ItemAt(styleIndex);
	if (!styleItem || !styleItem->style->Gradient()) {
...
template<class GradientFunction>
void
KIconRenderer::StyleHandler::_GenerateGradient(agg::rgba8* span, int x, int y,
	unsigned len, GradientFunction function, int32 start, int32 end,
	const agg::rgba8* gradientColors, Transformation& gradientTransform)
{
	typedef agg::pod_auto_array<agg::rgba8, 256>	ColorArray;
...
// constructor
KIconRenderer::KIconRenderer(KBitmap* bitmap)
	: fBitmap(bitmap),
	  fBackground(NULL),
	  fBackgroundColor(0, 0, 0, 0),
	  fIcon(NULL),
...
// destructor
KIconRenderer::~KIconRenderer()
{
}

// SetIcon
void
KIconRenderer::SetIcon(const Icon* icon)
{
	if (fIcon == icon)
		return;
...
// Render
void
KIconRenderer::Render(const BRect& area)
{
	_Render(fBitmap->Bounds() & area);
}
...
//SetScale
void
KIconRenderer::SetScale(double scale)
{
	fGlobalTransform.reset();
	fGlobalTransform.multiply(agg::trans_affine_scaling(scale));
}
...
//SetBackground
void
KIconRenderer::SetBackground(const KBitmap* background)
{
	fBackground = background;
}
...
//SetBackground
void
KIconRenderer::SetBackground(const agg::rgba8& background)
{
	fBackgroundColor.r = fGammaTable.dir(background.r);
	fBackgroundColor.g = fGammaTable.dir(background.g);
	fBackgroundColor.b = fGammaTable.dir(background.b);
...
// _Render
void
KIconRenderer::_Render(const BRect& r)
{
	if (!fIcon)
		return;
...
//bigtime_t start = system_time();
	...
	for (int32 i = 0; i < shapeCount; i++) {
		...
		if (!styleAdded) {
			printf("KIconRenderer::_Render() - out of memory\n");
			break;
		}
...
// _CommitRenderPass
void
KIconRenderer::_CommitRenderPass(StyleHandler& styleHandler, bool reset)
{
	agg::render_scanlines_compound(fRasterizer, fScanline, fBinaryScanline,
		fBaseRendererPre, fSpanAllocator, styleHandler);
