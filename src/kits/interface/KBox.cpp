...
KBox::KBox(BRect frame, const char* name, uint32 resizingMode, uint32 flags,
		border_style border)
	:
	KView(frame, name, resizingMode, flags  | B_WILL_DRAW | B_FRAME_EVENTS),
	  fStyle(border)
{
	_InitObject();
}
...
KBox::KBox(const char* name, uint32 flags, border_style border, KView* child)
	:
	KView(name, flags | B_WILL_DRAW | B_FRAME_EVENTS),
	fStyle(border)
{
	_InitObject();
...
KBox::KBox(border_style border, KView* child)
	:
	KView(NULL, B_WILL_DRAW | B_FRAME_EVENTS | B_NAVIGABLE_JUMP),
	fStyle(border)
{
	_InitObject();
...
BArchivable*
KBox::Instantiate(BMessage* archive)
{
	if (validate_instantiation(archive, "KBox"))
		return new KBox(archive);
...
status_t
KBox::Archive(BMessage* archive, bool deep) const
{
	status_t ret = KView::Archive(archive, deep);
...
void
KBox::SetBorder(border_style border)
{
	if (border == fStyle)
		return;
...
void
KBox::SetLabel(const char* string)
{
	_ClearLabel();
...
void
KBox::FrameResized(float width, float height)
{
	BRect bounds(Bounds());
	...
	if (fStyle != B_NO_BORDER) {
		// TODO: this must be made part of the k_be_control_look stuff!
...
void
KBox::MessageReceived(BMessage* message)
{
...
void
KBox::WindowActivated(bool active)
{
...
void
KBox::MouseMoved(BPoint point, uint32 transit, const BMessage* message)
{
	KView::MouseMoved(point, transit, message);
}


void
KBox::FrameMoved(BPoint newLocation)
{
	KView::FrameMoved(newLocation);
}


BHandler*
KBox::ResolveSpecifier(BMessage* message, int32 index, BMessage* specifier,
...
void
KBox::GetPreferredSize(float* _width, float* _height)
{
	_ValidateLayoutData();
...
status_t
KBox::GetSupportedSuites(BMessage* message)
{
...
status_t
KBox::Perform(perform_code code, void* _data)
{
	switch (code) {
		...
		case PERFORM_CODE_LAYOUT_ALIGNMENT:
			((perform_data_layout_alignment*)_data)->return_value
				= KBox::LayoutAlignment();
			return B_OK;
		case PERFORM_CODE_HAS_HEIGHT_FOR_WIDTH:
			((perform_data_has_height_for_width*)_data)->return_value
				= KBox::HasHeightForWidth();
			return B_OK;
		case PERFORM_CODE_GET_HEIGHT_FOR_WIDTH:
		{
			perform_data_get_height_for_width* data
				= (perform_data_get_height_for_width*)_data;
			KBox::GetHeightForWidth(data->width, &data->min, &data->max,
				&data->preferred);
			return B_OK;
		}
		...
		case PERFORM_CODE_LAYOUT_INVALIDATED:
		{
			...
			KBox::LayoutInvalidated(data->descendants);
			return B_OK;
		}
...
KBox::LayoutAlignment()
{
	...
	return KLayoutUtils::ComposeAlignment(ExplicitAlignment(), alignment);
}


void
KBox::LayoutInvalidated(bool descendants)
{
...
KBox::DoLayout()
{
	...
	KLayout* layout = GetLayout();

	// If the user set a layout, let the base class version call its
	// hook. In case when we have KView as a label, remove it from child list
	...
	if (child != NULL) {
		...
		if ((child->Flags() & B_SUPPORTS_LAYOUT) != 0)
			KLayoutUtils::AlignInFrame(child, frame);
		else
...
void KBox::_ReservedBox1() {}
void KBox::_ReservedBox2() {}


KBox &
KBox::operator=(const KBox &)
{
	return *this;
}
...
void
KBox::_InitObject(BMessage* archive)
{
	fBounds = Bounds().OffsetToCopy(0, 0);
...
void
KBox::_DrawPlain(BRect labelBox)
{
	BRect rect = Bounds();
...
void
KBox::_DrawFancy(BRect labelBox)
{
	BRect rect = Bounds();
	rect.top += TopBorderOffset();
	...
	if (rect.Height() == 1.0) {
		...
		k_be_control_look->DrawGroupFrame(this, rect, rect, base,
			KControlLook::B_TOP_BORDER);
	} else if (rect.Width() == 1.0) {
		// used as vertical separator
		k_be_control_look->DrawGroupFrame(this, rect, rect, base,
			KControlLook::B_LEFT_BORDER);
	} else {
		// used as box
		k_be_control_look->DrawGroupFrame(this, rect, rect, base);
	}
...
KBox::_Child() const
{
	for (int32 i = 0; KView* view = ChildAt(i); i++) {
		if (view != fLabelView)
			return view;
	}
...
KBox::_ValidateLayoutData()
{
	...
	if (child && (child->Flags() & B_SUPPORTS_LAYOUT)) {
		...
		max.width = KLayoutUtils::AddDistances(max.width, addWidth - 1);
		max.height = KLayoutUtils::AddDistances(max.height, addHeight - 1);

		if (min.width < minWidth)
			min.width = minWidth;
		KLayoutUtils::FixSizeConstraints(min, max, preferred);
...
extern "C" void
B_IF_GCC_2(InvalidateLayout__4KBoxb, _ZN4KBox16InvalidateLayoutEb)(
	KBox* box, bool descendants)
{
	perform_data_layout_invalidated data;
