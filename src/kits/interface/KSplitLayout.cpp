...
// archivng constants
namespace {
	const char* const kItemCollapsibleField = "KSplitLayout:item:collapsible";
	const char* const kItemWeightField = "KSplitLayout:item:weight";
	const char* const kSpacingField = "KSplitLayout:spacing";
	const char* const kSplitterSizeField = "KSplitLayout:splitterSize";
	const char* const kIsVerticalField = "KSplitLayout:vertical";
	const char* const kInsetsField = "KSplitLayout:insets";
}


class KSplitLayout::ItemLayoutInfo {
public:
	float		weight;
	BRect		layoutFrame;
	BSize		min;
...
class KSplitLayout::SplitterItem : public KLayoutItem {
public:
	SplitterItem(KSplitLayout* layout)
		:
		fLayout(layout),
		fFrame()
	{
...
KSplitLayout::KSplitLayout(orientation orientation, float spacing)
	:
	...
	fBottomInset(0),
	fSplitterSize(6),
	fSpacing(KControlLook::ComposeSpacing(spacing)),
...
KSplitLayout::KSplitLayout(BMessage* from)
	:
	...
	fBottomInset(0),
	fSplitterSize(6),
	fSpacing(k_be_control_look->DefaultItemSpacing()),
...
KSplitLayout::~KSplitLayout()
{
}


void
KSplitLayout::SetInsets(float left, float top, float right, float bottom)
{
	fLeftInset = left;
	fTopInset = top;
	fRightInset = right;
...
void
KSplitLayout::GetInsets(float* left, float* top, float* right,
	float* bottom) const
{
	if (left)
		*left = fLeftInset;
...
void
KSplitLayout::SetSpacing(float spacing)
{
...
orientation
KSplitLayout::Orientation() const
{
	return fOrientation;
}
...
void
KSplitLayout::SetOrientation(orientation orientation)
{
...
float
KSplitLayout::SplitterSize() const
{
...
void
KSplitLayout::SetSplitterSize(float size)
{
...
KLayoutItem*
KSplitLayout::AddView(KView* child)
{
	return KAbstractLayout::AddView(child);
}


KLayoutItem*
KSplitLayout::AddView(int32 index, KView* child)
{
	return KAbstractLayout::AddView(index, child);
}


KLayoutItem*
KSplitLayout::AddView(KView* child, float weight)
{
...
KLayoutItem*
KSplitLayout::AddView(int32 index, KView* child, float weight)
{
...
bool
KSplitLayout::AddItem(KLayoutItem* item)
{
	return KAbstractLayout::AddItem(item);
}


bool
KSplitLayout::AddItem(int32 index, KLayoutItem* item)
{
	return KAbstractLayout::AddItem(index, item);
}


bool
KSplitLayout::AddItem(KLayoutItem* item, float weight)
{
...
bool
KSplitLayout::AddItem(int32 index, KLayoutItem* item, float weight)
{
...
float
KSplitLayout::ItemWeight(int32 index) const
{
...
float
KSplitLayout::ItemWeight(KLayoutItem* item) const
{
...
void
KSplitLayout::SetItemWeight(int32 index, float weight, bool invalidateLayout)
{
	if (index < 0 || index >= CountItems())
		return;

	KLayoutItem* item = ItemAt(index);
	SetItemWeight(item, weight);
...
void
KSplitLayout::SetItemWeight(KLayoutItem* item, float weight)
{
	if (ItemLayoutInfo* info = _ItemLayoutInfo(item))
		info->weight = weight;
}
...
bool
KSplitLayout::IsCollapsible(int32 index) const
{
...
void
KSplitLayout::SetCollapsible(bool collapsible)
{
...
void
KSplitLayout::SetCollapsible(int32 index, bool collapsible)
{
...
void
KSplitLayout::SetCollapsible(int32 first, int32 last, bool collapsible)
{
...
bool
KSplitLayout::IsItemCollapsed(int32 index) const
{
...
void
KSplitLayout::SetItemCollapsed(int32 index, bool collapsed)
{
...
BSize
KSplitLayout::BasePreferredSize()
{
	_ValidateMinMax();
...
bool
KSplitLayout::HasHeightForWidth()
{
	_ValidateMinMax();
...
void
KSplitLayout::GetHeightForWidth(float width, float* min, float* max,
	float* preferred)
{
...
void
KSplitLayout::LayoutInvalidated(bool children)
{
...
BRect
KSplitLayout::SplitterItemFrame(int32 index) const
{
...
bool
KSplitLayout::IsAboveSplitter(const BPoint& point) const
{
...
bool
KSplitLayout::StartDraggingSplitter(BPoint point)
{
...
bool
KSplitLayout::DragSplitter(BPoint point)
{
...
bool
KSplitLayout::StopDraggingSplitter()
{
	if (fDraggingSplitterIndex < 0)
		return false;
...
int32
KSplitLayout::DraggedSplitter() const
{
...
status_t
KSplitLayout::Archive(BMessage* into, bool deep) const
{
...
BArchivable*
KSplitLayout::Instantiate(BMessage* from)
{
	if (validate_instantiation(from, "KSplitLayout"))
		return new(std::nothrow) KSplitLayout(from);
	return NULL;
}


status_t
KSplitLayout::ItemArchived(BMessage* into, KLayoutItem* item, int32 index) const
{
...
status_t
KSplitLayout::ItemUnarchived(const BMessage* from,
...
bool
KSplitLayout::ItemAdded(KLayoutItem* item, int32 atIndex)
{
...
void
KSplitLayout::ItemRemoved(KLayoutItem* item, int32 atIndex)
{
...
void
KSplitLayout::_InvalidateCachedHeightForWidth()
{
...
KSplitLayout::SplitterItem*
KSplitLayout::_SplitterItemAt(const BPoint& point, int32* index) const
{
...
KSplitLayout::SplitterItem*
KSplitLayout::_SplitterItemAt(int32 index) const
{
...
void
KSplitLayout::_GetSplitterValueRange(int32 index, ValueRange& range)
{
...
int32
KSplitLayout::_SplitterValue(int32 index) const
{
...
void
KSplitLayout::_LayoutItem(KLayoutItem* item, BRect frame, bool visible)
{
...
void
KSplitLayout::_LayoutItem(KLayoutItem* item, ItemLayoutInfo* info)
{
...
bool
KSplitLayout::_SetSplitterValue(int32 index, int32 value)
{
	// if both items are collapsed, nothing can be dragged
	KLayoutItem* previousItem = ItemAt(index);
	KLayoutItem* nextItem = ItemAt(index + 1);
...
KSplitLayout::ItemLayoutInfo*
KSplitLayout::_ItemLayoutInfo(KLayoutItem* item) const
{
...
void
KSplitLayout::_UpdateSplitterWeights()
{
...
KSplitLayout::_ValidateMinMax()
{
	...
	if (itemCount > 0) {
		for (int32 i = 0; i < itemCount; i++) {
			KLayoutItem* item = (KLayoutItem*)fVisibleItems.ItemAt(i);
...
void
KSplitLayout::_InternalGetHeightForWidth(float width, bool realLayout,
...
float
KSplitLayout::_SplitterSpace() const
{
...
BSize
KSplitLayout::_AddInsets(BSize size)
{
	size.width = KLayoutUtils::AddDistances(size.width,
		fLeftInset + fRightInset - 1);
	size.height = KLayoutUtils::AddDistances(size.height,
		fTopInset + fBottomInset - 1);

	float spacing = _SplitterSpace();
	if (fOrientation == B_HORIZONTAL)
		size.width = KLayoutUtils::AddDistances(size.width, spacing - 1);
	else
		size.height = KLayoutUtils::AddDistances(size.height, spacing - 1);
...
void
KSplitLayout::_AddInsets(float* minHeight, float* maxHeight,
	float* preferredHeight)
{
	...
	if (minHeight)
		*minHeight = KLayoutUtils::AddDistances(*minHeight, insets);
	if (maxHeight)
		*maxHeight = KLayoutUtils::AddDistances(*maxHeight, insets);
	if (preferredHeight)
		*preferredHeight = KLayoutUtils::AddDistances(*preferredHeight, insets);
}


BSize
KSplitLayout::_SubtractInsets(BSize size)
{
	size.width = KLayoutUtils::SubtractDistances(size.width,
		fLeftInset + fRightInset - 1);
	size.height = KLayoutUtils::SubtractDistances(size.height,
		fTopInset + fBottomInset - 1);

	float spacing = _SplitterSpace();
	if (fOrientation == B_HORIZONTAL)
		size.width = KLayoutUtils::SubtractDistances(size.width, spacing - 1);
	else
		size.height = KLayoutUtils::SubtractDistances(size.height, spacing - 1);
