...
KListView::KListView(BRect frame, const char* name, list_view_type type,
	uint32 resizingMode, uint32 flags)
	:
	KView(frame, name, resizingMode, flags | B_SCROLL_VIEW_AWARE)
{
	_InitObject(type);
}


KListView::KListView(const char* name, list_view_type type, uint32 flags)
	:
	KView(name, flags | B_SCROLL_VIEW_AWARE)
{
	_InitObject(type);
}


KListView::KListView(list_view_type type)
	:
	KView(NULL, B_WILL_DRAW | B_FRAME_EVENTS | B_NAVIGABLE
		| B_SCROLL_VIEW_AWARE)
{
	_InitObject(type);
}
...
KListView::KListView(BMessage* archive)
	:
	KView(archive)
{
	int32 listType;
...
KListView::~KListView()
{
	// NOTE: According to BeBook, KListView does not free the items itself.
	delete fTrack;
...
BArchivable*
KListView::Instantiate(BMessage* archive)
{
	if (validate_instantiation(archive, "KListView"))
		return new KListView(archive);
...
status_t
KListView::Archive(BMessage* data, bool deep) const
{
...
void
KListView::Draw(BRect updateRect)
{
...
void
KListView::FrameResized(float newWidth, float newHeight)
{
	_FixupScrollBar();
...
void
KListView::FrameMoved(BPoint newPosition)
{
...
void
KListView::TargetedByScrollView(KScrollView* view)
{
	fScrollView = view;
...
void
KListView::WindowActivated(bool active)
{
...
void
KListView::MessageReceived(BMessage* message)
{
...
void
KListView::KeyDown(const char* bytes, int32 numBytes)
{
...
void
KListView::MouseDown(BPoint where)
{
	if (!IsFocus()) {
		MakeFocus();
		Sync();
...
void
KListView::MouseUp(BPoint where)
{
	KView::MouseUp(where);
...
void
KListView::MouseMoved(BPoint where, uint32 code, const BMessage* dragMessage)
{
...
bool
KListView::InitiateDrag(BPoint where, int32 index, bool wasSelected)
{
	return false;
}
...
void
KListView::GetPreferredSize(float *_width, float *_height)
{
	int32 count = CountItems();
...
KListView::MinSize()
{
	// We need a stable min size: the KView implementation uses
	// GetPreferredSize(), which by default just returns the current size.
	return KLayoutUtils::ComposeSize(ExplicitMinSize(), BSize(10, 10));
}
...
KListView::MaxSize()
{
	return KView::MaxSize();
}


BSize
KListView::PreferredSize()
{
	// We need a stable preferred size: the KView implementation uses
	// GetPreferredSize(), which by default just returns the current size.
	return KLayoutUtils::ComposeSize(ExplicitPreferredSize(), BSize(100, 50));
}
...
void
KListView::MakeFocus(bool focused)
{
	if (IsFocus() == focused)
		return;
...
void
KListView::SetFont(const BFont* font, uint32 mask)
{
...
void
KListView::ScrollTo(BPoint point)
{
	KView::ScrollTo(point);
}
...
bool
KListView::AddItem(KListItem* item, int32 index)
{
...
bool
KListView::AddItem(KListItem* item)
{
...
bool
KListView::AddList(BList* list, int32 index)
{
...
KListItem*
KListView::RemoveItem(int32 index)
{
...
bool
KListView::RemoveItem(KListItem* item)
{
	return KListView::RemoveItem(IndexOf(item)) != NULL;
}


bool
KListView::RemoveItems(int32 index, int32 count)
{
...
void
KListView::SetSelectionMessage(BMessage* message)
{
...
void
KListView::SetInvocationMessage(BMessage* message)
{
...
BMessage*
KListView::InvocationMessage() const
{
...
uint32
KListView::InvocationCommand() const
{
...
BMessage*
KListView::SelectionMessage() const
{
...
uint32
KListView::SelectionCommand() const
{
	if (fSelectMessage)
...
void
KListView::SetListType(list_view_type type)
{
	if (fListType == B_MULTIPLE_SELECTION_LIST
...
KListItem*
KListView::ItemAt(int32 index) const
{
	return (KListItem*)fList.ItemAt(index);
}


int32
KListView::IndexOf(KListItem* item) const
{
	if (Window()) {
		if (item != NULL) {
...
int32
KListView::IndexOf(BPoint point) const
{
	int32 low = 0;
...
KListItem*
KListView::FirstItem() const
{
	return (KListItem*)fList.FirstItem();
}


KListItem*
KListView::LastItem() const
{
	return (KListItem*)fList.LastItem();
}


bool
KListView::HasItem(KListItem *item) const
{
...
void
KListView::DoForEach(bool (*func)(KListItem*))
{
...
void
KListView::DoForEach(bool (*func)(KListItem*, void*), void* arg)
{
...
const KListItem**
KListView::Items() const
{
	return (const KListItem**)fList.Items();
}


void
KListView::InvalidateItem(int32 index)
{
...
void
KListView::Select(int32 index, bool extend)
{
...
void
KListView::Select(int32 start, int32 finish, bool extend)
{
...
bool
KListView::IsItemSelected(int32 index) const
{
	KListItem* item = ItemAt(index);
	if (item != NULL)
...
int32
KListView::CurrentSelection(int32 index) const
{
	if (fFirstSelected == -1)
		return -1;
...
status_t
KListView::Invoke(BMessage* message)
{
...
void
KListView::DeselectExcept(int32 exceptFrom, int32 exceptTo)
{
...
void
KListView::Deselect(int32 index)
{
	if (_Deselect(index)) {
		SelectionChanged();
		InvokeNotify(fSelectMessage, B_CONTROL_MODIFIED);
...
void
KListView::SortItems(int (*cmp)(const void *, const void *))
{
	if (_DeselectAll(-1, -1)) {
		SelectionChanged();
		InvokeNotify(fSelectMessage, B_CONTROL_MODIFIED);
...
bool
KListView::SwapItems(int32 a, int32 b)
{
	MiscData data;
...
bool
KListView::MoveItem(int32 from, int32 to)
{
	MiscData data;
...
bool
KListView::ReplaceItem(int32 index, KListItem* item)
{
	MiscData data;
...
BRect
KListView::ItemFrame(int32 index)
{
	BRect frame = Bounds();
	if (index < 0 || index >= CountItems()) {
		frame.top = 0;
...
BHandler*
KListView::ResolveSpecifier(BMessage* message, int32 index,
...
status_t
KListView::GetSupportedSuites(BMessage* data)
{
	if (data == NULL)
		return B_BAD_VALUE;
...
status_t
KListView::Perform(perform_code code, void* _data)
{
	switch (code) {
		...
		case PERFORM_CODE_LAYOUT_ALIGNMENT:
			((perform_data_layout_alignment*)_data)->return_value
				= KListView::LayoutAlignment();
			return B_OK;
		case PERFORM_CODE_HAS_HEIGHT_FOR_WIDTH:
			((perform_data_has_height_for_width*)_data)->return_value
				= KListView::HasHeightForWidth();
			return B_OK;
		case PERFORM_CODE_GET_HEIGHT_FOR_WIDTH:
		{
			perform_data_get_height_for_width* data
				= (perform_data_get_height_for_width*)_data;
			KListView::GetHeightForWidth(data->width, &data->min, &data->max,
				&data->preferred);
			return B_OK;
		}
		case PERFORM_CODE_SET_LAYOUT:
		{
			...
			KListView::SetLayout(data->layout);
			return B_OK;
		}
		case PERFORM_CODE_LAYOUT_INVALIDATED:
		{
			...
			KListView::LayoutInvalidated(data->descendants);
			return B_OK;
		}
...
bool
KListView::DoMiscellaneous(MiscCode code, MiscData* data)
{
	if (code > B_SWAP_OP)
		return false;
...
void KListView::_ReservedListView2() {}
void KListView::_ReservedListView3() {}
void KListView::_ReservedListView4() {}


KListView&
KListView::operator=(const KListView& /*other*/)
{
	return *this;
}
...
void
KListView::_InitObject(list_view_type type)
{
	fListType = type;
	fFirstSelected = -1;
	fLastSelected = -1;
...
void
KListView::_FixupScrollBar()
{

	KScrollBar* vertScroller = ScrollBar(B_VERTICAL);
	...
	KScrollBar* horizontalScroller = ScrollBar(B_HORIZONTAL);
...
void
KListView::_InvalidateFrom(int32 index)
{
...
*/
bool
KListView::_Select(int32 index, bool extend)
{
	if (index < 0 || index >= CountItems())
		return false;
...
*/
bool
KListView::_Select(int32 from, int32 to, bool extend)
{
	if (to < from)
		return false;
...
bool
KListView::_Deselect(int32 index)
{
	if (index < 0 || index >= CountItems())
		return false;
...
bool
KListView::_DeselectAll(int32 exceptFrom, int32 exceptTo)
{
	if (fFirstSelected == -1)
		return false;
...
int32
KListView::_CalcFirstSelected(int32 after)
{
	if (after >= CountItems())
		return -1;
...
int32
KListView::_CalcLastSelected(int32 before)
{
	if (before < 0)
		return -1;
...
void
KListView::DrawItem(KListItem* item, BRect itemRect, bool complete)
{
	if (!item->IsEnabled()) {
...
bool
KListView::_SwapItems(int32 a, int32 b)
{
...
bool
KListView::_MoveItem(int32 from, int32 to)
{
...
bool
KListView::_ReplaceItem(int32 index, KListItem* item)
{
	if (item == NULL)
		return false;

	KListItem* old = ItemAt(index);
	if (!old)
		return false;
...
void
KListView::_RescanSelection(int32 from, int32 to)
{
	if (from > to) {
		int32 tmp = from;
		from = to;
...
void
KListView::_RecalcItemTops(int32 start, int32 end)
{
	int32 count = CountItems();
