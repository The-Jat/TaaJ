...
public:
	...
	void DrawScrollBarButton(KScrollBar* owner, arrow_direction direction,
...
int32
KScrollBar::Private::button_repeater_thread(void* data)
{
	KScrollBar::Private* privateData = (KScrollBar::Private*)data;
	return privateData->ButtonRepeaterThread();
}


int32
KScrollBar::Private::ButtonRepeaterThread()
{
...
//	#pragma mark - KScrollBar


KScrollBar::KScrollBar(BRect frame, const char* name, KView* target,
	float min, float max, orientation direction)
	:
	KView(frame, name, B_FOLLOW_NONE,
	...
	fOrientation(direction)
{
	...
	fPrivateData = new KScrollBar::Private(this);
...
KScrollBar::KScrollBar(const char* name, KView* target,
	float min, float max, orientation direction)
	:
	KView(name, B_WILL_DRAW | B_FULL_UPDATE_ON_RESIZE | B_FRAME_EVENTS),
	fMin(min),
	fMax(max),
	fSmallStep(1.0f),
	fLargeStep(10.0f),
...
KScrollBar::KScrollBar(BMessage* data)
	:
	KView(data),
	fTarget(NULL)
{
...
{
	SetTarget((KView*)NULL);
	delete fPrivateData;
}


BArchivable*
KScrollBar::Instantiate(BMessage* data)
{
	if (validate_instantiation(data, "KScrollBar"))
		return new KScrollBar(data);
	return NULL;
}


status_t
KScrollBar::Archive(BMessage* data, bool deep) const
{
...
void
KScrollBar::Draw(BRect updateRect)
{
	...
	if (scrollingEnabled)
		flags |= KControlLook::B_PARTIALLY_ACTIVATED;
	...
	// draw border
	BRect rect = Bounds();
	k_be_control_look->DrawScrollBarBorder(this, rect, updateRect, base, flags,
		fOrientation);
	...
	if (fOrientation == B_HORIZONTAL) {
		...
		k_be_control_look->DrawScrollBarButton(this, buttonFrame, updateRect,
			base, flags | (fPrivateData->fButtonDown == ARROW1
				? KControlLook::B_ACTIVATED : 0),
			KControlLook::B_LEFT_ARROW, fOrientation,
		...
		if (doubleArrows) {
			...
			k_be_control_look->DrawScrollBarButton(this, buttonFrame, updateRect,
				base, flags | (fPrivateData->fButtonDown == ARROW2
					? KControlLook::B_ACTIVATED : 0),
				KControlLook::B_RIGHT_ARROW, fOrientation,
		...
		k_be_control_look->DrawScrollBarButton(this, buttonFrame, updateRect,
			base, flags | (fPrivateData->fButtonDown == ARROW1
				? KControlLook::B_ACTIVATED : 0),
			KControlLook::B_UP_ARROW, fOrientation,
			fPrivateData->fButtonDown == ARROW1);

		if (doubleArrows) {
			buttonFrame.OffsetBy(0, rect.Width() + 1);
			k_be_control_look->DrawScrollBarButton(this, buttonFrame,
				updateRect, base, flags | (fPrivateData->fButtonDown == ARROW2
					? KControlLook::B_ACTIVATED : 0),
				KControlLook::B_DOWN_ARROW, fOrientation,
				fPrivateData->fButtonDown == ARROW2);
	...
	if (fOrientation == B_HORIZONTAL) {
		...
		k_be_control_look->DrawScrollBarBackground(this, leftOfThumb,
		...
		BRect bottomOfThumb(thumbBG.left, rect.bottom + 1,
			thumbBG.right, thumbBG.bottom);
		k_be_control_look->DrawScrollBarBackground(this, topOfThumb,
			bottomOfThumb, updateRect, base, flags, fOrientation);
	}

	// draw thumb
	rect = fPrivateData->fThumbFrame;
	k_be_control_look->DrawScrollBarThumb(this, rect, updateRect,
...
void
KScrollBar::FrameMoved(BPoint newPosition)
{
...
void
KScrollBar::FrameResized(float newWidth, float newHeight)
{
	_UpdateThumbFrame();
}
...
void
KScrollBar::MessageReceived(BMessage* message)
{
	switch(message->what) {
		...
		{
			// Must handle this here since KView checks for the existence of
...
void
KScrollBar::MouseDown(BPoint where)
{
...
void
KScrollBar::MouseMoved(BPoint where, uint32 code, const BMessage* dragMessage)
{
...
void
KScrollBar::MouseUp(BPoint where)
{
	if (fPrivateData->fButtonDown == THUMB)
		Invalidate(fPrivateData->fThumbFrame);
	else
...
void
KScrollBar::WindowActivated(bool active)
{
...
void
KScrollBar::SetValue(float value)
{
	if (value > fMax)
		value = fMax;
	else if (value < fMin)
	...
	TRACE("KScrollBar(%s)::SetValue(%.1f)\n", Name(), value);
...
void
KScrollBar::ValueChanged(float newValue)
{
	TRACE("KScrollBar(%s)::ValueChanged(%.1f)\n", Name(), newValue);
...
void
KScrollBar::SetProportion(float value)
{
	if (value < 0.0f)
		value = 0.0f;
	else if (value > 1.0f)
	...
	TRACE("KScrollBar(%s)::SetProportion(%.1f)\n", Name(), value);
...
void
KScrollBar::SetRange(float min, float max)
{
	...
	TRACE("KScrollBar(%s)::SetRange(min=%.1f, max=%.1f)\n", Name(), min, max);
...
void
KScrollBar::GetRange(float* min, float* max) const
{
	if (min != NULL)
		*min = fMin;
...
void
KScrollBar::SetSteps(float smallStep, float largeStep)
{
	...
	TRACE("KScrollBar(%s)::SetSteps(small=%.1f, large=%.1f)\n", Name(),
		smallStep, largeStep);
...
void
KScrollBar::GetSteps(float* smallStep, float* largeStep) const
{
	if (smallStep != NULL)
...
void
KScrollBar::SetTarget(KView* target)
{
	if (fTarget) {
...
void
KScrollBar::SetTarget(const char* targetName)
{
	...
	KView* target = Window()->FindView(targetName);
	if (target != NULL)
		SetTarget(target);
}
...
void
KScrollBar::SetOrientation(orientation direction)
{
...
status_t
KScrollBar::SetBorderHighlighted(bool highlight)
{
...
void
KScrollBar::GetPreferredSize(float* _width, float* _height)
{
	if (fOrientation == B_VERTICAL) {
		if (_width)
			*_width = k_be_control_look->GetScrollBarWidth(B_VERTICAL);
		...
		if (_height)
			*_height = k_be_control_look->GetScrollBarWidth(B_HORIZONTAL);
	}
}


void
KScrollBar::ResizeToPreferred()
{
...
void
KScrollBar::MakeFocus(bool focus)
{
...
KScrollBar::MinSize()
{
	return KLayoutUtils::ComposeSize(ExplicitMinSize(), _MinSize());
}
...
status_t
KScrollBar::GetSupportedSuites(BMessage* message)
{
...
BHandler*
KScrollBar::ResolveSpecifier(BMessage* message, int32 index,
...
status_t
KScrollBar::Perform(perform_code code, void* _data)
{
	switch (code) {
		...
		case PERFORM_CODE_LAYOUT_ALIGNMENT:
			((perform_data_layout_alignment*)_data)->return_value
				= KScrollBar::LayoutAlignment();

			return B_OK;

		case PERFORM_CODE_HAS_HEIGHT_FOR_WIDTH:
			((perform_data_has_height_for_width*)_data)->return_value
				= KScrollBar::HasHeightForWidth();
		...
		case PERFORM_CODE_GET_HEIGHT_FOR_WIDTH:
		{
			...
			KScrollBar::GetHeightForWidth(data->width, &data->min, &data->max,
				&data->preferred);
		...
		case PERFORM_CODE_SET_LAYOUT:
		{
			...
			KScrollBar::SetLayout(data->layout);
		...
		case PERFORM_CODE_LAYOUT_INVALIDATED:
		{
			...
			KScrollBar::LayoutInvalidated(data->descendants);
...
void KScrollBar::_ReservedScrollBar1() {}
void KScrollBar::_ReservedScrollBar2() {}
void KScrollBar::_ReservedScrollBar3() {}
void KScrollBar::_ReservedScrollBar4() {}


KScrollBar&
KScrollBar::operator=(const KScrollBar&)
{
	return *this;
}
...
bool
KScrollBar::_DoubleArrows() const
{
...
float
KScrollBar::_ValueFor(BPoint where) const
{
	BRect bounds = Bounds();
...
int32
KScrollBar::_ButtonFor(BPoint where) const
{
	BRect bounds = Bounds();
	bounds.InsetBy(1.0f, 1.0f);
...
BRect
KScrollBar::_ButtonRectFor(int32 button) const
{
	BRect bounds = Bounds();
	bounds.InsetBy(1.0f, 1.0f);
...
void
KScrollBar::_UpdateTargetValue(BPoint where)
{
	if (fOrientation == B_VERTICAL) {
...
void
KScrollBar::_UpdateArrowButtons()
{
...
status_t
control_scrollbar(scroll_bar_info* info, KScrollBar* bar)
{
