 ...
 *		Hiroshi Lockheimer (KTextView is based on his STEEngine)
...
#include "KStyleBuffer.h"
#include "KTextGapBuffer.h"
#include "KUndoBuffer.h"
#include "KWidthBuffer.h"
...
#	define CALLED(x...)	FunctionTracer _ft("KTextView", __FUNCTION__, \
							sFunctionDepth)
...
class KTextView::KTextTrackState {
public:
	KTextTrackState(BMessenger messenger);
	~KTextTrackState();

	void SimulateMouseMovement(KTextView* view);
...
static property_info sPropertyList[] = {
	...
	},
	{
		...
		"Returns the text in the specified range in the KTextView.", 0,
		{ B_STRING_TYPE, 0 }
	},
	{
		"Text",
		{ B_SET_PROPERTY, 0 },
		{ B_RANGE_SPECIFIER, B_REVERSE_RANGE_SPECIFIER, 0 },
		"Removes or inserts text into the specified range in the KTextView.", 0,
		{ B_STRING_TYPE, 0 }
	},
	{
		"k_text_run_array",
...
KTextView::KTextView(const char* name, uint32 flags)
	:
	KView(name,
		flags | B_FRAME_EVENTS | B_PULSE_NEEDED | B_INPUT_METHOD_AWARE),
	fText(NULL),
...
KTextView::KTextView(const char* name, const BFont* initialFont,
...
KTextView::KTextView(BMessage* archive)
	:
	KView(archive),
	fText(NULL),
	fLines(NULL),
	...
	fLayoutData(NULL)
{
	...
	if (archive->FindData("_runs", B_RAW_TYPE, &flattenedRun, &runSize)
			== B_OK) {
		k_text_run_array* runArray = UnflattenRunArray(flattenedRun,
			(int32*)&runSize);
		if (runArray) {
...
BArchivable*
KTextView::Instantiate(BMessage* archive)
{
	CALLED();
	if (validate_instantiation(archive, "KTextView"))
		return new KTextView(archive);
	return NULL;
}


status_t
KTextView::Archive(BMessage* data, bool deep) const
{
	CALLED();
	status_t err = KView::Archive(data, deep);
	if (err == B_OK)
	...
	if (err == B_OK) {
		int32 runSize = 0;
		k_text_run_array* runArray = RunArray(0, fText->Length());
...
void
KTextView::Draw(BRect updateRect)
{
...
KTextView::MouseDown(BPoint where)
{
	...
	fTrackingMouse = new (nothrow) KTextTrackState(messenger);
...
void
KTextView::MouseUp(BPoint where)
{
	KView::MouseUp(where);
...
void
KTextView::MouseMoved(BPoint where, uint32 code, const BMessage* dragMessage)
{
...
void
KTextView::WindowActivated(bool active)
{
...
void
KTextView::FrameResized(float newWidth, float newHeight)
{
...
void
KTextView::MakeFocus(bool focus)
{
...
void
KTextView::MessageReceived(BMessage* message)
{
...
BHandler*
KTextView::ResolveSpecifier(BMessage* message, int32 index, BMessage* specifier,
	int32 what, const char* property)
{
	...
	if (propInfo.FindMatch(message, index, specifier, what, property) < B_OK) {
		target = KView::ResolveSpecifier(message, index, specifier, what,
			property);
	}
...
status_t
KTextView::GetSupportedSuites(BMessage* data)
{
	if (data == NULL)
		return B_BAD_VALUE;
...
status_t
KTextView::Perform(perform_code code, void* _data)
{
	switch (code) {
		...
		case PERFORM_CODE_PREFERRED_SIZE:
			((perform_data_preferred_size*)_data)->return_value
				= KTextView::PreferredSize();
			return B_OK;
		case PERFORM_CODE_LAYOUT_ALIGNMENT:
			((perform_data_layout_alignment*)_data)->return_value
				= KTextView::LayoutAlignment();
			return B_OK;
		case PERFORM_CODE_HAS_HEIGHT_FOR_WIDTH:
			((perform_data_has_height_for_width*)_data)->return_value
				= KTextView::HasHeightForWidth();
			return B_OK;
		case PERFORM_CODE_GET_HEIGHT_FOR_WIDTH:
		{
			perform_data_get_height_for_width* data
				= (perform_data_get_height_for_width*)_data;
			KTextView::GetHeightForWidth(data->width, &data->min, &data->max,
				&data->preferred);
			return B_OK;
		}
		case PERFORM_CODE_SET_LAYOUT:
		{
			...
			KTextView::SetLayout(data->layout);
			return B_OK;
		}
		case PERFORM_CODE_LAYOUT_INVALIDATED:
		{
			...
			KTextView::LayoutInvalidated(data->descendants);
			return B_OK;
		}
...
void
KTextView::SetText(const char* text, const k_text_run_array* runs)
{
...
void
KTextView::SetText(const char* text, int32 length, const k_text_run_array* runs)
{
	_CancelInputMethod();
...
void
KTextView::SetText(BFile* file, int32 offset, int32 length,
	const k_text_run_array* runs)
{
	CALLED();
...
void
KTextView::Insert(const char* text, const k_text_run_array* runs)
{
	if (text != NULL)
...
void
KTextView::Insert(const char* text, int32 length, const k_text_run_array* runs)
{
	if (text != NULL && length > 0)
...
void
KTextView::Insert(int32 offset, const char* text, int32 length,
	const k_text_run_array* runs)
{
...
void
KTextView::Delete(int32 startOffset, int32 endOffset)
{
	CALLED();
...
void
KTextView::GetText(int32 offset, int32 length, char* buffer) const
{
	if (buffer != NULL)
...
uchar
KTextView::ByteAt(int32 offset) const
{
...
void
KTextView::GoToLine(int32 index)
{
	_CancelInputMethod();
	_HideCaret();
...
KTextView::Copy(BClipboard* clipboard)
{
	...
	if (clipboard->Lock()) {
		...
		if (clip != NULL) {
			...
			int32 size;
			if (fStylable) {
				k_text_run_array* runArray = RunArray(fSelStart, fSelEnd, &size);
				clip->AddData("application/x-vnd.Be-k_text_run_array",
...
void
KTextView::Paste(BClipboard* clipboard)
{
	CALLED();
	_CancelInputMethod();
	...
	if (clip != NULL) {
		...
		if (clip->FindData("text/plain", B_MIME_TYPE,
			...
			k_text_run_array* runArray = NULL;
			ssize_t runLength = 0;

			if (fStylable) {
				clip->FindData("application/x-vnd.Be-k_text_run_array",
...
bool
KTextView::AcceptsPaste(BClipboard* clipboard)
{
	bool result = false;
...
bool
KTextView::AcceptsDrop(const BMessage* message)
{
...
void
KTextView::Select(int32 startOffset, int32 endOffset)
{
	CALLED();
	if (!fSelectable)
		return;
...
void
KTextView::GetSelection(int32* _start, int32* _end) const
{
	int32 start = 0;
	int32 end = 0;
...
void
KTextView::SetFontAndColor(const BFont* font, uint32 mode,
...
void
KTextView::SetFontAndColor(int32 startOffset, int32 endOffset,
...
void
KTextView::GetFontAndColor(int32 offset, BFont* _font,
...
void
KTextView::GetFontAndColor(BFont* _font, uint32* _mode,
...
void
KTextView::SetRunArray(int32 startOffset, int32 endOffset,
	const k_text_run_array* runs)
{
	CALLED();
...
k_text_run_array*
KTextView::RunArray(int32 startOffset, int32 endOffset, int32* _size) const
{
	// pin offsets at reasonable values
	if (startOffset < 0)
		startOffset = 0;
	...
	KSTEStyleRange* styleRange
		= fStyles->GetStyleRange(startOffset, endOffset - 1);
	if (styleRange == NULL)
		return NULL;

	k_text_run_array* runArray = AllocRunArray(styleRange->count, _size);
	if (runArray != NULL) {
...
int32
KTextView::LineAt(int32 offset) const
{
	// pin offset at reasonable values
	if (offset < 0)
		offset = 0;
...
int32
KTextView::LineAt(BPoint point) const
{
...
BPoint
KTextView::PointAt(int32 offset, float* _height) const
{
	// pin offset at reasonable values
	if (offset < 0)
		offset = 0;
...
int32
KTextView::OffsetAt(BPoint point) const
{
	const int32 textLength = fText->Length();
...
int32
KTextView::OffsetAt(int32 line) const
{
	if (line < 0)
		return 0;
...
void
KTextView::FindWord(int32 offset, int32* _fromOffset, int32* _toOffset)
{
	if (offset < 0) {
		if (_fromOffset)
			*_fromOffset = 0;
...
bool
KTextView::CanEndLine(int32 offset)
{
	if (offset < 0 || offset > fText->Length())
		return false;
...
float
KTextView::LineWidth(int32 lineNumber) const
{
	if (lineNumber < 0 || lineNumber >= fLines->NumLines())
		return 0;

	KSTELine* line = (*fLines)[lineNumber];
...
float
KTextView::LineHeight(int32 lineNumber) const
{
...
float
KTextView::TextHeight(int32 startLine, int32 endLine) const
{
...
void
KTextView::GetTextRegion(int32 startOffset, int32 endOffset,
...
void
KTextView::ScrollToOffset(int32 offset)
{
	BRect bounds = Bounds();
...
void
KTextView::Highlight(int32 startOffset, int32 endOffset)
{
	// pin offsets at reasonable values
	if (startOffset < 0)
		startOffset = 0;
...
void
KTextView::SetTextRect(BRect rect)
{
	if (rect == fTextRect)
		return;
...
void
KTextView::SetInsets(float left, float top, float right, float bottom)
{
...
void
KTextView::GetInsets(float* _left, float* _top, float* _right,
	float* _bottom) const
{
	if (_left)
...
void
KTextView::SetStylable(bool stylable)
{
	fStylable = stylable;
}
...
void
KTextView::SetTabWidth(float width)
{
	if (width == fTabWidth)
		return;
...
void
KTextView::MakeSelectable(bool selectable)
{
...
void
KTextView::MakeEditable(bool editable)
{
...
void
KTextView::SetWordWrap(bool wrap)
{
	if (wrap == fWrap)
		return;
...
void
KTextView::SetMaxBytes(int32 max)
{
	const int32 textLength = fText->Length();
	fMaxBytes = max;
...
void
KTextView::DisallowChar(uint32 character)
{
...
void
KTextView::AllowChar(uint32 character)
{
	if (fDisallowedChars != NULL)
...
void
KTextView::SetAlignment(alignment align)
{
	// Do a reality check
	if (fAlignment != align &&
			(align == B_ALIGN_LEFT ||
...
void
KTextView::SetAutoindent(bool state)
{
	fAutoindent = state;
}
...
bool
KTextView::DoesAutoindent() const
{
	return fAutoindent;
}
...
void
KTextView::SetColorSpace(color_space colors)
{
...
void
KTextView::MakeResizable(bool resize, KView* resizeView)
{
	if (resize) {
		fResizable = true;
...
void
KTextView::SetDoesUndo(bool undo)
{
	if (undo && fUndo == NULL)
		fUndo = new UndoBuffer(this, K_UNDO_UNAVAILABLE);
...
void
KTextView::HideTyping(bool enabled)
{
	if (enabled)
...
bool
KTextView::IsTypingHidden() const
{
...
void
KTextView::GetPreferredSize(float* _width, float* _height)
{
	CALLED();
...
{
	if (IsEditable())
		return KView::HasHeightForWidth();
...
void
KTextView::GetHeightForWidth(float width, float* min, float* max,
	float* preferred)
{
	if (IsEditable()) {
		KView::GetHeightForWidth(width, min, max, preferred);
		return;
	}
...
void
KTextView::LayoutInvalidated(bool descendants)
{
	CALLED();
...
/* static */
k_text_run_array*
KTextView::AllocRunArray(int32 entryCount, int32* outSize)
{
	int32 size = sizeof(k_text_run_array) + (entryCount - 1) * sizeof(k_text_run);

	k_text_run_array* runArray = (k_text_run_array*)calloc(size, 1);
	if (runArray == NULL) {
		if (outSize != NULL)
			*outSize = 0;
		return NULL;
	...
	// Call constructors explicitly as the k_text_run_array
...
/* static */
k_text_run_array*
KTextView::CopyRunArray(const k_text_run_array* orig, int32 countDelta)
{
	k_text_run_array* copy = AllocRunArray(countDelta, NULL);
	if (copy != NULL) {
...
/* static */
void
KTextView::FreeRunArray(k_text_run_array* array)
{
	if (array == NULL)
		return;
...
/* static */
void*
KTextView::FlattenRunArray(const k_text_run_array* runArray, int32* _size)
{
	CALLED();
...
/* static */
k_text_run_array*
KTextView::UnflattenRunArray(const void* data, int32* _size)
{
	CALLED();
	...
	k_text_run_array* runArray = AllocRunArray(count, _size);
	if (runArray == NULL)
		return NULL;
...
void
KTextView::InsertText(const char* text, int32 length, int32 offset,
	const k_text_run_array* runs)
{
	CALLED();
...
k_undo_state
KTextView::UndoState(bool* isRedo) const
{
	return fUndo == NULL ? K_UNDO_UNAVAILABLE : fUndo->State(isRedo);
}
...
void
KTextView::GetDragParameters(BMessage* drag, KBitmap** bitmap, BPoint* point,
	BHandler** handler)
{
	CALLED();
	if (drag == NULL)
	...
	int32 size = 0;
	k_text_run_array* styles = RunArray(fSelStart, fSelEnd, &size);

	if (styles != NULL) {
		drag->AddData("application/x-vnd.Be-k_text_run_array", B_MIME_TYPE,
			styles, size);
...
void KTextView::_ReservedTextView3() {}
void KTextView::_ReservedTextView4() {}
void KTextView::_ReservedTextView5() {}
void KTextView::_ReservedTextView6() {}
void KTextView::_ReservedTextView7() {}
void KTextView::_ReservedTextView8() {}
void KTextView::_ReservedTextView9() {}
void KTextView::_ReservedTextView10() {}
void KTextView::_ReservedTextView11() {}
void KTextView::_ReservedTextView12() {}


// #pragma mark - Private methods


/*!	Inits the KTextView object.

	\param textRect The KTextView's text rect.
	\param initialFont The font which the KTextView will use.
	\param initialColor The initial color of the text.
*/
void
KTextView::_InitObject(BRect textRect, const BFont* initialFont,
	const rgb_color* initialColor)
{
debug_printf("[KTextView]{_InitObject}\n");
	BFont font;
	if (initialFont == NULL)
		GetFont(&font);
	else
	...
	fText = new BPrivate::KTextGapBuffer;
...
debug_printf("[KTextView]{_InitObject}end\n");
}


//!	Handles when Backspace key is pressed.
void
KTextView::_HandleBackspace(int32 modifiers)
{
	if (modifiers < 0) {
		BMessage* currentMessage = Window()->CurrentMessage();
		if (currentMessage == NULL
...
void
KTextView::_HandleArrowKey(uint32 arrowKey, int32 modifiers)
{
...
void
KTextView::_HandleDelete(int32 modifiers)
{
	if (modifiers < 0) {
		BMessage* currentMessage = Window()->CurrentMessage();
		if (currentMessage == NULL
...
void
KTextView::_HandlePageKey(uint32 pageKey, int32 modifiers)
{
	if (modifiers < 0) {
		BMessage* currentMessage = Window()->CurrentMessage();
		if (currentMessage == NULL
...
*/
void
KTextView::_HandleAlphaKey(const char* bytes, int32 numBytes)
{
	// TODO: block input if not editable (Andrew)
	if (fUndo) {
		TypingUndoBuffer* undoBuffer = dynamic_cast<TypingUndoBuffer*>(fUndo);
...
*/
void
KTextView::_Refresh(int32 fromOffset, int32 toOffset, int32 scrollTo)
{
	// TODO: Cleanup
...
*/
void
KTextView::_RecalculateLineBreaks(int32* startLine, int32* endLine)
{
	CALLED();
	...
	*endLine = (*endLine > fLines->NumLines() - 1) ? fLines->NumLines() - 1
	...
	KSTELine* curLine = (*fLines)[lineIndex];
	KSTELine* nextLine = curLine + 1;
...
int32
KTextView::_FindLineBreak(int32 fromOffset, float* _ascent, float* _descent,
	float* inOutWidth)
{
	*_ascent = 0.0;
	*_descent = 0.0;
...
int32
KTextView::_PreviousLineStart(int32 offset)
{
	if (offset <= 0)
		return 0;
...
int32
KTextView::_NextLineEnd(int32 offset)
{
...
int32
KTextView::_PreviousWordBoundary(int32 offset)
{
...
int32
KTextView::_NextWordBoundary(int32 offset)
{
	int32 textLen = fText->Length();
	uint32 charType = _CharClassification(offset);
	while (offset < textLen) {
...
int32
KTextView::_PreviousWordStart(int32 offset)
{
	if (offset <= 1)
		return 0;
...
int32
KTextView::_NextWordEnd(int32 offset)
{
	int32 textLen = fText->Length();
	if (_CharClassification(offset) != CHAR_CLASS_DEFAULT) {
		// skip non-word characters
...
*/
float
KTextView::_TabExpandedStyledWidth(int32 offset, int32 length, float* _ascent,
...
*/
float
KTextView::_StyledWidth(int32 fromOffset, int32 length, float* _ascent,
	float* _descent) const
{
	if (length == 0) {
	...
	while ((numBytes = fStyles->Iterate(fromOffset, length, fInline, &font,
		...
		if (BPrivate::k_gWidthBuffer != NULL) {
			result += BPrivate::k_gWidthBuffer->StringWidth(*fText, fromOffset,
				numBytes, font);
		} else {
#endif
...
float
KTextView::_ActualTabWidth(float location) const
{
...
void
KTextView::_DoInsertText(const char* text, int32 length, int32 offset,
	const k_text_run_array* runs)
{
	_CancelInputMethod();
...
void
KTextView::_DoDeleteText(int32 fromOffset, int32 toOffset)
{
	CALLED();
}
...
void
KTextView::_DrawLine(KView* view, const int32 &lineNum,
...
void
KTextView::_DrawLines(int32 startLine, int32 endLine, int32 startOffset,
	bool erase)
{
	if (!Window())
		return;
	...
	KSTELine* line = (*fLines)[startLine];
...
void
KTextView::_RequestDrawLines(int32 startLine, int32 endLine)
{
	if (!Window())
		return;
	...
	KSTELine* from = (*fLines)[startLine];
	KSTELine* to = endLine == maxLine ? NULL : (*fLines)[endLine + 1];
...
void
KTextView::_DrawCaret(int32 offset, bool visible)
{
	float lineHeight;
...
*/
void
KTextView::_DragCaret(int32 offset)
{
...
bool
KTextView::_PerformMouseUp(BPoint where)
{
	if (fTrackingMouse == NULL)
		return false;
...
bool
KTextView::_PerformMouseMoved(BPoint where, uint32 code)
{
	fWhere = where;
...
*/
void
KTextView::_TrackMouse(BPoint where, const BMessage* message, bool force)
{
	BRegion textRegion;
...
void
KTextView::_TrackDrag(BPoint where)
{
	CALLED();
	if (Bounds().Contains(where))
...
void
KTextView::_InitiateDrag()
{
	BMessage dragMessage(B_MIME_DATA);
	KBitmap* dragBitmap = NULL;
	BPoint bitmapPoint;
...
bool
KTextView::_MessageDropped(BMessage* message, BPoint where, BPoint offset)
{
	ASSERT(message);
	...
	if (message->FindData("text/plain", B_MIME_TYPE, (const void**)&text,
		...
		ssize_t runLength = 0;
		if (fStylable) {
			message->FindData("application/x-vnd.Be-k_text_run_array",
				B_MIME_TYPE, (const void**)&runArray, &runLength);
		}
...
void
KTextView::_PerformAutoScrolling()
{
...
void
KTextView::_UpdateScrollbars()
{
	BRect bounds(Bounds());
	KScrollBar* horizontalScrollBar = ScrollBar(B_HORIZONTAL);
 	KScrollBar* verticalScrollBar = ScrollBar(B_VERTICAL);
...
void
KTextView::_ScrollBy(float horizontal, float vertical)
{
	BRect bounds = Bounds();
...
void
KTextView::_ScrollTo(float x, float y)
{
	BRect bounds = Bounds();
	long viewWidth = bounds.IntegerWidth();
	long viewHeight = bounds.IntegerHeight();
...
void
KTextView::_AutoResize(bool redraw)
{
	if (!fResizable)
		return;
...
//!	Creates a new offscreen KBitmap with an associated KView.
void
KTextView::_NewOffscreen(float padding)
{
	if (fOffscreen != NULL)
		_DeleteOffscreen();
	...
	fOffscreen = new KBitmap(bitmapRect, fColorSpace, true, false);
	if (fOffscreen != NULL && fOffscreen->Lock()) {
		KView* bufferView = new KView(bitmapRect, "drawing view", 0, 0);
...
KTextView::_Activate()
{
	...
	// Create a new offscreen KBitmap
	_NewOffscreen();
...
*/
void
KTextView::_NormalizeFont(BFont* font)
{
	if (font) {
...
void
KTextView::_SetRunArray(int32 startOffset, int32 endOffset,
	const k_text_run_array* runs)
{
	const int32 numStyles = runs->count;
	if (numStyles > 0) {
		const k_text_run* theRun = &runs->runs[0];
...
*/
uint32
KTextView::_CharClassification(int32 offset) const
{
...
*/
int32
KTextView::_NextInitialByte(int32 offset) const
{
	if (offset >= fText->Length())
		return offset;
...
*/
int32
KTextView::_PreviousInitialByte(int32 offset) const
{
	if (offset <= 0)
		return 0;
...
bool
KTextView::_GetProperty(BMessage* specifier, int32 form, const char* property,
	BMessage* reply)
{
	CALLED();
	if (strcmp(property, "selection") == 0) {
		...
		return true;
	} else if (strcmp(property, "k_text_run_array") == 0)
		return false;
...
bool
KTextView::_SetProperty(BMessage* specifier, int32 form, const char* property,
	BMessage* reply)
{
	CALLED();
	if (strcmp(property, "selection") == 0) {
...
bool
KTextView::_CountProperties(BMessage* specifier, int32 form,
...
void
KTextView::_HandleInputMethodChanged(BMessage* message)
{
	// TODO: block input if not editable (Andrew)
	ASSERT(fInline != NULL);
...
*/
void
KTextView::_HandleInputMethodLocationRequest()
{
	ASSERT(fInline != NULL);
...
*/
int32
KTextView::_LineAt(int32 offset) const
{
...
*/
int32
KTextView::_LineAt(const BPoint& point) const
{
...
*/
bool
KTextView::_IsOnEmptyLastLine(int32 offset) const
{
...
void
KTextView::_ApplyStyleRange(int32 fromOffset, int32 toOffset, uint32 mode,
...
float
KTextView::_NullStyleHeight() const
{
	const BFont* font = NULL;
	fStyles->GetNullStyle(&font, NULL);
...
void
KTextView::_ShowContextMenu(BPoint where)
{
	bool isRedo;
	k_undo_state state = UndoState(&isRedo);
	bool isUndo = state != K_UNDO_UNAVAILABLE && !isRedo;
	...
	KPopUpMenu* menu = new KPopUpMenu(B_EMPTY_STRING, false, false);

	KLayoutBuilder::Menu<>(menu)
...
void
KTextView::_FilterDisallowedChars(char* text, ssize_t& length,
...
void
KTextView::_UpdateInsets(const BRect& rect)
{
debug_printf("[KTextView]{_UpdateInsets}\n");
	// do not update insets if SetInsets() was called
	if (fLayoutData->overridden)
	{
	debug_printf("[KTextView]{_UpdateInsets}overridden\n");
		return;
	}
debug_printf("[KTextView]{_UpdateInsets}before Bounds()\n");
	const BRect& bounds = Bounds();
debug_printf("[KTextView]{_UpdateInsets}after Bounds\n");
	...
	debug_printf("[KTextView]{_UpdateInsets}fEditabel || fSelectable\n");
		float hPadding = k_be_control_look->DefaultLabelSpacing();
	debug_printf("[KTextView]{_UpdateInsets}after k_be_control_look\n");
	debug_printf("[KTextView]{_UpdateInsets}hPadding=%f\n",hPadding);
		float hInset = floorf(hPadding / 2.0f);
	debug_printf("[KTextView]{_UpdateInsets}hInset=%f\n",hInset);
		float vInset = 1;
...
debug_printf("[KTextView]{_UpdateInsets}ends\n");
}


// #pragma mark - KTextView::KTextTrackState


KTextView::KTextTrackState::KTextTrackState(BMessenger messenger)
	:
	clickOffset(0),
	shiftDown(false),
	anchor(0),
...
KTextView::KTextTrackState::~KTextTrackState()
{
	delete fRunner;
}
...
void
KTextView::KTextTrackState::SimulateMouseMovement(KTextView* textView)
{
	BPoint where;
	uint32 buttons;
...
extern "C" void
B_IF_GCC_2(InvalidateLayout__9KTextViewb,  _ZN9KTextView16InvalidateLayoutEb)(
	KTextView* view, bool descendants)
{
	perform_data_layout_invalidated data;
