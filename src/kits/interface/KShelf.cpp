...
/*!	KShelf stores replicant views that are dropped onto it */
...
struct k_replicant_data {
	k_replicant_data(BMessage *message, KView *view, KDragger *dragger,
		KDragger::relation relation, unsigned long id);
	k_replicant_data();
	~k_replicant_data();

	static k_replicant_data* Find(BList const *list, BMessage const *msg);
	static k_replicant_data* Find(BList const *list, KView const *view, bool allowZombie);
	static k_replicant_data* Find(BList const *list, unsigned long id);

	static int32 IndexOf(BList const *list, BMessage const *msg);
	static int32 IndexOf(BList const *list, KView const *view, bool allowZombie);
...
class KShelfContainerViewFilter : public BMessageFilter {
	public:
		KShelfContainerViewFilter(KShelf *shelf, KView *view);
...
class KReplicantViewFilter : public BMessageFilter {
	public:
		KReplicantViewFilter(KShelf *shelf, KView *view);
...
using BPrivate::k_replicant_data;
using BPrivate::KReplicantViewFilter;
using BPrivate::KShelfContainerViewFilter;


//	#pragma mark -


/*!	\brief Helper function for KShelf::_AddReplicant()
*/
static status_t
...
{
	int32 i = 0;
	k_replicant_data *item;
	while ((item = (k_replicant_data *)list.ItemAt(i++)) != NULL) {
...
k_replicant_data::k_replicant_data(BMessage *_message, KView *_view, KDragger *_dragger,
	KDragger::relation _relation, unsigned long _id)
	:
	message(_message),
	view(_view),
	dragger(NULL),
...
k_replicant_data::k_replicant_data()
	:
	message(NULL),
	view(NULL),
	dragger(NULL),
	relation(KDragger::TARGET_UNKNOWN),
	id(0),
	error(B_ERROR),
	zombie_view(NULL)
{
...
status_t
k_replicant_data::Archive(BMessage* msg)
{
...
//static
k_replicant_data *
k_replicant_data::Find(BList const *list, BMessage const *msg)
{
	int32 i = 0;
	k_replicant_data *item;
	while ((item = (k_replicant_data*)list->ItemAt(i++)) != NULL) {
...
//static
k_replicant_data *
k_replicant_data::Find(BList const *list, KView const *view, bool allowZombie)
{
	int32 i = 0;
	k_replicant_data *item;
	while ((item = (k_replicant_data*)list->ItemAt(i++)) != NULL) {
...
//static
k_replicant_data *
k_replicant_data::Find(BList const *list, unsigned long id)
{
	int32 i = 0;
	k_replicant_data *item;
	while ((item = (k_replicant_data*)list->ItemAt(i++)) != NULL) {
...
//static
int32
k_replicant_data::IndexOf(BList const *list, BMessage const *msg)
{
	int32 i = 0;
	k_replicant_data *item;
	while ((item = (k_replicant_data*)list->ItemAt(i)) != NULL) {
...
//static
int32
k_replicant_data::IndexOf(BList const *list, KView const *view, bool allowZombie)
{
	int32 i = 0;
	k_replicant_data *item;
	while ((item = (k_replicant_data*)list->ItemAt(i)) != NULL) {
...
//static
int32
k_replicant_data::IndexOf(BList const *list, unsigned long id)
{
	int32 i = 0;
	k_replicant_data *item;
	while ((item = (k_replicant_data*)list->ItemAt(i)) != NULL) {
...
KShelfContainerViewFilter::KShelfContainerViewFilter(KShelf *shelf, KView *view)
...
filter_result
KShelfContainerViewFilter::Filter(BMessage *msg, BHandler **handler)
{
...
filter_result
KShelfContainerViewFilter::_ObjectDropFilter(BMessage *msg, BHandler **_handler)
{
	KView *mouseView = NULL;
	if (*_handler)
		mouseView = dynamic_cast<KView*>(*_handler);
	...
	if (Looper() == looper) {
		KDragger *dragger = NULL;
		if (handler)
			dragger = dynamic_cast<KDragger*>(handler);
		else
			return B_SKIP_MESSAGE;

		BRect rect;
		if (dragger->fRelation == KDragger::TARGET_IS_CHILD)
			rect = dragger->Frame();
		else
		...
		if (dragger->fRelation == KDragger::TARGET_IS_PARENT)
			dragger->fTarget->MoveTo(point);
		else if (dragger->fRelation == KDragger::TARGET_IS_CHILD)
...
KReplicantViewFilter::KReplicantViewFilter(KShelf *shelf, KView *view)
	: BMessageFilter(B_ANY_DELIVERY, B_ANY_SOURCE),
	fShelf(shelf),
	fView(view)
{
...
filter_result
KReplicantViewFilter::Filter(BMessage *message, BHandler **handler)
{
...
KShelf::KShelf(KView *view, bool allowDrags, const char *shelfType)
	: BHandler(shelfType)
{
...
KShelf::KShelf(const entry_ref *ref, KView *view, bool allowDrags,
...
KShelf::KShelf(BDataIO *stream, KView *view, bool allowDrags,
	const char *shelfType)
	: BHandler(shelfType)
{
...
KShelf::~KShelf()
{
	...
	while (fReplicants.CountItems() > 0) {
		k_replicant_data *data = (k_replicant_data *)fReplicants.ItemAt(0);
...
status_t
KShelf::Archive(BMessage *data, bool deep) const
{
	return B_ERROR;
}
...
BArchivable *
KShelf::Instantiate(BMessage *data)
{
	return NULL;
}
...
BHandler *
KShelf::ResolveSpecifier(BMessage *msg, int32 index, BMessage *specifier,
...
status_t
KShelf::GetSupportedSuites(BMessage *message)
{
	status_t err;
...
status_t
KShelf::Perform(perform_code d, void *arg)
{
...
void
KShelf::SetAllowsDragging(bool state)
{
...
void
KShelf::SetAllowsZombies(bool state)
{
	fAllowZombies = state;
}
...
void
KShelf::SetDisplaysZombies(bool state)
{
...
void
KShelf::SetTypeEnforced(bool state)
{
	fTypeEnforced = state;
}
...
status_t
KShelf::SetSaveLocation(BDataIO *data_io)
{
	fDirty = true;
...
status_t
KShelf::SetSaveLocation(const entry_ref *ref)
{
	fDirty = true;
...
BDataIO *
KShelf::SaveLocation(entry_ref *ref) const
{
	if (fStream) {
		if (ref)
			*ref = entry_ref();
...
status_t
KShelf::AddReplicant(BMessage *data, BPoint location)
{
...
status_t
KShelf::DeleteReplicant(KView *replicant)
{
	int32 index = k_replicant_data::IndexOf(&fReplicants, replicant, true);

	k_replicant_data *item = (k_replicant_data*)fReplicants.ItemAt(index);
	if (item == NULL)
		return B_BAD_VALUE;
...
status_t
KShelf::DeleteReplicant(BMessage *data)
{
	int32 index = k_replicant_data::IndexOf(&fReplicants, data);
...
status_t
KShelf::DeleteReplicant(int32 index)
{
	k_replicant_data *item = (k_replicant_data*)fReplicants.ItemAt(index);
	if (!item)
		return B_BAD_INDEX;
...
BMessage *
KShelf::ReplicantAt(int32 index, KView **_view, uint32 *_uniqueID,
...
int32
KShelf::IndexOf(const KView* replicantView) const
{
	return k_replicant_data::IndexOf(&fReplicants, replicantView, false);
}


int32
KShelf::IndexOf(const BMessage *archive) const
{
	return k_replicant_data::IndexOf(&fReplicants, archive);
}


int32
KShelf::IndexOf(uint32 id) const
{
	return k_replicant_data::IndexOf(&fReplicants, id);
}


bool
KShelf::CanAcceptReplicantMessage(BMessage*) const
{
	return true;
}
...
bool
KShelf::CanAcceptReplicantView(BRect, KView*, BMessage*) const
{
	return true;
}
...
BPoint
KShelf::AdjustReplicantBy(BRect, BMessage*) const
{
	return B_ORIGIN;
}
...
void
KShelf::ReplicantDeleted(int32 index, const BMessage *archive,
...
extern "C" void
_ReservedShelf1__6KShelfFv(KShelf *const, int32, const BMessage*, const KView*)
{
	// is not contained in BeOS R5's libbe, so we leave it empty
}


void KShelf::_ReservedShelf2() {}
void KShelf::_ReservedShelf3() {}
void KShelf::_ReservedShelf4() {}
void KShelf::_ReservedShelf5() {}
void KShelf::_ReservedShelf6() {}
void KShelf::_ReservedShelf7() {}
void KShelf::_ReservedShelf8() {}


KShelf::KShelf(const KShelf&)
{
}


KShelf &
KShelf::operator=(const KShelf &)
{
	return *this;
}
...
status_t
KShelf::_Archive(BMessage *data) const
{
	...
	for (int32 i = 0; i < fReplicants.CountItems(); i++) {
		if (((k_replicant_data *)fReplicants.ItemAt(i))->Archive(&archive) == B_OK)
...
void
KShelf::_InitData(BEntry *entry, BDataIO *stream, KView *view,
	bool allowDrags)
{
	fContainerView = view;
	fStream = NULL;
	...
	fFilter = new KShelfContainerViewFilter(this, fContainerView);
...
status_t
KShelf::_DeleteReplicant(k_replicant_data* item)
{
	...
	int32 index = k_replicant_data::IndexOf(&fReplicants, item->message);

	ReplicantDeleted(index, item->message, view);

	fReplicants.RemoveItem(item);

	if (item->relation == KDragger::TARGET_IS_PARENT
		|| item->relation == KDragger::TARGET_IS_SIBLING) {
		delete view;
	}
	if (item->relation == KDragger::TARGET_IS_CHILD
		|| item->relation == KDragger::TARGET_IS_SIBLING) {
		delete item->dragger;
	}
...
status_t
KShelf::_AddReplicant(BMessage *data, BPoint *location, uint32 uniqueID)
{
	...
	if (fTypeEnforced) {
		...
		if (data->FindString("shelf_type", &shelfType) == B_OK
			...
			if (Name() && strcmp(shelfType, Name()) != 0) {
				printf("Replicant was rejected by KShelf: The KShelf's type and the Replicant's type don't match.");
				return send_reply(data, B_ERROR, uniqueID);
			} else {
				printf("Replicant was rejected by KShelf: Replicant indicated a <type> (%s), but the shelf does not.", shelfType);
				return send_reply(data, B_ERROR, uniqueID);
			}
		} else {
			printf("Replicant was rejected by KShelf: Replicant did not have a <type>");
			return send_reply(data, B_ERROR, uniqueID);
		}
	}
	...
	if (!CanAcceptReplicantMessage(data)) {
		printf("Replicant was rejected by KShelf::CanAcceptReplicantMessage()");
		return send_reply(data, B_ERROR, uniqueID);
	}
	...
	if (archivable != NULL) {
		view = dynamic_cast<KView*>(archivable);
	...
	KDragger* dragger = NULL;
	KView* replicant = NULL;
	KDragger::relation relation = KDragger::TARGET_UNKNOWN;
	_KZombieReplicantView_* zombie = NULL;
	if (view != NULL) {
	...
	k_replicant_data *item = new k_replicant_data(data, replicant, dragger,
		relation, uniqueID);
...
KView *
KShelf::_GetReplicant(BMessage *data, KView *view, const BPoint &point,
	KDragger *&dragger, KDragger::relation &relation)
{
	...
	if (!CanAcceptReplicantView(frame, replicant, data)) {
		...
		if (relation == KDragger::TARGET_IS_PARENT
			|| relation == KDragger::TARGET_IS_SIBLING) {
			delete replicant;
		}
		if (relation == KDragger::TARGET_IS_CHILD
			|| relation == KDragger::TARGET_IS_SIBLING) {
			delete dragger;
		}
		return NULL;
	...
	if (relation == KDragger::TARGET_IS_SIBLING
		|| relation == KDragger::TARGET_IS_CHILD)
		fContainerView->AddChild(dragger);

	if (relation != KDragger::TARGET_IS_CHILD)
		fContainerView->AddChild(replicant);

	replicant->AddFilter(new KReplicantViewFilter(this, replicant));
...
/* static */
void
KShelf::_GetReplicantData(BMessage *data, KView *view, KView *&replicant,
	KDragger *&dragger, KDragger::relation &relation)
{
	...
	if (data->FindMessage("__widget", &widget) == B_OK) {
		...
		replicant = view;
		dragger = dynamic_cast<KDragger*>(_InstantiateObject(&widget, &draggerImage));
		// Replicant is a sibling, or unknown, if there isn't a dragger
		if (dragger != NULL)
			relation = KDragger::TARGET_IS_SIBLING;

	} else if ((dragger = dynamic_cast<KDragger*>(view)) != NULL) {
		...
		relation = KDragger::TARGET_IS_PARENT;
		replicant = view;
		dragger = dynamic_cast<KDragger *>(replicant->FindView("_dragger_"));
			// can be NULL, the replicant could not have a dragger at all
	}
}


_KZombieReplicantView_ *
KShelf::_CreateZombie(BMessage *data, KDragger *&dragger)
{
	...
	if (data->WasDropped()) {
		...
		zombie = new _KZombieReplicantView_(frame, B_ERROR);

		frame.OffsetTo(B_ORIGIN);

		dragger = new KDragger(frame, zombie);
		dragger->_SetShelf(this);
		dragger->_SetZombied(true);

		zombie->AddChild(dragger);
		zombie->SetArchive(data);
		zombie->AddFilter(new KReplicantViewFilter(this, zombie));
...
status_t
KShelf::_GetProperty(BMessage *msg, BMessage *reply)
{
	uint32 ID;
...
/* static */
BArchivable *
KShelf::_InstantiateObject(BMessage *archive, image_id *image)
{
