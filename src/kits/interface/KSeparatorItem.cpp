...
KSeparatorItem::KSeparatorItem()
	:
	KMenuItem("", NULL)
{
	KMenuItem::SetEnabled(false);
}


KSeparatorItem::KSeparatorItem(BMessage* data)
	:
	KMenuItem(data)
{
	KMenuItem::SetEnabled(false);
...
status_t
KSeparatorItem::Archive(BMessage* data, bool deep) const
{
	return KMenuItem::Archive(data, deep);
}


BArchivable*
KSeparatorItem::Instantiate(BMessage* data)
{
	if (validate_instantiation(data, "KSeparatorItem"))
		return new KSeparatorItem(data);
...
void
KSeparatorItem::SetEnabled(bool enable)
{
...
void
KSeparatorItem::GetContentSize(float* _width, float* _height)
{
	if (Menu() != NULL && Menu()->Layout() == K_ITEMS_IN_ROW) {
		if (_width != NULL)
			*_width = 2.0;
...
void KSeparatorItem::_ReservedSeparatorItem1() {}
void KSeparatorItem::_ReservedSeparatorItem2() {}


KSeparatorItem &
KSeparatorItem::operator=(const KSeparatorItem &)
{
	return *this;
}
