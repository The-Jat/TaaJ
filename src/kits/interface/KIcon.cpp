...
status_t
KIcon::SetTo(const KBitmap* bitmap, uint32 flags)
{
	if (!bitmap->IsValid())
		return B_BAD_VALUE;
	...
	if ((flags & (B_TRIM_ICON_BITMAP | B_TRIM_ICON_BITMAP_KEEP_ASPECT)) != 0
		...
		if (bitmap->ColorSpace() == B_RGBA32) {
			...
			KBitmap* rgb32Bitmap = _ConvertToRGB32(bitmap, true);
			if (rgb32Bitmap != NULL) {
	...
	if (canUseForMakeBitmaps) {
		...
		if (KBitmap* rgb32Bitmap = _ConvertToRGB32(bitmap, true)) {
...
bool
KIcon::SetBitmap(KBitmap* bitmap, uint32 which)
{
...
KBitmap*
KIcon::Bitmap(uint32 which) const
{
...
KBitmap*
KIcon::CreateBitmap(const BRect& bounds, color_space colorSpace, uint32 which)
{
	KBitmap* bitmap = new(std::nothrow) KBitmap(bounds, colorSpace);
...
status_t
KIcon::SetExternalBitmap(const KBitmap* bitmap, uint32 which, uint32 flags)
{
	KBitmap* ourBitmap = NULL;
	if (bitmap != NULL) {
		if (!bitmap->IsValid())
			return B_BAD_VALUE;

		if ((flags & B_KEEP_ICON_BITMAP) != 0) {
			ourBitmap = const_cast<KBitmap*>(bitmap);
		} else {
...
KBitmap*
KIcon::CopyBitmap(const KBitmap& bitmapToClone, uint32 which)
{
	KBitmap* bitmap = new(std::nothrow) KBitmap(bitmapToClone);
	if (bitmap == NULL || !bitmap->IsValid() || !SetBitmap(bitmap, which)) {
		delete bitmap;
		return NULL;
	}
...
/*static*/ status_t
KIcon::UpdateIcon(const KBitmap* bitmap, uint32 flags, KIcon*& _icon)
{
	if (bitmap == NULL) {
		delete _icon;
		_icon = NULL;
	...
	KIcon* icon = new(std::nothrow) KIcon;
	if (icon == NULL)
		return B_NO_MEMORY;
...
/*static*/ status_t
KIcon::SetIconBitmap(const KBitmap* bitmap, uint32 which, uint32 flags,
	KIcon*& _icon)
{
	bool newIcon = false;
	if (_icon == NULL) {
...
/*static*/ KBitmap*
KIcon::_ConvertToRGB32(const KBitmap* bitmap, bool noAppServerLink)
{
	KBitmap* rgb32Bitmap = new(std::nothrow) KBitmap(bitmap->Bounds(),
...
/*static*/ status_t
KIcon::_TrimBitmap(const KBitmap* bitmap, bool keepAspect,
	KBitmap*& _trimmedBitmap)
{
	...
	KBitmap* trimmedBitmap = new(std::nothrow) KBitmap(
...
status_t
KIcon::_MakeBitmaps(const KBitmap* bitmap, uint32 flags)
{
	// make our own versions of the bitmap
	BRect b(bitmap->Bounds());

	color_space format = bitmap->ColorSpace();
	KBitmap* normalBitmap = CreateBitmap(b, format, B_INACTIVE_ICON_BITMAP);
	if (normalBitmap == NULL)
		return B_NO_MEMORY;

	KBitmap* disabledBitmap = NULL;
	...
	KBitmap* disabledClickedBitmap = NULL;
