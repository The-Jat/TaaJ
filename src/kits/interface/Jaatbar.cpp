...
// TODO: in case the KDeskbar methods are called from a Deskbar add-on,
...
status_t
k_get_deskbar_frame(BRect* frame)
{
	BMessenger deskbar(k_kDeskbarSignature);
...
//	#pragma mark - KDeskbar


KDeskbar::KDeskbar()
	:
	fMessenger(new BMessenger(k_kDeskbarSignature))
{
}
...
BRect
KDeskbar::Frame() const
{
	BRect frame(0.0, 0.0, 0.0, 0.0);
	k_get_deskbar_frame(&frame);
...
k_deskbar_location
KDeskbar::Location(bool* _isExpanded) const
{
	k_deskbar_location location = B_DESKBAR_RIGHT_TOP;
	...
	if (fMessenger->SendMessage(&request, &reply) == B_OK) {
		...
		if (reply.FindInt32("location", &value) == B_OK)
			location = static_cast<k_deskbar_location>(value);
...
status_t
KDeskbar::SetLocation(k_deskbar_location location, bool expanded)
{
...
status_t
KDeskbar::SetAlwaysOnTop(bool alwaysOnTop)
{
...
status_t
KDeskbar::SetAutoRaise(bool autoRaise)
{
...
status_t
KDeskbar::SetAutoHide(bool autoHide)
{
...
status_t
KDeskbar::GetItemInfo(int32 id, const char** _name) const
{
	if (_name == NULL)
		return B_BAD_VALUE;
...
status_t
KDeskbar::GetItemInfo(const char* name, int32* _id) const
{
	if (name == NULL)
		return B_BAD_VALUE;
...
bool
KDeskbar::HasItem(int32 id) const
{
...
bool
KDeskbar::HasItem(const char* name) const
{
	BMessage request(kMsgHasItem);
	request.AddString("name", name);
...
status_t
KDeskbar::AddItem(KView* view, int32* _id)
{
	BMessage archive;
...
status_t
KDeskbar::AddItem(entry_ref* addon, int32* _id)
{
...
status_t
KDeskbar::RemoveItem(const char* name)
{
	BMessage request(kMsgRemoveItem);
	request.AddString("name", name);
