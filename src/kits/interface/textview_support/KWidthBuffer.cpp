...
KWidthBuffer* k_gWidthBuffer = NULL;
...
*/
float
KWidthBuffer::StringWidth(const char* inText, int32 fromOffset, int32 length,
	const BFont* inStyle)
{
...
/*! \brief Returns how much room is required to draw a string in the font.
	\param inBuffer The KTextGapBuffer to be examined.
	\param fromOffset The offset in the KTextGapBuffer where to begin the
	examination.
	\param lenght The amount of bytes to be examined.
	\param inStyle The font.
	\return The space (in pixels) required to draw the given string.
*/
float
KWidthBuffer::StringWidth(KTextGapBuffer &inBuffer, int32 fromOffset,
...
*/
bool
KWidthBuffer::FindTable(const BFont* inStyle, int32* outIndex)
{
	if (inStyle == NULL)
		return false;
...
*/
int32
KWidthBuffer::InsertTable(const BFont* font)
{
	_width_table_ table;
...
*/
bool
KWidthBuffer::GetEscapement(uint32 value, int32 index, float* escapement)
{
...
*/
float
KWidthBuffer::HashEscapements(const char* inText, int32 numChars, int32 textLen,
...
_KWidthBuffer_* gCompatibilityWidthBuffer = NULL;

extern "C"
float
StringWidth__14_KWidthBuffer_PCcllPC5BFont(_KWidthBuffer_* widthBuffer,
	const char* inText, int32 fromOffset, int32 length, const BFont* inStyle)
{
	return BPrivate::k_gWidthBuffer->StringWidth(inText, fromOffset, length,
		inStyle);
}
