...
#define JS_HIDE_APPLICATION '_AHD'
...
void
_set_menu_sem_(JSWindow* jwindow, sem_id sem)
{
	if (jwindow != NULL)
		jwindow->fMenuSem = sem;
}
...
JSWindow::unpack_cookie::unpack_cookie()
	:
...
//	#pragma mark - JSWindow::Shortcut


JSWindow::Shortcut::Shortcut(uint32 key, uint32 modifiers, BMenuItem* item)
	:
	fKey(PrepareKey(key)),
...
JSWindow::Shortcut::Shortcut(uint32 key, uint32 modifiers, BMessage* message,
	BHandler* target)
	:
	fKey(PrepareKey(key)),
	fModifiers(PrepareModifiers(modifiers)),
...
bool
JSWindow::Shortcut::Matches(uint32 key, uint32 modifiers) const
{
...
/*static*/
uint32
JSWindow::Shortcut::AllowedModifiers()
{
...
/*static*/
uint32
JSWindow::Shortcut::PrepareModifiers(uint32 modifiers)
{
...
/*static*/
uint32
JSWindow::Shortcut::PrepareKey(uint32 key)
{
	return BUnicodeChar::ToLower(key);
}


//	#pragma mark - JSWindow


JSWindow::JSWindow(BRect frame, const char* title, jwindow_type type,
...
JSWindow::JSWindow(BRect frame, const char* title, jwindow_look look,
		jwindow_feel feel, uint32 flags, uint32 workspace)
	:
	BLooper(title, B_DISPLAY_PRIORITY)
{
	_InitData(frame, title, look, feel, flags, workspace);
...
JSWindow::JSWindow(BMessage* data)
	:
	BLooper(data)
{
	...
	if (data->FindInt32("_type", (int32*)&type) == B_OK)
		_DecomposeType((jwindow_type)type, &fLook, &fFeel);
...
JSWindow::JSWindow(BRect frame, int32 bitmapToken)
	:
	BLooper("offscreen bitmap")
{
	_DecomposeType(JS_UNTYPED_WINDOW, &fLook, &fFeel);
...
JSWindow::~JSWindow()
{
	...
	// The JSWindow is locked when the destructor is called,
	// we need to unlock because the menubar thread tries
	// to post a message, which will deadlock otherwise.
	// TODO: I replaced Unlock() with UnlockFully() because the jwindow
...
BArchivable*
JSWindow::Instantiate(BMessage* data)
{
	if (!validate_instantiation(data, "JSWindow"))
		return NULL;

	return new(std::nothrow) JSWindow(data);
}


status_t
JSWindow::Archive(BMessage* data, bool deep) const
{
...
JSWindow::Quit()
{
	...
	if (fFlags & JS_QUIT_ON_WINDOW_CLOSE)
...
void
JSWindow::AddChild(BView* child, BView* before)
{
...
void
JSWindow::AddChild(BLayoutItem* child)
{
	BAutolock locker(this);
	if (locker.IsLocked())
...
bool
JSWindow::RemoveChild(BView* child)
{
	BAutolock locker(this);
	if (!locker.IsLocked())
		return false;
...
int32
JSWindow::CountChildren() const
{
	BAutolock locker(const_cast<JSWindow*>(this));
	if (!locker.IsLocked())
		return 0;
...
BView*
JSWindow::ChildAt(int32 index) const
{
	BAutolock locker(const_cast<JSWindow*>(this));
	if (!locker.IsLocked())
		return NULL;
...
void
JSWindow::Minimize(bool minimize)
{
...
status_t
JSWindow::SendBehind(const JSWindow* jwindow)
{
	if (!Lock())
		return B_ERROR;

	fLink->StartMessage(AS_SEND_BEHIND);
	fLink->Attach<int32>(jwindow != NULL ? _get_object_token_(jwindow) : -1);
...
void
JSWindow::Flush() const
{
	if (const_cast<JSWindow*>(this)->Lock()) {
		fLink->Flush();
		const_cast<JSWindow*>(this)->Unlock();
	}
}
...
bool
JSWindow::InViewTransaction() const
{
	BAutolock locker(const_cast<JSWindow*>(this));
...
void
JSWindow::MessageReceived(BMessage* message)
{
	if (!message->HasSpecifiers()) {
		...
		if (message->what == (int32)kMsgAppServerRestarted) {
			...
			// Redirect our link to the new jwindow connection
...
void
JSWindow::DispatchMessage(BMessage* message, BHandler* target)
{
	if (message == NULL)
		return;
	...
	switch (message->what) {
		...
		case _MINIMIZE_:
			...
			if ((Flags() & JS_NOT_MINIMIZABLE) == 0)
				Minimize(true);
			break;

		case _ZOOM_:
			// Used by the zoom shortcut
			if ((Flags() & JS_NOT_ZOOMABLE) == 0)
				Zoom();
			break;
...
// TODO: When a jwindow is resized programmatically,
...
// call hook function anyways
// TODO: When a jwindow is moved programmatically,
// it receives this message, and maybe it is wise to
// keep the asynchronous nature of this process to
// not risk breaking any apps.
FrameMoved(origin);
		...
		case B_WINDOW_ACTIVATED:
			...
			if (active != fActive) {
				...
				// views attached to this jwindow.
				fTopView->_Activate(active);

				// we notify the input server if we are gaining or losing focus
				// from a view which has the B_INPUT_METHOD_AWARE on a jwindow
				// activation
				if (!active)
					break;
...
//bigtime_t drawTime = 0;
			STRACE(("info:JSWindow handling _UPDATE_.\n"));
			...
			if (fLink->FlushWithReply(code) == B_OK
				&& code == B_OK) {
				// read current jwindow position and size first,
				...
				if (origin != fFrame.LeftTop()) {
					...
					// B_WINDOW_MOVED case...
					//printf("jwindow position was not up to date\n");
					fFrame.OffsetTo(origin);
					FrameMoved(origin);
				}
				if (width != fFrame.Width() || height != fFrame.Height()) {
					...
					//printf("jwindow size was not up to date\n");
					fFrame.right = fFrame.left + width;
					fFrame.bottom = fFrame.top + height;
...
//printf("JSWindow(%s) - UPDATE took %lld usecs\n", Title(), system_time() - now);
			break;
		}
...
void
JSWindow::FrameMoved(BPoint newPosition)
{
	// does nothing
	// Hook function
}
...
void
JSWindow::FrameResized(float newWidth, float newHeight)
{
	// does nothing
	// Hook function
}
...
void
JSWindow::WorkspacesChanged(uint32 oldWorkspaces, uint32 newWorkspaces)
{
	// does nothing
	// Hook function
}
...
void
JSWindow::WorkspaceActivated(int32 workspace, bool state)
{
	// does nothing
	// Hook function
}
...
void
JSWindow::SetSizeLimits(float minWidth, float maxWidth,
	float minHeight, float maxHeight)
{
	...
	if (fLink->FlushWithReply(code) == B_OK
		...
		// the server side (the jwindow frame could have
		// been changed, too)
		fLink->Read<BRect>(&fFrame);
		fLink->Read<float>(&fMinWidth);
		fLink->Read<float>(&fMaxWidth);
		...
		_AdoptResize();
			...
			//		the size limits, and hence, the size of the jwindow
	}
	Unlock();
}
...
void
JSWindow::GetSizeLimits(float* _minWidth, float* _maxWidth, float* _minHeight,
	float* _maxHeight)
{
...
void
JSWindow::UpdateSizeLimits()
{
	BAutolock locker(this);

	if ((fFlags & B_AUTO_UPDATE_SIZE_LIMITS) != 0) {
		// Get min/max constraints of the top view and enforce jwindow
...
status_t
JSWindow::SetDecoratorSettings(const BMessage& settings)
{
...
status_t
JSWindow::GetDecoratorSettings(BMessage* settings) const
{
...
void
JSWindow::SetZoomLimits(float maxWidth, float maxHeight)
{
	// TODO: What about locking?!?
...
void
JSWindow::Zoom(BPoint origin, float width, float height)
{
...
JSWindow::Zoom()
{
	...
	// TODO: Broken for tab on left side jwindows...
	float borderWidth;
	float tabHeight;
	...
	if (fPreviousFrame.IsValid()
		...
		// -> makes it easier on the user to get a jwindow back into place
...
void
JSWindow::ScreenChanged(BRect screenSize, color_space depth)
{
	// Hook function
}
...
void
JSWindow::SetPulseRate(bigtime_t rate)
{
	// TODO: What about locking?!?
	if (rate < 0
...
void
JSWindow::AddShortcut(uint32 key, uint32 modifiers, BMenuItem* item)
{
...
void
JSWindow::AddShortcut(uint32 key, uint32 modifiers, BMessage* message)
{
...
bool
JSWindow::HasShortcut(uint32 key, uint32 modifiers)
{
...
void
JSWindow::RemoveShortcut(uint32 key, uint32 modifiers)
{
...
void
JSWindow::SetDefaultButton(BButton* button)
{
	// TODO: What about locking?!?
...
void
JSWindow::UpdateIfNeeded()
{
	// works only from the jwindow thread
	...
	while (true) {
		...
		JSWindow::DispatchMessage(message, this);
		delete message;
	}
...
BView*
JSWindow::FindView(const char* viewName) const
{
	BAutolock locker(const_cast<JSWindow*>(this));
	if (!locker.IsLocked())
		return NULL;
...
BView*
JSWindow::FindView(BPoint point) const
{
	BAutolock locker(const_cast<JSWindow*>(this));
	if (!locker.IsLocked())
		return NULL;

	// point is assumed to be in jwindow coordinates,
	// fTopView has same bounds as jwindow
	return _FindView(fTopView, point);
}


BView*
JSWindow::CurrentFocus() const
{
	return fFocus;
}
...
JSWindow::Activate(bool active)
{
	...
	if (!IsHidden()) {
		fMinimized = false;
			// activating a jwindow will also unminimize it
...
void
JSWindow::WindowActivated(bool focus)
{
	// hook function
	// does nothing
}
...
void
JSWindow::ConvertToScreen(BPoint* point) const
{
	point->x += fFrame.left;
	point->y += fFrame.top;
}
...
void
JSWindow::ConvertFromScreen(BPoint* point) const
{
	point->x -= fFrame.left;
	point->y -= fFrame.top;
}
...
void
JSWindow::ConvertToScreen(BRect* rect) const
{
...
void
JSWindow::ConvertFromScreen(BRect* rect) const
{
...
JSWindow::DecoratorFrame() const
{
	...
	if (GetDecoratorSettings(&settings) == B_OK) {
		...
		// probably no-border jwindow look
		if (fLook == JS_NO_BORDER_WINDOW_LOOK)
			borderWidth = 0.f;
		else if (fLook == JS_BORDERED_WINDOW_LOOK)
			borderWidth = 1.f;
...
void
JSWindow::SetTitle(const char* title)
{
	if (title == NULL)
		title = "";
...
void
JSWindow::SetKeyMenuBar(BMenuBar* bar)
{
	fKeyMenuBar = bar;
}
...
bool
JSWindow::IsModal() const
{
	return fFeel == JS_MODAL_SUBSET_WINDOW_FEEL
		|| fFeel == JS_MODAL_APP_WINDOW_FEEL
		|| fFeel == JS_MODAL_ALL_WINDOW_FEEL
		|| fFeel == kMenuWindowFeel;
}


bool
JSWindow::IsFloating() const
{
	return fFeel == JS_FLOATING_SUBSET_WINDOW_FEEL
		|| fFeel == JS_FLOATING_APP_WINDOW_FEEL
		|| fFeel == JS_FLOATING_ALL_WINDOW_FEEL;
}


status_t
JSWindow::AddToSubset(JSWindow* jwindow)
{
	if (jwindow == NULL || jwindow->Feel() != JS_NORMAL_WINDOW_FEEL
		|| (fFeel != JS_MODAL_SUBSET_WINDOW_FEEL
			&& fFeel != JS_FLOATING_SUBSET_WINDOW_FEEL))
		return B_BAD_VALUE;
	...
	fLink->Attach<int32>(_get_object_token_(jwindow));
	fLink->FlushWithReply(status);
...
status_t
JSWindow::RemoveFromSubset(JSWindow* jwindow)
{
	if (jwindow == NULL || jwindow->Feel() != JS_NORMAL_WINDOW_FEEL
		|| (fFeel != JS_MODAL_SUBSET_WINDOW_FEEL
			&& fFeel != JS_FLOATING_SUBSET_WINDOW_FEEL))
...
status_t
JSWindow::Perform(perform_code code, void* _data)
{
	switch (code) {
		case PERFORM_CODE_SET_LAYOUT:
		{
			perform_data_set_layout* data = (perform_data_set_layout*)_data;
			JSWindow::SetLayout(data->layout);
			return B_OK;
}
	}
...
status_t
JSWindow::SetType(jwindow_type type)
{
	jwindow_look look;
	jwindow_feel feel;
	_DecomposeType(type, &look, &feel);
...
status_t
JSWindow::SetLook(jwindow_look look)
{
	BAutolock locker(this);
	if (!locker.IsLocked())
		return B_BAD_VALUE;
	...
	// TODO: this could have changed the jwindow size, and thus, we
...
status_t
JSWindow::SetFeel(jwindow_feel feel)
{
	BAutolock locker(this);
	if (!locker.IsLocked())
		return B_BAD_VALUE;
...
status_t
JSWindow::SetFlags(uint32 flags)
{
	BAutolock locker(this);
	if (!locker.IsLocked())
		return B_BAD_VALUE;
...
status_t
JSWindow::GetWindowAlignment(jwindow_alignment* mode,
	int32* h, int32* hOffset, int32* width, int32* widthOffset,
...
void
JSWindow::SetWorkspaces(uint32 workspaces)
{
	// TODO: don't forget about Tracker's background jwindow.
	if (fFeel != JS_NORMAL_WINDOW_FEEL)
		return;
...
BView*
JSWindow::LastMouseMovedView() const
{
...
void
JSWindow::MoveBy(float dx, float dy)
{
...
void
JSWindow::MoveTo(float x, float y)
{
	if (!Lock())
		return;
...
void
JSWindow::ResizeBy(float dx, float dy)
{
	if (Lock()) {
...
void
JSWindow::ResizeTo(float width, float height)
{
	if (!Lock())
		return;
...
void
JSWindow::CenterIn(const BRect& rect)
{
	BAutolock locker(this);
...
void
JSWindow::CenterOnScreen()
{
	CenterIn(BScreen(this).Frame());
}


// Centers the jwindow on the screen with the passed in id.
void
JSWindow::CenterOnScreen(screen_id id)
{
...
void
JSWindow::MoveOnScreen(uint32 flags)
{
	// Set size limits now if needed
	UpdateSizeLimits();
	...
	if ((flags & JS_DO_NOT_RESIZE_TO_FIT) == 0) {
		// Make sure the jwindow fits on the screen
	...
	if (((flags & JS_MOVE_IF_PARTIALLY_OFFSCREEN) == 0
	...
	// Move such that the upper left corner, and most of the jwindow
	// will be visible.
...
JSWindow::Show()
{
	...
	if (!runCalled) {
		...
		// looper. NOTE: The jwindow is still locked if it has not been
...
void
JSWindow::SetLayout(BLayout* layout)
{
...
void
JSWindow::InvalidateLayout(bool descendants)
{
...
bool
JSWindow::IsOffscreenWindow() const
{
	return fOffscreen;
}
...
status_t
JSWindow::GetSupportedSuites(BMessage* data)
{
	if (data == NULL)
		return B_BAD_VALUE;

	status_t status = data->AddString("suites", "suite/vnd.Be-jwindow");
	if (status == B_OK) {
...
BHandler*
JSWindow::ResolveSpecifier(BMessage* message, int32 index, BMessage* specifier,
		...
		if (strcmp(property, "View") == 0) {
			...
			if (fKeyMenuBar) {
				...
				replyMsg.AddString("message",
					"This jwindow doesn't have a main MenuBar");
				message->SendReply(&replyMsg);
				return NULL;
			}
		} else
...
void
JSWindow::_InitData(BRect frame, const char* title, jwindow_look look,
	jwindow_feel feel, uint32 flags,	uint32 workspace, int32 bitmapToken)
{
	STRACE(("JSWindow::InitData()\n"));
	...
	fFeel = feel;
	fLook = look;
	fFlags = flags | JS_ASYNCHRONOUS_CONTROLS;
	...
	// It is only installed for non-modal jwindows, though.
	fNoQuitShortcut = IsModal();

	if ((fFlags & JS_NOT_CLOSABLE) == 0 && !IsModal()) {
		// Modal jwindows default to non-closable, but you can add the
	...
	AddShortcut('H', B_COMMAND_KEY | B_CONTROL_KEY,
		new BMessage(JS_HIDE_APPLICATION), NULL);
	...
	// Create the server-side jwindow

	port_id receivePort = create_port(B_LOOPER_PORT_DEFAULT_CAPACITY,
		"w<app_server");
	if (receivePort < B_OK) {
		// TODO: huh?
		debugger("Could not create JSWindow's receive port, used for "
				 "interacting with the app_server!");
		delete this;
		return;
	}

	STRACE(("JSWindow::InitData(): contacting app_server...\n"));

	// let app_server know that a jwindow has been created.
...
void
JSWindow::_SetName(const char* title)
{
	if (title == NULL)
		title = "";

	// we will change JSWindow's thread name to "w>jwindow title"
...
//!	Reads all pending messages from the jwindow port and put them into the queue.
void
JSWindow::_DequeueAll()
{
...
*/
void
JSWindow::task_looper()
{
	STRACE(("info: JSWindow::task_looper() started.\n"));
	...
	if (IsLocked())
		debugger("jwindow must not be locked!");
...
jwindow_type
JSWindow::_ComposeType(jwindow_look look, jwindow_feel feel) const
{
	switch (feel) {
		case JS_NORMAL_WINDOW_FEEL:
			switch (look) {
				case JS_TITLED_WINDOW_LOOK:
					return JS_TITLED_WINDOW;

				case JS_DOCUMENT_WINDOW_LOOK:
					return JS_DOCUMENT_WINDOW;

				case JS_BORDERED_WINDOW_LOOK:
					return JS_BORDERED_WINDOW;

				default:
					return JS_UNTYPED_WINDOW;
			}
			break;

		case JS_MODAL_APP_WINDOW_FEEL:
			if (look == JS_MODAL_WINDOW_LOOK)
				return JS_MODAL_WINDOW;
			break;

		case JS_FLOATING_APP_WINDOW_FEEL:
			if (look == JS_FLOATING_WINDOW_LOOK)
				return JS_FLOATING_WINDOW;
			break;
...
void
JSWindow::_DecomposeType(jwindow_type type, jwindow_look* _look,
	jwindow_feel* _feel) const
{
	switch (type) {
		case JS_DOCUMENT_WINDOW:
			*_look = JS_DOCUMENT_WINDOW_LOOK;
			*_feel = JS_NORMAL_WINDOW_FEEL;
			break;

		case JS_MODAL_WINDOW:
			*_look = JS_MODAL_WINDOW_LOOK;
			*_feel = JS_MODAL_APP_WINDOW_FEEL;
			break;

		case JS_FLOATING_WINDOW:
			*_look = JS_FLOATING_WINDOW_LOOK;
			*_feel = JS_FLOATING_APP_WINDOW_FEEL;
			break;

		case JS_BORDERED_WINDOW:
			*_look = JS_BORDERED_WINDOW_LOOK;
			*_feel = JS_NORMAL_WINDOW_FEEL;
			break;

		case JS_TITLED_WINDOW:
		case JS_UNTYPED_WINDOW:
		default:
			*_look = JS_TITLED_WINDOW_LOOK;
			*_feel = JS_NORMAL_WINDOW_FEEL;
			break;
	}
}
...
JSWindow::_CreateTopView()
{
	...
	// set fTopView's owner, add it to jwindow's eligible handler list
	// and also set its next handler to be this jwindow.
...
/*!
	Resizes the top view to match the jwindow size. This will also
	adapt the size of all its child views as needed.
	This method has to be called whenever the frame of the jwindow
	changes.
*/
void
JSWindow::_AdoptResize()
{
...
void
JSWindow::_SetFocus(BView* focusView, bool notifyInputServer)
{
	if (fFocus == focusView)
		return;
...
*/
BHandler*
JSWindow::_DetermineTarget(BMessage* message, BHandler* target)
{
	if (target == NULL)
		target = this;
...
*/
bool
JSWindow::_IsFocusMessage(BMessage* message)
{
...
*/
bool
JSWindow::_UnpackMessage(unpack_cookie& cookie, BMessage** _message,
...
/*!	Some messages don't get to the jwindow in a shape an application should see.
	This method is supposed to give a message the last grinding before
	it's acceptable for the receiving application.
*/
void
JSWindow::_SanitizeMessage(BMessage* message, BHandler* target, bool usePreferred)
{
	if (target == NULL)
		return;
	...
	switch (message->what) {
		...
		case B_MOUSE_DOWN:
		{
			...
			if (view == NULL || message->what == B_MOUSE_MOVED) {
				// add local jwindow coordinates, only
...
/*!
	...
	It allows the jwindow to update the last mouse moved view, and
...
*/
bool
JSWindow::_StealMouseMessage(BMessage* message, bool& deleteMessage)
{
	BMessage::Private messagePrivate(message);
	if (!messagePrivate.UsePreferredTarget()) {
...
uint32
JSWindow::_TransitForMouseMoved(BView* view, BView* viewUnderMouse) const
{
	uint32 transit;
...
*/
void
JSWindow::_Switcher(int32 rawKey, uint32 modifiers, bool repeat)
{
...
*/
bool
JSWindow::_HandleKeyDown(BMessage* event)
{
	...
	// Optionally close jwindow when the escape key is pressed
	...
	if ((modifiers & B_COMMAND_KEY) != 0) {
		...
		// the shortcut mechanism doesn't allow handlers outside the jwindow
...
bool
JSWindow::_HandleUnmappedKeyDown(BMessage* event)
{
	// Only handle special functions when the event targeted the active focus
	// view
	if (!_IsFocusMessage(event))
...
/*!
	\brief Return the position of the jwindow centered horizontally to the passed
           in \a frame and vertically 3/4 from the top of \a frame.

	If the jwindow is on the borders

	\param width The width of the jwindow.
	\param height The height of the jwindow.
	\param frame The \a frame to center the jwindow in.

	\return The new jwindow position.
*/
BPoint
JSWindow::AlertPosition(const BRect& frame)
{
	...
	// lower the jwindow down if it is covering the jwindow tab
...
BMessage*
JSWindow::ConvertToMessage(void* raw, int32 code)
{
...
JSWindow::Shortcut*
JSWindow::_FindShortcut(uint32 key, uint32 modifiers)
{
...
BView*
JSWindow::_FindView(int32 token)
{
	BHandler* handler;
...
BView*
JSWindow::_FindView(BView* view, BPoint point) const
{
...
BView*
JSWindow::_FindNextNavigable(BView* focus, uint32 flags)
{
	if (focus == NULL)
		focus = fTopView;
...
BView*
JSWindow::_FindPreviousNavigable(BView* focus, uint32 flags)
{
	if (focus == NULL)
		focus = fTopView;
...
*/
BView*
JSWindow::_LastViewChild(BView* parent)
{
	while (true) {
...
void
JSWindow::SetIsFilePanel(bool isFilePanel)
{
...
void
JSWindow::_GetDecoratorSize(float* _borderWidth, float* _tabHeight) const
{
...
void
JSWindow::_SendShowOrHideMessage()
{
...
extern "C" void
_ReservedWindow1__7JSWindow(JSWindow* jwindow, BLayout* layout)
{
	// SetLayout()
	perform_data_set_layout data;
	data.layout = layout;
	jwindow->Perform(PERFORM_CODE_SET_LAYOUT, &data);
}


void JSWindow::_ReservedWindow2() {}
void JSWindow::_ReservedWindow3() {}
void JSWindow::_ReservedWindow4() {}
void JSWindow::_ReservedWindow5() {}
void JSWindow::_ReservedWindow6() {}
void JSWindow::_ReservedWindow7() {}
void JSWindow::_ReservedWindow8() {}
