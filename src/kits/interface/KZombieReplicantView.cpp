...
_KZombieReplicantView_::_KZombieReplicantView_(BRect frame, status_t error)
	:
	KBox(frame, "<Zombie>", B_FOLLOW_NONE, B_WILL_DRAW),
	fError(error)
{
...
_KZombieReplicantView_::~_KZombieReplicantView_()
{
}


void
_KZombieReplicantView_::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		case B_ABOUT_REQUESTED:
		{
			...
			KAlert* alert = new (std::nothrow) KAlert(B_TRANSLATE("Error"),
...
void
_KZombieReplicantView_::Draw(BRect updateRect)
{
	BRect bounds(Bounds());
	font_height fh;
	...
	KBox::Draw(updateRect);
}


void
_KZombieReplicantView_::MouseDown(BPoint)
{
}


status_t
_KZombieReplicantView_::Archive(BMessage* archive, bool) const
{
	*archive = *fArchive;
...
void
_KZombieReplicantView_::SetArchive(BMessage* archive)
{
	fArchive = archive;
}
