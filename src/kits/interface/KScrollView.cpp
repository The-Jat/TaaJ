...
KScrollView::KScrollView(const char* name, KView* target, uint32 resizingMode,
	uint32 flags, bool horizontal, bool vertical, border_style border)
	:
	KView(BRect(), name, resizingMode, _ModifyFlags(flags, target, border)),
	fTarget(target),
	fBorder(border)
{
...
KScrollView::KScrollView(const char* name, KView* target, uint32 flags,
	bool horizontal, bool vertical, border_style border)
	:
	KView(name, _ModifyFlags(flags, target, border)),
	fTarget(target),
	fBorder(border)
{
	_Init(horizontal, vertical);
...
KScrollView::KScrollView(BMessage* archive)
	:
	KView(archive),
	fHighlighted(false)
{
	...
	while ((view = ChildAt(firstBar++)) != NULL) {
		KScrollBar *bar = dynamic_cast<KScrollBar *>(view);
		if (bar == NULL)
			continue;
...
BArchivable*
KScrollView::Instantiate(BMessage* archive)
{
	if (validate_instantiation(archive, "KScrollView"))
		return new KScrollView(archive);
...
status_t
KScrollView::Archive(BMessage* archive, bool deep) const
{
	status_t status = KView::Archive(archive, deep);
	if (status != B_OK)
		return status;

	// If this is a deep archive, the KView class will take care
	// of our children.
...
status_t
KScrollView::AllUnarchived(const BMessage* archive)
{
	status_t result = KView::AllUnarchived(archive);
	if (result != B_OK)
		return result;
	...
	while ((view = ChildAt(firstBar++)) != NULL) {
		...
		// So the target view can't be a KScrollBar, but who would do that?
		if (bar == NULL) {
			// in a shallow archive, we may not have a target anymore. We must
			// be prepared for this case
...
void
KScrollView::DetachedFromWindow()
{
...
void
KScrollView::Draw(BRect updateRect)
{
	uint32 flags = 0;
	...
	k_be_control_look->DrawScrollViewFrame(this, rect, updateRect,
...
void
KScrollView::FrameMoved(BPoint newPosition)
{
...
void
KScrollView::FrameResized(float newWidth, float newHeight)
{
...
void
KScrollView::MessageReceived(BMessage* message)
{
...
void
KScrollView::MouseDown(BPoint where)
{
...
void
KScrollView::MouseMoved(BPoint where, uint32 code,
...
void
KScrollView::MouseUp(BPoint where)
{
	KView::MouseUp(where);
}
...
void
KScrollView::WindowActivated(bool active)
{
	if (fHighlighted)
		Invalidate();
...
void
KScrollView::GetPreferredSize(float* _width, float* _height)
{
...
void
KScrollView::MakeFocus(bool focus)
{
...
// #pragma mark - KScrollView methods


KScrollBar*
KScrollView::ScrollBar(orientation direction) const
{
...
void
KScrollView::SetBorder(border_style border)
{
	if (fBorder == border)
		return;
...
void
KScrollView::SetBorders(uint32 borders)
{
...
status_t
KScrollView::SetBorderHighlighted(bool highlight)
{
...
bool
KScrollView::IsBorderHighlighted() const
{
	return fHighlighted;
}
...
void
KScrollView::SetTarget(KView* target)
{
	if (fTarget == target)
		return;
	...
	if (target != NULL) {
		...
		target->MoveTo((fBorders & KControlLook::B_LEFT_BORDER) != 0
			? borderSize : 0, (fBorders & KControlLook::B_TOP_BORDER) != 0
				? borderSize : 0);
...
BHandler*
KScrollView::ResolveSpecifier(BMessage* message, int32 index,
...
status_t
KScrollView::GetSupportedSuites(BMessage* message)
{
...
status_t
KScrollView::Perform(perform_code code, void* _data)
{
	switch (code) {
		...
		case PERFORM_CODE_LAYOUT_ALIGNMENT:
			((perform_data_layout_alignment*)_data)->return_value
				= KScrollView::LayoutAlignment();
			return B_OK;

		case PERFORM_CODE_HAS_HEIGHT_FOR_WIDTH:
			((perform_data_has_height_for_width*)_data)->return_value
				= KScrollView::HasHeightForWidth();
			return B_OK;
		...
		case PERFORM_CODE_GET_HEIGHT_FOR_WIDTH:
		{
			...
			KScrollView::GetHeightForWidth(data->width, &data->min, &data->max,
				&data->preferred);
			return B_OK;
		}
		...
		case PERFORM_CODE_SET_LAYOUT:
		{
			...
			KScrollView::SetLayout(data->layout);
			return B_OK;
		}
		...
		case PERFORM_CODE_LAYOUT_INVALIDATED:
		{
			...
			KScrollView::LayoutInvalidated(data->descendants);
			return B_OK;
		}

		case PERFORM_CODE_DO_LAYOUT:
		{
			KScrollView::DoLayout();
			return B_OK;
		}
	}
...
void
KScrollView::LayoutInvalidated(bool descendants)
{
}
...
KScrollView::DoLayout()
{
	...
	if (fTarget != NULL) {
		...
		//KLayoutUtils::AlignInFrame(fTarget, fTarget->Bounds());
	}
...
void
KScrollView::_Init(bool horizontal, bool vertical)
{
	fHorizontalScrollBar = NULL;
	fVerticalScrollBar = NULL;
	fHighlighted = false;
	...
	if (horizontal) {
		fHorizontalScrollBar = new KScrollBar(BRect(0, 0, 14, 14), "_HSB_",
			fTarget, 0, 1000, B_HORIZONTAL);
		AddChild(fHorizontalScrollBar);
	}

	if (vertical) {
		fVerticalScrollBar = new KScrollBar(BRect(0, 0, 14, 14), "_VSB_",
	...
	if ((Flags() & B_SUPPORTS_LAYOUT) == 0) {
		BRect frame = _ComputeFrame(fTarget, fHorizontalScrollBar,
			fVerticalScrollBar, fBorder, KControlLook::B_ALL_BORDERS);
...
BSize
KScrollView::_ComputeSize(BSize targetSize) const
{
...
BRect
KScrollView::_ComputeFrame(BRect targetRect) const
{
...
void
KScrollView::_AlignScrollBars(bool horizontal, bool vertical, BRect targetFrame)
{
	if (horizontal) {
...
*/
/*static*/ BRect
KScrollView::_ComputeFrame(BRect frame, KScrollBar* horizontal,
	KScrollBar* vertical, border_style border, uint32 borders)
{
	if (vertical != NULL)
...
/*static*/ BRect
KScrollView::_ComputeFrame(KView *target, KScrollBar* horizontal,
	KScrollBar* vertical, border_style border, uint32 borders)
{
...
*/
/*static*/ float
KScrollView::_BorderSize(border_style border)
{
...
/*!	This method changes the "flags" argument as passed on to
	the KView constructor.
*/
/*static*/ uint32
KScrollView::_ModifyFlags(uint32 flags, KView* target, border_style border)
{
...
/*static*/ void
KScrollView::_InsetBorders(BRect& frame, border_style border, uint32 borders, bool expand)
{
	float borderSize = _BorderSize(border);
	if (expand)
		borderSize = -borderSize;
	if ((borders & KControlLook::B_LEFT_BORDER) != 0)
		frame.left += borderSize;
	if ((borders & KControlLook::B_TOP_BORDER) != 0)
		frame.top += borderSize;
	if ((borders & KControlLook::B_RIGHT_BORDER) != 0)
		frame.right -= borderSize;
	if ((borders & KControlLook::B_BOTTOM_BORDER) != 0)
...
KScrollView&
KScrollView::operator=(const KScrollView &)
{
	return *this;
}


void KScrollView::_ReservedScrollView1() {}
void KScrollView::_ReservedScrollView2() {}
void KScrollView::_ReservedScrollView3() {}
void KScrollView::_ReservedScrollView4() {}


extern "C" void
B_IF_GCC_2(InvalidateLayout__11KScrollViewb,
	_ZN11KScrollView16InvalidateLayoutEb)(KScrollView* view, bool descendants)
{
	perform_data_layout_invalidated data;
