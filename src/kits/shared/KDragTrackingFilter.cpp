...
KDragTrackingFilter::KDragTrackingFilter(KView* targetView, uint32 messageWhat)
...
filter_result
KDragTrackingFilter::Filter(BMessage* message, BHandler** /*_target*/)
{
	if (fTargetView == NULL)
			...
			if (fIsTracking && (fClickButtons & B_PRIMARY_MOUSE_BUTTON)) {
				...
				if (squaredDelta >= kSquaredDragThreshold) {
					...
					dragClickMessage.AddPoint("be:view_where", fClickPoint);
						// name it "be:view_where" since KView::DragMessage
