...
KCalendarView::KCalendarView(BRect frame, const char* name, uint32 resizeMask,
	uint32 flags)
	:
...
KCalendarView::KCalendarView(const char* name, uint32 flags)
	:
	KView(name, flags),
	BInvoker(),
	fSelectionMessage(NULL),
...
KCalendarView::KCalendarView(BMessage* archive)
	:
	KView(archive),
	BInvoker(),
	fSelectionMessage(NULL),
...
BArchivable*
KCalendarView::Instantiate(BMessage* archive)
{
	if (validate_instantiation(archive, "KCalendarView"))
		return new KCalendarView(archive);
...
status_t
KCalendarView::Archive(BMessage* archive, bool deep) const
{
...
void
KCalendarView::AttachedToWindow()
{
...
void
KCalendarView::FrameResized(float width, float height)
{
	_SetupDayNames();
	Invalidate(Bounds());
}
...
void
KCalendarView::Draw(BRect updateRect)
{
	if (LockLooper()) {
		if (fFocusChanged) {
			_DrawFocusRect();
...
void
KCalendarView::DrawDay(KView* owner, BRect frame, const char* text,
...
void
KCalendarView::DrawDayName(KView* owner, BRect frame, const char* text)
{
...
void
KCalendarView::DrawWeekNumber(KView* owner, BRect frame, const char* text)
{
	// we get the full rect, fake this as the internal function
	// shrinks the frame to work properly when drawing a day item
	_DrawItem(owner, frame.InsetByCopy(-1.0, -1.0), text, true);
...
uint32
KCalendarView::SelectionCommand() const
{
	if (SelectionMessage())
...
BMessage*
KCalendarView::SelectionMessage() const
{
...
void
KCalendarView::SetSelectionMessage(BMessage* message)
{
...
uint32
KCalendarView::InvocationCommand() const
{
...
BMessage*
KCalendarView::InvocationMessage() const
{
...
void
KCalendarView::SetInvocationMessage(BMessage* message)
{
...
void
KCalendarView::MakeFocus(bool state)
{
	if (IsFocus() == state)
		return;
...
status_t
KCalendarView::Invoke(BMessage* message)
{
	bool notify = false;
...
void
KCalendarView::MouseDown(BPoint where)
{
	if (!IsFocus()) {
		MakeFocus();
		Sync();
...
void
KCalendarView::KeyDown(const char* bytes, int32 numBytes)
{
	const int32 kRows = 6;
...
void
KCalendarView::ResizeToPreferred()
{
	float width;
	float height;
...
void
KCalendarView::GetPreferredSize(float* width, float* height)
{
...
BSize
KCalendarView::MinSize()
{
	float width, height;
	_GetPreferredSize(&width, &height);
	return KLayoutUtils::ComposeSize(ExplicitMinSize(), BSize(width, height));
}


BSize
KCalendarView::PreferredSize()
{
	return KLayoutUtils::ComposeSize(ExplicitPreferredSize(), MinSize());
}


int32
KCalendarView::Day() const
{
	return fDate.Day();
}
...
bool
KCalendarView::SetDay(int32 day)
{
	BDate date = Date();
	date.SetDay(day);
	if (!date.IsValid())
...
bool
KCalendarView::SetMonth(int32 month)
{
	if (month < 1 || month > 12)
		return false;
	BDate date = Date();
...
bool
KCalendarView::SetYear(int32 year)
{
	BDate date = Date();
...
bool
KCalendarView::SetDate(const BDate& date)
{
	if (!date.IsValid())
		return false;
...
bool
KCalendarView::SetDate(int32 year, int32 month, int32 day)
{
...
BWeekday
KCalendarView::StartOfWeek() const
{
...
void
KCalendarView::SetStartOfWeek(BWeekday startOfWeek)
{
...
bool
KCalendarView::IsDayNameHeaderVisible() const
{
...
void
KCalendarView::SetDayNameHeaderVisible(bool visible)
{
...
void
KCalendarView::UpdateDayNameHeader()
{
	if (!fDayNameHeaderVisible)
		return;
...
bool
KCalendarView::IsWeekNumberHeaderVisible() const
{
...
void
KCalendarView::SetWeekNumberHeaderVisible(bool visible)
{
...
void
KCalendarView::_SetToCurrentDay()
{
...
void
KCalendarView::_GetYearMonthForSelection(const Selection& selection,
...
void
KCalendarView::_GetPreferredSize(float* _width, float* _height)
{
	BFont font;
	GetFont(&font);
...
void
KCalendarView::_PopulateDayNames(BDateFormatStyle style)
{
	for (int32 i = 0; i <= 6; ++i) {
		fDayNames[i] = "";
...
void
KCalendarView::_SetupDayNumbers()
{
	BDate startOfMonth(fDate.Year(), fDate.Month(), 1);
...
void
KCalendarView::_SetupWeekNumbers()
{
	BDate date(fDate.Year(), fDate.Month(), 1);
	if (!date.IsValid())
		return;
...
void
KCalendarView::_DrawDay(int32 currRow, int32 currColumn, int32 row,
...
void
KCalendarView::_DrawItem(KView* owner, BRect frame, const char* text,
...
void
KCalendarView::_UpdateSelection()
{
	BRect frame = _FirstCalendarItemFrame();
...
void
KCalendarView::_UpdateCurrentDay()
{
	BRect frame = _FirstCalendarItemFrame();
...
void
KCalendarView::_UpdateCurrentDate()
{
...
BRect
KCalendarView::_FirstCalendarItemFrame() const
{
	int32 rows = 7;
	int32 columns = 8;
...
BRect
KCalendarView::_SetNewSelectedDay(const BPoint& where)
{
	BRect frame = _FirstCalendarItemFrame();
...
BRect
KCalendarView::_RectOfDay(const Selection& selection) const
{
	BRect frame = _FirstCalendarItemFrame();
