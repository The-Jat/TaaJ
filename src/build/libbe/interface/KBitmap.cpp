...
*/
class KPaletteConverter {
public:
	KPaletteConverter();
	KPaletteConverter(const rgb_color *palette);
	KPaletteConverter(const color_map *colorMap);
	~KPaletteConverter();
...
using BPrivate::KPaletteConverter;
using namespace std;


/*!	\brief Creates an uninitialized KPaletteConverter.
*/
KPaletteConverter::KPaletteConverter()
	: fColorMap(NULL),
	  fOwnColorMap(NULL),
	  fCStatus(B_NO_INIT)
{
...
/*!	\brief Creates a KPaletteConverter and initializes it to the supplied
		   palette.
	\param palette The palette being a 256 entry rgb_color array.
*/
KPaletteConverter::KPaletteConverter(const rgb_color *palette)
	: fColorMap(NULL),
	  fOwnColorMap(NULL),
	  fCStatus(B_NO_INIT)
{
...
*/
KPaletteConverter::KPaletteConverter(const color_map *colorMap)
	: fColorMap(NULL),
	  fOwnColorMap(NULL),
	  fCStatus(B_NO_INIT)
{
...
*/
status_t
KPaletteConverter::SetTo(const rgb_color *palette)
{
	// cleanup
...
*/
status_t
KPaletteConverter::SetTo(const color_map *colorMap)
{
	// cleanup
	if (fOwnColorMap) {
		delete fOwnColorMap;
...
*/
status_t
KPaletteConverter::InitCheck() const
{
	return fCStatus;
}
...
*/
inline uint8
KPaletteConverter::IndexForRGB15(uint16 rgb) const
{
...
*/
inline uint8
KPaletteConverter::IndexForRGB15(uint8 red, uint8 green, uint8 blue) const
{
...
*/
inline uint8
KPaletteConverter::IndexForRGB16(uint16 rgb) const
{
...
*/
inline uint8
KPaletteConverter::IndexForRGB16(uint8 red, uint8 green, uint8 blue) const
{
...
*/
inline uint8
KPaletteConverter::IndexForRGB24(uint32 rgb) const
{
...
*/
inline uint8
KPaletteConverter::IndexForRGB24(uint8 red, uint8 green, uint8 blue) const
{
...
*/
inline uint8
KPaletteConverter::IndexForGray(uint8 gray) const
{
...
KPaletteConverter::RGBColorForIndex(uint8 index) const
{
...
*/
inline uint16
KPaletteConverter::RGB15ColorForIndex(uint8 index) const
{
...
*/
inline uint16
KPaletteConverter::RGB16ColorForIndex(uint8 index) const
{
	const rgb_color &color = fColorMap->color_list[index];
...
*/
inline uint32
KPaletteConverter::RGB24ColorForIndex(uint8 index) const
{
	const rgb_color &color = fColorMap->color_list[index];
...
*/
inline void
KPaletteConverter::RGB24ColorForIndex(uint8 index, uint8 &red, uint8 &green,
	uint8 &blue, uint8 &alpha) const
{
	const rgb_color &color = fColorMap->color_list[index];
	red = color.red;
...
*/
inline uint8
KPaletteConverter::GrayColorForIndex(uint8 index) const
{
	const rgb_color &color = fColorMap->color_list[index];
...
static KPaletteConverter	gPaletteConverter;


/*!	\brief Returns a KPaletteConverter using the system color palette.
	\return A KPaletteConverter.
*/
static const KPaletteConverter*
palette_converter()
{
...
struct CMAP8Reader : public BaseReader<uint8> {
	...
	CMAP8Reader(const void *data, const KPaletteConverter &converter)
	...
	const KPaletteConverter &converter;
};
...
struct CMAP8Writer : public BaseWriter<uint8> {
	...
	CMAP8Writer(void *data, const KPaletteConverter &converter)
...
/*!	\brief Helper function that reads bitmap data from one buffer and writes
	...
	\param reader A reader object. The pointer to the data doesn't need to
		   be initialized.
	\param paletteConverter Reference to a KPaletteConverter to be used, if
...
set_bits(const void *inData, int32 inLength, int32 inBPR, int32 inRowSkip,
		 ...
		 Reader reader, const KPaletteConverter &paletteConverter)
{
	status_t error = B_OK;
	  ...
	  fInitError(B_NO_INIT)
{
	...
	InitObject(bounds, colorSpace, flags, K_ANY_BYTES_PER_ROW,
			   B_MAIN_SCREEN_ID);
...
/*!	\brief Creates a KBitmap as a clone of another bitmap.
	...
	\param needsContiguous If \c true a physically contiguous chunk of memory
		   will be allocated.
*/
KBitmap::KBitmap(const KBitmap *source, bool acceptsViews,
		bool needsContiguous)
	: fBasePtr(NULL),
	  fSize(0),
	  fColorSpace(B_NO_COLOR_SPACE),
	...
	color_space colorSpace)
{
	...
	if (error == B_OK) {
		...
		inRowSkip = bpr - k_get_raw_bytes_per_row(colorSpace, width);
		if (inRowSkip < 0)
			error = B_BAD_VALUE;
	}
	if (error != B_OK) {
		...
		// KPaletteConverter paletteConverter(BScreen().ColorMap());
		const KPaletteConverter &paletteConverter = *palette_converter();
		int32 rawOutBPR = k_get_raw_bytes_per_row(fColorSpace, width);
		switch (colorSpace) {
			// supported
			case B_RGB32: case B_RGBA32:
			{
...
int32
KBitmap::get_server_token() const
{
	return fServerToken;
}
...
*/
void
KBitmap::InitObject(BRect bounds, color_space colorSpace, uint32 flags,
	int32 bytesPerRow, screen_id screenID)
{
